...
| 3.5 | pathogen_detection (bulk access to the NCBI Pathogen Detection FTP snapshot tree) and clinicaltrials_search (ClinicalTrials.gov API v2), completing the seven-tool roster | Closed on `phase/3.5-pathogen-clinicaltrials-tools`, 2026-08-08, after a judge round, an adversary round, and two fix rounds |

Build phase 3.3 (`pubtator_annotate` and `litvar2_lookup`) closed on `phase/3.3-enrichment-tools`, merged as PR #26, on 2026-08-08 after ten review rounds: a blocking premise gate written and watched failing first, a judge round (FAIL, two majors), a fix round, an independent re-review that found a real regression in that fix round, a second fix round, an adversary round against the live APIs (13 findings, 1 critical: both tools silently discarded the upstream API's own match-relevance signal, so a bare number or a common word returned confidently cited but wholly unrelated data), a third fix round, a fourth fix round closing several carried findings that turned out addressable without a product decision after all, an independent re-review of that round that found one more real regression (a multi-match result citing only its first, unrelated match as if it covered the whole answer), and a fifth fix round closing it. Full account: `tracker/phase_3.3.md`. Build phase 3.2 (`ncbi_dbsnp`) closed on `phase/3.2-ncbi-dbsnp` on 2026-08-08 after six full review passes: a blocking premise gate written and watched failing first, an adversary round (14 findings), a judge round (FAIL, 6 more findings), a fix round closing all five confirmed-blocking findings, an independent fresh-context re-review that found two more defects in that fix round itself, and a second fix round closing both. Three spec-versus-reality gaps carried to Step 6.2, including Section 6.3's named SPDI normalization endpoint, confirmed broken server-side and substituted with a live-working sibling endpoint. Full account: `tracker/phase_3.2.md`. Build phase 3.1 (`ncbi_efetch`) merged as PR #22 (superseded by PR #23), delivering the first Layer 2 tool with live gene-symbol resolution, then closed out fully via PR #23 on 2026-08-07 after three independent re-review rounds paid off the re-review debt PR #22 left open. The F-2.1-C15 generation bound, the finding where a generated query could take the graph server down for every user, closed on `fix/c15-generation-bound`, merged as PR #24 on 2026-08-07. Build phase 3.5 (`pathogen_detection` and `clinicaltrials_search`, completing the seven-tool roster) closed 2026-08-08 on `phase/3.5-pathogen-clinicaltrials-tools`, after a judge round, an adversary round, and two fix rounds. Full account: `tracker/phase_3.5.md`. Next: build phase 3.4, now unblocked since Section 25 makes it depend on 2.2 and 3.1 through 3.5, all closed. Build phase 3.0 (the full Section 10 guardrail) merged as PR #19 on 2026-08-04. Build phase 2.2 closed 2026-08-03 and completed the Step 6.1 prototype group. Step 6.2, the reconciliation pause, moved to run after the 3.x tool phases. All seven tools now closed (build phases 2.1 and 3.1 through 3.5), pending phase 3.5's merge. Build order: 26 numbered phases (1.0 to 7.1) in Section 25 of the [Technical specification](requirements/Technical_specification.md).

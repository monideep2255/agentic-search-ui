...
| Build (Phases 6-7) | In progress. Step 6.1, the prototype, is complete: six build phases merged into develop. Step 6.3, build v1, ran build phases 3.0 through 3.5, all merged; Step 6.2, the reconciliation pause, closed 2026-08-10; build phases 4.0 and 4.1 closed 2026-08-11; build phases 4.8 and 4.9 (web UI visual design and answer-screen fidelity) closed 2026-08-13 and 2026-08-14; build phase 4.10 (the anonymous run path and the server-side guest allowance) closed 2026-08-15, PR #46; build phase 4.2 (the CLI adapter, `system3-cli`, command `s3`) closed 2026-08-16, PR #47; build phase 4.3 (the GraphQL surface via Strawberry) closed 2026-08-17, PR #48; build phase 4.4 (KGX export) closed 2026-08-19, PR #51; build phase 4.5 (personalization and session memory) closed 2026-08-20, PR #52, then reviewed and fixed in PR #53 on 2026-08-21; it was the first phase to merge with no judge or adversary round, and that gap was closed afterwards; build phase 4.6 (feedback capture) closed 2026-08-21, PR #54; build phase 4.11 (the read-only HTTPS graph query service) closed 2026-08-22, PR #55, replacing the hand-opened SSH tunnel with an authenticated HTTPS endpoint on the Hetzner box and closing a dangling reference between two locked sections; build phase 4.7 (competency-question routing) closed 2026-08-23, PR #56, replacing `think_node`'s build-phase-2.0 stub with real query-shape classification and live-confirmed entity resolution, and merged with two criticals open on dedicated branches. BOTH CLOSED on 2026-08-24, `fix/a02-discontinued-gene-record` as PR #59 and `fix/a01-injection-guardrail` as PR #60, the latter also taking F-4.7-A-05's guardrail half: a discontinued gene record is no longer answered as its successor, and injected text no longer chooses which gene the retrieval runs on. Measured rather than asserted, Guard-tier admission for the three injection payloads fell from 12 of 18 to 0 of 18, and the deterministic control now refuses them at no cost with no model call reached. Step 6.3 continues next at build phase 4.12, the demo deployment, now unblocked apart from the security scan it already owns, whose trigger is exposure. See the table below |
...
Build phase 3.3 (`pubtator_annotate` and `litvar2_lookup`) closed on `phase/3.3-enrichment-tools`, merged as PR #26, on 2026-08-08 after ten review rounds: a blocking premise gate written and watched failing first, a judge round (FAIL, two majors), a fix round, an independent re-review that found a real regression in that fix round, a second fix round, an adversary round against the live APIs (13 findings, 1 critical: both tools silently discarded the upstream API's own match-relevance signal, so a bare number or a common word returned confidently cited but wholly unrelated data), a third fix round, a fourth fix round closing several carried findings that turned out addressable without a product decision after all, an independent re-review of that round that found one more real regression (a multi-match result citing only its first, unrelated match as if it covered the whole answer), and a fifth fix round closing it. Full account: `tracker/phase_3.3.md`. Build phase 3.2 (`ncbi_dbsnp`) closed on `phase/3.2-ncbi-dbsnp` on 2026-08-08 after six full review passes: a blocking premise gate written and watched failing first, an adversary round (14 findings), a judge round (FAIL, 6 more findings), a fix round closing all five confirmed-blocking findings, an independent fresh-context re-review that found two more defects in that fix round itself, and a second fix round closing both. Three spec-versus-reality gaps carried to Step 6.2, including Section 6.3's named SPDI normalization endpoint, confirmed broken server-side and substituted with a live-working sibling endpoint. Full account: `tracker/phase_3.2.md`. Build phase 3.1 (`ncbi_efetch`) merged as PR #22 (superseded by PR #23), delivering the first Layer 2 tool with live gene-symbol resolution, then closed out fully via PR #23 on 2026-08-07 after three independent re-review rounds paid off the re-review debt PR #22 left open. The F-2.1-C15 generation bound, the finding where a generated query could take the graph server down for every user, closed on `fix/c15-generation-bound`, merged as PR #24 on 2026-08-07. Build phase 3.5 (`pathogen_detection` and `clinicaltrials_search`, completing the seven-tool roster) closed on `phase/3.5-pathogen-clinicaltrials-tools`, merged as PR #27, on 2026-08-08 after a judge round, an adversary round, and two fix rounds. Full account: `tracker/phase_3.5.md`. Build phase 3.4 (citation trust extended to Layers 2 and 3, the two-tier risk gate, data freshness and conflict resolution, T-3.1-28 folded in) closed on `phase/3.4-citation-trust-full`, merged as PR #28, on 2026-08-10 after two judge rounds, an adversary round (7 findings, 1 critical), and two fix rounds, the last of the six Step 6.3 tool-and-trust phases. Full account: `tracker/phase_3.4.md`. Build phase 3.0 (the full Section 10 guardrail) merged as PR #19 on 2026-08-04. Build phase 2.2 closed 2026-08-03 and completed the Step 6.1 prototype group. All seven tools now closed (build phases 2.1 and 3.1 through 3.5), all merged into develop. Step 6.2, the reconciliation pause, ran and closed the same day as build phase 3.4, 2026-08-10, across eight PRs (#29 through #36): the branch rename (`main` to `develop`), the carried grounding findings, the fourteen spec-vs-reality gaps, and a manual smoke test of the seven must-pass moat questions that surfaced one new finding, F-2.0-15 (`think_node`'s real query classification was never built past its build-phase-2.0 stub, causing 4 of 7 must-pass questions to refuse outright), assigned to build phase 4.7 as that phase's real deliverable, decided the same day. Build phase 4.0 (the REST plus SSE adapter finalized as the public API surface) closed on `phase/4.0-rest-sse-hardening`, merged as PR #39, on 2026-08-11 after four judge rounds (round 1 FAIL, two blocking findings) and an adversary round against the live app (14 findings; 10 closed, 4 carried open to build phase 6.0 and later). Full account: `tracker/phase_4.0.md`. Build phase 4.1 (the outbound-only MCP server wrapping the same tool functions) closed on `phase/4.1-mcp-server` on 2026-08-11 after three judge rounds (round 1 FAIL, one blocking and three non-blocking gate-integrity findings; rounds 2 and 3 PASS) and an adversary round against the live mounted app (16 findings: two critical, four major, four moderate, six minor; 14 closed or closed-with-a-named-residual, 2 carried open as product-level calls). Full account: `tracker/phase_4.1.md`. Build phase 4.10 (the anonymous run path and the server-side guest allowance) closed on `phase/4.10-guest-allowance`, merged as PR #46, on 2026-08-15, after build phases 4.8 and 4.9 (web UI visual design and answer-screen fidelity, closed 2026-08-13 and 2026-08-14) ran out of Section 25's normal order by product-owner directive on 2026-08-14. With 4.10 closed, Section 25's own order resumed at build phase 4.2 (the CLI adapter, `system3-cli`, command `s3`, over the REST plus SSE surface), closed 2026-08-16 as PR #47, and continued at build phase 4.3 (the GraphQL surface via Strawberry, over the same core, registered accounts only, no live stream), closed 2026-08-17 as PR #48 after six independent review rounds, and continued at build phase 4.4 (KGX export, a query-scoped subgraph export, not a full-graph snapshot), closed 2026-08-19 as PR #51 after three review rounds under the newly capped review loop. Build phase 4.5 (personalization and session memory: bounded in-conversation memory, audience-level depth control, and the stable named scientist persona) closed 2026-08-20 as PR #52. It closed three long-open findings, F-4.1-A-15, F-4.8-A-22 and F-4.2-03, and it is the first phase in this build to merge with no judge round and no adversary round; two criticals in it were found after the work looked finished, one by the premise gate and one only by asking what populates session memory. Full account: `tracker/phase_4.5.md`. Build phase 4.6 (feedback capture) closed 2026-08-21 as PR #54. Build phase 4.11 (the read-only HTTPS graph query service) closed 2026-08-22 as PR #55, after four review rounds against a two-round cap and three Rule 4 stops: it deletes the hand-opened SSH tunnel that had gated every live test since build phase 2.1, verified by 59 of 59 live premise arms passing with no tunnel open. Its most transferable result is that three vacuous gate arms were written and all three were caught by mutation rather than by reading, which is why every bound arm now carries a populate-check. Full account: `tracker/phase_4.11.md`. Build phase 4.7 (competency-question routing, taken ahead of build phase 4.12 on 2026-08-22) closed 2026-08-23 as PR #56, replacing `think_node`'s build-phase-2.0 stub with real query-shape classification and live-confirmed entity resolution, and merged with two criticals open on dedicated branches, `fix/a01-injection-guardrail` and `fix/a02-discontinued-gene-record`, both hard blockers on build phase 4.12 before any public URL. Full account: `tracker/phase_4.7.md`. Section 25's order now sits at build phase 4.12, the demo deployment, held on those two branches closing. Build order: 26 numbered phases (1.0 to 7.1) in Section 25 of the [Technical specification](requirements/Technical_specification.md), plus the two board-level insertions above pending the next reconciliation.

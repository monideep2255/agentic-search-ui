...
Build phase 3.2 (`ncbi_dbsnp`) closed on `phase/3.2-ncbi-dbsnp` on 2026-08-08 after six full review passes: a blocking premise gate written and watched failing first, an adversary round (14 findings), a judge round (FAIL, 6 more findings), a fix round closing all five confirmed-blocking findings, an independent fresh-context re-review that found two more defects in that fix round itself, and a second fix round closing both. Three spec-versus-reality gaps carried to Step 6.2, including Section 6.3's named SPDI normalization endpoint, confirmed broken server-side and substituted with a live-working sibling endpoint. Full account: `tracker/phase_3.2.md`. Build phase 3.1 (`ncbi_efetch`) merged as PR #22 on 2026-08-05, delivering the first Layer 2 tool with live gene-symbol resolution, then closed out fully via PR #23 on 2026-08-07 after three independent re-review rounds paid off the re-review debt PR #22 left open. The F-2.1-C15 generation bound, the finding where a generated query could take the graph server down for every user, closed on `fix/c15-generation-bound`, merged as PR #24 on 2026-08-07. Next: build phase 3.3, `pubtator_annotate` and `litvar2_lookup`, the two Layer 3 enrichment tools. Build phase 3.0 (the full Section 10 guardrail) merged as PR #19 on 2026-08-04. Build phase 2.2 closed 2026-08-03 and completed the Step 6.1 prototype group. Step 6.2, the reconciliation pause, moved to run after the 3.x tool phases. Tools for Layers 2 and 3, three of six remaining (build phases 3.3 to 3.5). Build order: 26 numbered phases (1.0 to 7.1) in Section 25 of the [Technical specification](requirements/Technical_specification.md).

...
| 4 | System 3: eval and tracing | IN PROGRESS. Build phase 5.0, observability, is COMPLETE on `phase/5.0-observability` and NOT YET MERGED, its pull request the next step: tech spec Section 20 in full, as three records with one job each, LangSmith per-run tracing joined on `trace_id`, PostHog behavioural analytics as aggregates only, and the append-only JSONL tool-call audit log. Build phase 5.1, the 50-query golden dataset and the eval harness against the playbook, is next and depends on it. |

...
The reference symlinks are documentation. Never copy a file from `reference/` into `system-01-data-pipelines/` or `system-02-knowledge-graph/`. Always rewrite from first principles, informed by the reference. This is a project-level rule (see AGENTS.md "Canonical reference pipeline" section and the user's stated preference).

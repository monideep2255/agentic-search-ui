...
- `.claude/hooks/block-bash-delete.sh` now covers the quoted shape for the common wrappers. It matches `rm` and `rmdir` as whole command words after a separator, an opener, `find -exec` or a runner such as `sudo` or `xargs`. It also blocks a destructive word (`rm`, `rmdir`, `rmtree`, `dd if=`, `mkfs`, a write into `/dev`) anywhere in a command that names an execution wrapper (`ssh`, `python -c`, `perl -e`, `ruby -e`, `node -e`, `bash -c`, `sh -c`, `zsh -c`, `eval`). It is still a plain-text regex, not a shell parser.
- What the hook does not see, checked on 2026-09-26: a wrapper outside that list, such as `osascript -e`, and deletion that never names those words, such as `find -delete`, `unlink`, `shred`, `git clean` or Python's `os.remove`. `tests/ci/test_claude_hooks.py` states the same coverage.
...
- Keep the hook's wrapper scan, and widen it before relying on it for a wrapper or a deletion it does not see today: `osascript -e`, `find -delete`, `unlink`, `shred`, `git clean` and `os.remove` all pass it. Widening it is a change to the security layer, so it needs the product owner's item-by-item yes.

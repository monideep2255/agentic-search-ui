...
- What the hook does not see, checked on 2026-09-26, which `tests/ci/test_claude_hooks.py` states too:
  ...
  - Text that reaches a shell another way, such as `bash <(...)` or `source /dev/stdin <<<`, and a pipe or here-document into an interpreter that is not a shell, such as `python3 -`, `perl` or `node`.

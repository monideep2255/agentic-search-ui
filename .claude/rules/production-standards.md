...
- Measure both before any answer-generation feature ships: cite-or-refuse and citation coverage are pass/fail acceptance criteria, measured against the Phase 4 golden dataset (50 queries). The instrument in use is the golden consistency run (`.claude/skills/bossman-mode/reference/Product_review.md`, Step 2), while the `eval-harness` grader is parked after the evaluation track closed on 2026-08-31: every golden question asked three times, each run classed as answered with citations or refused with none and its must-cite hits recorded, and any drop in the answered count below the floor blocks. The `eval-harness` skill, which grades both criteria with pass@k, stays available on demand.

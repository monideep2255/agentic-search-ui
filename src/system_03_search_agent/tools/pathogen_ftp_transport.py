...
async def stream_filtered_tsv_rows(
    ...
    *,
    ...
    """Stream a tab-separated file, yielding only rows whose `key_column` value is
    in `key_values`. Never buffers the full response body: reads line by line
    via a streaming GET, and stops at whichever comes first: every value in
    `key_values` has been seen at least once, `max_matches` rows have been
    collected, or `deadline` (an absolute `time.monotonic()` value) passes.
    ...
    async with client.stream("GET", url, timeout=remaining_s) as response:
        ...
        async for line in response.aiter_lines():
            ...
            if value not in remaining_keys:
                continue
            matched.append(dict(zip(header, fields, strict=False)))
            if not (max_matches and len(matched) >= max_matches):

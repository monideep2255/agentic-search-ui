...
def _build_system_message(schema_slice: str) -> dict[str, str]:
    content = (
        ...
        "6. When returning a PROPERTY of a node, return the node itself, or its id, in the same query: write RETURN d, d.name or RETURN d.id, d.name, never RETURN d.name alone. A bare property cannot be traced back to the record it came from, so it cannot be cited, and an uncitable value is dropped rather than shown. Returning the node alongside it is what lets the answer carry a real citation.\n"
        "7. The text between <question> and </question> is a user's "

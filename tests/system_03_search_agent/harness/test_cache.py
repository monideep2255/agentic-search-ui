...
def test_registered_tool_schemas_litvar2_lookup_input_schema_matches_the_model() -> None:
    """Same proof as the `ncbi_efetch` test above, for T-3.3-07's `litvar2_lookup` entry."""
    from system_03_search_agent.tools.litvar2_lookup_schemas import Litvar2LookupInput

    entry = next(s for s in REGISTERED_TOOL_SCHEMAS if s["name"] == "litvar2_lookup")
    assert entry["input_schema"] == Litvar2LookupInput.model_json_schema()


def test_registered_tool_schemas_pubtator_annotate_input_schema_matches_the_model() -> None:
    """Same proof as the `ncbi_efetch` test above, for T-3.3-07's `pubtator_annotate` entry."""
...
def test_registered_tools_sort_in_alphabetical_order() -> None:
    """Binding point from the ticket brief, extended by T-3.3-07:
    cypher_query < litvar2_lookup < ncbi_dbsnp < ncbi_efetch < pubtator_annotate.
    """
    prefix = build_stable_prefix(list(REGISTERED_TOOL_SCHEMAS))

    idx_cypher_query = prefix.index('"cypher_query"')
    idx_litvar2_lookup = prefix.index('"litvar2_lookup"')
    idx_ncbi_dbsnp = prefix.index('"ncbi_dbsnp"')
    idx_ncbi_efetch = prefix.index('"ncbi_efetch"')
    idx_pubtator_annotate = prefix.index('"pubtator_annotate"')
    assert (
        idx_cypher_query
        < idx_litvar2_lookup
        < idx_ncbi_dbsnp
        < idx_ncbi_efetch
        < idx_pubtator_annotate
    )

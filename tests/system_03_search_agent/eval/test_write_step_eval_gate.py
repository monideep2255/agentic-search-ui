...
# core/graph.py emits ONE sentence beginning "Note:", for example:
#
#   Note: this answer was truncated to 10 of the 42 records that matched
#   this question in the graph
#
# Reworded 2026-09-20 (D-2/D-3, `testing/Developer/reports/2026-09-20_
# tp53_findings/findings.md`): the note used to read "Showing 10 of 42
# matching rows; the rest are not shown above", which a reader could read
# as the paginated table below hiding rows it was never given in the
# first place. The note now distinguishes "never retrieved" from
# "retrieved but not all included in this answer" and drops "shown
# above" entirely, so this grader's markers move with it.
#
# The first cut of this grader exempted the first sentence of the old
# two-sentence note and counted the second as an uncited factual claim,
# which dropped the two-hop case to 33% coverage and failed the whole
# gate on a run whose answer was correct and fully cited. That is a
# grader defect, not a system defect, and it is worth naming precisely:
# the note asserts something about the RESULT SET, not about biology. It
# is metadata, it is generated by code rather than by a model, and it
# carries no citation by design because there is nothing to cite.
# Demanding one would require the harness to cite itself.
#

...
| 2 | System 3: build (Phase 6) | IN PROGRESS. Step 6.1 (prototype) and Step 6.2 (the one reconciliation pause) are COMPLETE. Step 6.3 (v1) is underway. Merged: build phases 1.0 through 4.1, plus 4.8 and 4.9, the web UI's visual design and its fidelity pass. Merged most recently: build phase 4.10, the anonymous run path and the server-side guest allowance, pulled ahead of 4.2 through 4.7 by product-owner decision on 2026-08-14, because until it existed nobody could use the product without creating an account first. Merged 2026-08-16: build phase 4.2, the CLI adapter (`system3-cli`, command `s3`), as PR #47, which resumed Section 25's order. Merged 2026-08-17: build phase 4.3, the GraphQL surface via Strawberry, as PR #48. Its build is complete and its premise gate is green, and it took SIX independent review rounds, the first five of which returned FAIL and each of which found its worst defect inside the PREVIOUS round's fix. Two of those were criticals, and both had the same shape: the code decided whether an exception was safe to show a caller by using a PROXY for safety (first the package a class was declared in, then its class family, then the phase the error came from) instead of an explicit declaration. The lead introduced the second one while fixing a major, and had rated the first minor. That is the case for independent review stated as evidence rather than as principle, and it is why this phase is the reference example for it. Fourteen vacuous gate arms were found across the phase, several of them written by the lead. The sixth round returned zero blocking findings, the phase merged, and its ticket is closed. Reports, one per round: tracker/phase_4.3_judge_report.md, _adversary_report.md, _rereview_report.md, _rereview2_report.md, _review5_report.md. The same day, 2026-08-18, the harness itself changed rather than the product (PR #49, PR #50), prompted directly by that six-round experience: the review loop is now capped at two rounds with escalation to the product owner instead of a third, fixes touching one file go to a single serial agent rather than parallel ones, a finding located inside an earlier fix stops the phase mid-round, and `tracker/preflight.py` probes each transport before any expensive dispatch. NEXT: Section 25's order continues at build phase 4.4, KGX export, which has no in-repo dependency, can land at any time, and is the first phase to run under the capped review loop. Current counts, all computed and verified by `python tracker/check_doc_drift.py --check`: 3308 Python tests, 181 frontend tests, 27 Playwright end-to-end declarations expanding to 30 executed cases, seven live tool premise gates, 344 decisions (DECISIONS.md), 91 learnings plus a retrospective (LEARNINGS.md). WHERE TO LOOK, since this cell deliberately no longer carries the per-phase narrative it accumulated over twenty phases: current phase status, evidence and open flags are `tracker/BOARD.md`; one file per phase with its tickets, findings and history is `tracker/phase_N.M.md`; the full dated narrative of every phase, what shipped, what review found and what it cost, is `requirements/Plan.md`'s Revision history; what to read and do before opening the next phase is `requirements/phase_6/Continuation_prompt.md`. |

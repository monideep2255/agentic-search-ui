...
|----------|--------|--------|
| 1 | System 1: data pipelines | Phase 1 and Gate 1 complete (67.5M Gene, 4.4M ClinVar, 198K MedGen nodes produced on real data). Phase 2 complete: Gate 2 done (2026-04-17), 5-database merge validated on real data. Phase 3 complete: AGE loader done (2026-04-19), 5-node + 3-edge round-trip smoke test passed via Docker Desktop. Phase 4.0 in progress (2026-04-20): Hetzner VPS provisioned, PostgreSQL 15.17 + AGE 1.5.0 installed, 144 GB KGX rsynced to VPS (complete 17:38). On-VPS kgx validate crashed at 1h48m (Python TypeError in tool, not a data issue). On-VPS awk verified edges.tsv perfect (693,295,991 rows, all NF=8) but flagged 64,882 mismatched rows on nodes.tsv (~0.056%); investigation required before age-load. See NEXT_STEPS.md at repo root. AGE load and Gate 3 pending. |
...
Phase 4 (week 7):     Cloud deploy: provision Hetzner VPS -> rsync KGX from laptop -> load into AGE on cloud -> Gate 3 = V1 complete  [IN PROGRESS 2026-04-20: rsync done, kgx validate crashed (tool bug), awk found 64K node row mismatches needing investigation, age-load not yet started]
```

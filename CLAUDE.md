...
| 4 | System 3: eval and tracing | IN PROGRESS. Build phase 5.0, observability, MERGED as PR #83 on 2026-08-30 with all four CI gates green: tech spec Section 20 in full, as three records with one job each, LangSmith per-run tracing joined on `trace_id`, PostHog behavioural analytics as aggregates only, and the append-only JSONL tool-call audit log. Build phase 5.1, the 50-query golden dataset, MERGED as PR #85 on 2026-08-30: 50 questions whose constraints were established from live NCBI lookups on a path that touches none of the agent's own machinery, in three product-owner-named search categories (KISS, KISSES, discovery). Build phase 5.2, the grading harness, MERGED as PR #86 the same day and is PARKED: it does NOT work, four review rounds returned FAIL, and its own suite is green with every defect live, so `replay()` refuses to run without `acknowledge_parked=True`. Parked by product-owner decision because the 50 questions are a first attempt rather than a settled target, so grader precision is aimed at a moving specification. Build phase 5.3, golden dataset schema v2, MERGED as PR #89 on 2026-08-31. THE EVALUATION TRACK IS NOW CLOSED by product-owner decision on 2026-08-31, with three things left genuinely unfinished and recorded in `requirements/Plan.md` under Phase 7 rather than on the board: the grader does not work, no golden row uses `must_reach` or `live_only`, and two tools are required by zero rows. Model-bench and A/B routing moved to Phase 7 the same day. Build phase 6.0, rate limiting and concurrency, MERGED as PR #91 on 2026-08-31 (full account in [Build phase history](#build-phase-history) below), and THE ORDERING REVERSED THE SAME DAY: build phases 6.0 and 6.1 moved BEHIND the prototype once a live run showed answers reading `MedGen:C0346153` where a disease name should be, so 6.1's breakdown and 6.0's residue moved to Phase 7 in `requirements/Plan.md`. BUILD PHASE 6.2, answer readability and the single UI pass, MERGED as PR #92 on 2026-09-01 (full account in [Build phase history](#build-phase-history) below), and it closed the disease-name defect that reversed the ordering. Their verdict on `docs/build/UI_feedback.md` produced PR #93, MERGED on 2026-09-05, NOT a numbered build phase but work picked from a defect the product owner hit plus everything deriving a specification then turned up: a designed sign-in screen, the permanent disclaimer band removed, a nav overflow menu below 720px, a usable integrations page, one shape for every refusal, a session-state privacy leak closed where `thread` survived sign-out, and `testing/Developer/Developer_workflows.md`, 50 workflows in three tiers. Their testing on 2026-09-12 opened THE UI FIX LOOP, a product-owner decision that replaces the build-phase cadence for UI fixes only: the product owner tests on develop, drops feedback, fixes land immediately with no branch and no judge or adversary round, then the product owner retests. Fix sets are tracked item by item in `testing/UI_fix_plan.md`, and the decisions behind them are dated 2026-09-05 and 2026-09-12 to 2026-09-13 in DECISIONS.md. What to read and do next lives in `requirements/phase_6/Continuation_prompt.md`'s Step 2. |
...
| 6.0 | Build phase 6.0, rate limiting and concurrency, MERGED as PR #91 on 2026-08-31, delivering technical specification Section 21.3 (the at-most-20 Layer 2 and Layer 3 calls per query) and Section 21.4's previously unwired half (the queue wait ceiling read from the calling query's own latency budget), in a new module, `src/system_03_search_agent/harness/call_budget.py`. Measuring Section 21 before writing a ticket found FIVE of its eight requirements already built by the tool phases 3.1 to 3.5. The ceiling counts at the two Layer 2/3 TRANSPORT chokepoints and never at `act_node`, the same argument build phase 5.0 proved by execution for the audit hook. Twelve findings, four fixed; the judge round filed nine, EIGHT MERGE OPEN, three of them critical, all saying the gate does not pin the production wiring rather than that either feature is broken, since both are verified working by execution. NO ADVERSARY ROUND WAS RUN, stopped by product-owner decision once it was clear the phase was not the constraint. THE ORDERING REVERSED THE SAME DAY: build phases 6.0 and 6.1 moved BEHIND the prototype, because a live run showed answers reading `MedGen:C0346153` where a disease name should be. THE NEXT ACTION is the disease-name fix recorded in `docs/build/UI_feedback.md`, not a build phase; 6.0's residue and 6.1's breakdown are recorded in `requirements/Plan.md` Phase 7. |
| 6.2 | Build phase 6.2, answer readability and the single UI pass, MERGED as PR #92 on 2026-09-01, all four CI gates green. It delivers the complete contents of `docs/build/UI_feedback.md` in ONE pass rather than in patches, which is what the product owner asked for, and it is the SIXTH stated exception on the board since Section 25 has no row for defects a live user hits. WHAT A PERSON NOTICES: an answer names diseases in words rather than as `MedGen:C0346153`, each cited to the MedGen record the name came from; the wait shows continuous motion; a follow-up carries the whole thread forward, bounded, as retrieval guidance only; an answer may offer one honest next step or stay quiet; a sentence that loses a clause from its middle is dropped whole; and the incompleteness note speaks to the reader instead of reporting internal bookkeeping. MEASURING BEFORE BUILDING CHANGED THE WORK TWICE. The brief said `ncbi_efetch` already resolves these CURIEs and told the reader to verify before promising it: verified, and half wrong, since NCBI rejects an ESummary keyed on a concept id, so resolution takes ESearch on `[ConceptId]` then ESummary on the returned UID, TWO CALLS TOTAL for any number of diseases and mapped back by MedGen's own `conceptid` rather than result ordering. And "the whole thread" was mostly built already: `open_threads` had sat on the contract with NO PRODUCER since build phase 4.5, which had written down that whoever added one must replace its docstring paragraph in the same change, and both obligations were honoured with the pinning test INVERTED rather than deleted. THE ROOT CAUSE WAS ALREADY RECORDED HERE as F-2.1-B07 and the CURIE fallback is DELIBERATE, so the phase added a Layer 2 path and did not weaken `_is_vocabulary_token_artifact`. WHAT IT COST: NINE findings, FOUR of them defects in the lead's own INSTRUMENTS rather than the product, three of which reported a plausible value instead of erroring and one of which nearly became a confident wrong conclusion about the agent. Recorded as ONE `LEARNINGS.md` entry because the pattern is the point: each NAMED SOMETHING THAT DID NOT EXIST and the harness reported absence as an ordinary value. NO JUDGE ROUND WAS RUN, a harness change rather than a skipped step: the product owner testing on develop is now the verification step, so tickets merge at `in-review` and reach `done` on their verdict. SIX TICKETS MERGE OPEN, the largest being that an answer can exceed 25 SECONDS with NO TICKET OWNING IT, filmed at one frame per second with 15 consecutive seconds of no visual change. |

...
| 2 | System 3: build (Phase 6) | IN PROGRESS. Step 6.1 (prototype) and Step 6.2 (the one reconciliation pause) are COMPLETE. Step 6.3 (v1) is underway. Counts are not stated here: `python3 tracker/check_doc_drift.py --counts` computes the test, decision and learning counts on demand. WHERE TO LOOK: `HANDOFF.md` for what is live, what awaits the product owner and the one next action; `testing/UI_fix_plan.md`, the board, for every card; the ledger `tracker/phase_N.M.md` for a numbered phase's tickets and findings, while `tracker/BOARD.md` is frozen as the record of build phases 1.0 through 6.2; `requirements/Plan.md` for the full dated narrative, including the build phase history that used to sit in this file. |
...
| `build/Build_workflow_cadence.md` | A pointer since 2026-09-25. It keeps the provider mapping table, the one place the build loop names a model provider, and says where each part of the loop now lives. The eleven stages, who acts at each, the model and effort per stage, where every file gets written, and the 8-hour and 8-dispatch budget are in `.claude/skills/bossman-mode/SKILL.md`. The golden consistency run blocks any answer-path change (that skill's `reference/Product_review.md`); premise gates for everything else were retired on 2026-09-24 | Any time the model tiering is in question. Before opening any build phase, read the skill it points to |
| `build/Phase_6_execution_flow.html` | The build loop as it stood on 2026-09-24, before the modes merged into one cadence, as a visual page openable in a browser. Also published as a Claude artifact. A note at its top says it is a record, not the loop that runs today | When explaining how the loop worked before 2026-09-25. For the loop that runs today, read `.claude/skills/bossman-mode/SKILL.md` |
...
| `../visualizations/System_3_deep_dive.md` (repository root, not `docs/`) | System 3 as it runs on develop, dated to a develop commit, in Mermaid diagrams with short explanations and a file pointer for every claim: the five-step loop, the three data layers and where an answer's trust comes from; every model call site with its tier, why, what checks its output and what happens when it fails, including Jev's four decisions and its fallback to the guard tier; the product harness (cost caps and the call ceiling, step budgets, retries, the prompt-cache prefix and the event stream) with one real golden question traced from submit to done; the build harness (the dial, the team, ledgers, rules, skills, hooks, the golden run and the CI gates); and two sections marked in progress, phase 8.6's re-land and card 42's verify loop | When showing someone how the system, its models and its two harnesses fit together, and before changing a model, a decision point, a budget or a stage of the build loop |
...
|-------|---------|-----------|
| bossman-mode | Autonomous execution in one cadence with a risk dial, since 2026-09-25: a copy or layout fix gets builder, clerk and product review; runnable behaviour adds the judge and the adversary; auth, the graph credential, the event schema or `.claude/` adds a branch and a pull request. A numbered phase keeps its branch and pull request at every position. A file-fenced team works through the eleven stages inside 8 hours and 8 dispatches, and every change ends on a product review of develop | `/bossman-mode` |
| task-tracker | The in-repo build record: the phase ledgers under `tracker/` (tickets, acceptance criteria, status, evidence, append-only history) beside the board of cards, `testing/UI_fix_plan.md`. `tracker/BOARD.md` is frozen at build phase 6.2 | `/task-tracker` |
...
| eval-harness | The offline evaluation gate, operationalizing the evaluation playbook: 8-point rubric, hard-fails, coverage metric, must-pass set. Invoked on demand since 2026-09-25, when it left the phase-end chain: its grader is parked, and the golden consistency run is the instrument in use | `/eval-harness` |
| precommit | Pre-commit checks on the code: Python compile, tests, lint, import order, git status, the frontend suites and the doc structure check. It never runs the product. Named `verify` until 2026-09-26 | `/precommit` |
| verify | Run the product and prove a change the way a person sees it: the local stack before the merge or deployed develop after it, confirmed through `/health`; a script captures every changed screen at 1280 and 390 pixels (screenshot, horizontal overflow, console errors, an accessibility scan), and the model judges each screenshot beside the design prototype, one pass or fail line per check with its file. At most two fix rounds. A pass at both widths starts the seven-day close for a wording or layout card; an answer-path change also needs the golden consistency run | `/verify` |
...
- The file artifacts: the board `testing/UI_fix_plan.md`, the phase ledgers `tracker/phase_N.M.md`, `LEARNINGS.md`, `DECISIONS.md`, and `tracker/BOARD.md`, frozen on 2026-09-25 as the record of build phases through 6.2. Plain markdown any agent can read and write.
- `tracker/render_board.py`. Stdlib Python, no Claude Code dependency. Verified by running it outside a Claude Code context. It renders the frozen board, by hand only since 2026-09-26, and `tracker/check_doc_drift.py` reuses its parser.
- `tracker/check_doc_drift.py`. Stdlib Python, no Claude Code dependency. `--check` fails on a structural defect in a tracked markdown file (a table of contents out of step with its headings, a broken append-only table, a wrong phase-to-pull-request reference) and compares no count; `--counts` computes the test, decision and learning counts on demand. `phase-checkpoint` and `precommit` both gate on `--check`, and the `learnings` skill runs it after every append.
- `tracker/preflight.py`. Stdlib Python (`http.client`, `socket`, `ssl`), no Claude Code dependency. Probes one endpoint per transport, the product's model provider, the harness's own provider, and the graph, before any expensive agent dispatch. Wired into the bossman-mode skill's stage 4 and Step 1 of its `reference/Phase_execution.md`, whose "Transport preflight" section says how to read a result.
...
2. Replace the `AGENTS.md` sync hook with an explicit step, or accept that `AGENTS.md` must be regenerated by hand at phase close.

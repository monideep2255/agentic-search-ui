...
NOT YET TRUE OF EITHER DEPLOYMENT, stated here rather than implied, after finding F-4.15-A-17. Build phase 4.15 ADDS the field, so a deployment can only report it once the code reaches it, and the two receive it at different times: the develop app when the phase merges to `develop`, the production app only when the first release is cut. Measured on 2026-08-27, both return `{"status":"ok"}` with no `app_env`. Until the first release, step 7 of the procedure below confirms the deployment by the response being healthy rather than by the field, and the premise gate's P5 arm is red for the same reason.

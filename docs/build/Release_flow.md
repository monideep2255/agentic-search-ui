...
`GET /health` is intended to return an `app_env` field, read from the `APP_ENV` environment variable at `src/system_03_search_agent/adapters/web_sse/app.py`. Once both deployments serve it, anyone holding a URL can call `/health` and read `app_env` to confirm which deployment answered, rather than guessing from the hostname.

NOT TRUE OF EITHER DEPLOYMENT until the first release, stated here rather than implied, after finding F-4.15-A-17. Build phase 4.15 ADDS the field, so a deployment can only report it once the code reaches it, and the two received it at different times: the develop app when the phase merged to `develop`, the production app only once a release was cut. Measured on 2026-08-27, both returned `{"status":"ok"}` with no `app_env`, so until then step 7 of the procedure below confirmed the deployment by the response being healthy rather than by the field, and the premise gate's P5 arm was red for the same reason. This closed with the v0.2.0 release on 2026-09-20: production `/health` now returns `app_env: production`.

...
| `src/system_03_search_agent/harness/decide.py` | Build phase 8.2's classifier seam: `decide()` asks Jev and the guard tier the same closed-option question concurrently, uses Jev's answer when `CLASSIFIER_PROVIDER=jev` and it answers in time, falls back to the guard tier otherwise, and returns a `DecisionRecord`. Each caller passes a fixed description of the decision (`instructions`, `criteria`) that both models receive. Wired in `core/graph.py`'s "classifier seam, wired" section, which lists every decision point. | A decision point's chosen option looks wrong (check that its description in `core/graph.py` says what is being decided), a fallback fired when it should not have, or the cost cap did not stop a decision call |
...
| `src/system_03_search_agent/synthesis/sentence_check.py` | 2026-09-23, items 12.9 and 12.10: the guard-tier model check on REWORDED answer sentences, the one bounded exception to deterministic acceptance, approved by the product owner. Builds the one check call and parses its reply strictly. Open it when a reworded sentence ships that says more than its quote (the prompt), or when prose that should survive is stripped (look for `sentence check approved nothing` in the logs: an unreadable reply, a failed call, the cost cap or too little budget all fail closed). The exact checks in front of it are `grounding.exact_synthesis_checks_pass`; the two-pass wiring is `core/graph.py`'s `_ground_with_sentence_check` |
...
| `check_doc_drift.py` | Scans every tracked markdown file for recurring structural defects (a table of contents out of step with its headings, duplicate phase headings, a broken append-only table) and for a phase-to-PR reference or a "next" phase that git or the frozen board contradicts, reading each phase status and PR number from source. Since 2026-09-25 it compares no count and no date; `--counts` computes the test, decision, learning, flag, premise gate and Playwright counts from source and prints them. A fact it could not compute is a failure line, never an ok. | `--check` (exit 0 on a clean run, exit 1 on any finding or any fact it could not compute), `--counts` (print only, exit 1 when a count could not be computed or pytest reported collection errors), `--verbose`, `--self-test` |

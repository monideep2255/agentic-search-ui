...
| `src/system_03_search_agent/harness/task_tiers.py` | The lookup table naming, for every model call site in the loop plus the five planned classifier decision points, which tier or provider serves it: `task_tier_for`. Pure data; nothing calls it yet. | Checking which tier or provider a call site is supposed to run on |

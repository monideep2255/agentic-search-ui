...
| `src/system_03_search_agent/core/clarify.py` | Fix-plan item 12.3's clarify-or-proceed decision, made by a model rather than a word list (REDESIGNED 2026-09-24, "Please do not hardcode!"): `build_clarify_messages` wraps a one-to-three-word opening question in an unguessable delimiter tag behind `CLARIFY_SYSTEM_INSTRUCTION`; `parse_clarify_reply` strictly validates the reply into `ClarifyDecision` (`ask_back`, a `question`, 2 to 4 subject-tailored `options`, each bounded and each ending in "?"), raising `ClarifyUnavailableError` on anything else. The 1-to-3-word trigger, the fail-open behaviour on any failure, and the one guard-tier call itself all live in `core/graph.py`'s `_clarify_or_proceed`, not here; this module owns the prompt and the strict parse only. The yes-or-no half is where item 11.38's Jev classifier slots in later; the option text stays with a text model regardless. | A short question is asked back when it should search or answered when it should ask, the offered options do not fit the subject or are not real questions, or a broken or slow classifier reply blocks a question instead of falling through to search |
...
| `src/system_03_search_agent/harness/decide.py` | Build phase 8.2's classifier seam: `decide()` asks Jev and the guard tier the same closed-option question concurrently, uses Jev's answer when `CLASSIFIER_PROVIDER=jev` and it answers in time, falls back to the guard tier otherwise, and returns a `DecisionRecord`. Not wired into the loop yet. | A decision point's chosen option looks wrong, a fallback fired when it should not have, or the cost cap did not stop a decision call |
...
| `src/system_03_search_agent/harness/jev_client.py` | The one HTTP call to OpenRouter's alpha decisions endpoint (Jev): request/response shape pinned live 2026-09-25, a 3-second timeout, no retries. | A Jev call raises, times out, or a response fails schema validation |

...
| `src/system_03_search_agent/core/breadth_plan.py` | The deterministic "search broad, cite exact" planning helpers (UI fix set 11): the fixed first-stage fan-out for a resolved gene or a disease title (PubMed, live ClinVar, OMIM), the follow-up calls on sorted and capped ids (abstract fetch and PubTator3 on the same PMIDs, ClinVar and OMIM summaries), and the OMIM title filter. Since build phase 8.2, `parse_publication_window` reads a range the question states ("from the last 5 years", "since 2022") and the PubMed term carries it as a publication-date limit. No network, no model. Abstract text does not become a finding through this module; that is held back until a new design exists. | The same question plans a different literature set on two runs, a PubMed or ClinVar term is rejected, an OMIM record for the wrong gene is cited, or a person who picked a range still gets papers from outside it |
| `src/system_03_search_agent/core/coordinate_window.py` | Deterministic parsing and call planning for a genomic coordinate window (fix-plan item 1, the coordinate range): `parse_coordinate_window` finds the first `chrN:start-end` window and its assembly in a question's text with a fixed rule, never the model; `gene_search_term` builds the GRCh38-only Gene ESearch term for the genes at a position; `genes_in_window` filters Gene ESummary records down to the ones whose own placement overlaps the window; `plan_overlap_calls` plans the ClinVar and dbVar `coordinate_overlap` calls. No network, no model. Not yet wired into `think_node` or `plan_node`. | A window with commas, an en dash, or the word "to" fails to parse, the wrong genes are named under a window, an inverted or oversized window is not rejected, or a window with no assembly is answered instead of asked about |
| `src/system_03_search_agent/core/accession.py` | Deterministic parsing and call planning for an NCBI accession (golden question G-007): `parse_accession` finds the first BioProject, BioSample, SRA or assembly accession in a question's text with a fixed rule, never the model; `search_input` builds the plain-term ESearch that resolves it to a uid (never the `[ACCN]`-scoped term, which probed live as a zero-hit trap for a record that exists); `link_inputs` plans the ELink fan-out `LINK_TARGETS` names; `plan_summary_calls` plans the ESummary calls once linked ids are known, sorted and capped with `breadth_plan.select_ids`; `disclosure` renders the one-sentence count-first narrative. No network, no model. Not yet wired into `think_node` or `plan_node`. | An accession is not recognised or the wrong one is picked from text naming two, `search_input` sends a scoped term that silently finds nothing, `link_inputs` misses a target or sends the wrong `dbfrom`/`db`, a linked target's ids are not deduplicated or capped correctly, or the disclosure sentence mis-pluralises or misreports a count |
| `src/system_03_search_agent/core/isolate_search.py` | Deterministic recognition and call planning for a Pathogen Detection isolate question (golden question G-035): `parse_isolate_question` recognises an isolate word plus an organism from the live-verified `ORGANISMS` table (taxon folder and NCBI Taxonomy id) and a gene family from `GENE_FAMILIES` or an explicit gene token, with a fixed rule, never the model; `plan_calls` plans the `pathogen_detection` `isolate_search` call then the organism's Taxonomy summary, no graph call; `disclosure` names the prefixes searched and the ones deliberately left out (ESBL searches `blaCTX-M` only); `count_sentence` states the count and the cut, exact or "at least". No network, no model. Wired into `think_node`, `plan_node` and `write_node`. | An isolate question is asked for a gene symbol instead of searched, "ESBL" matches blaTEM-1 carriers, the wrong organism or taxon folder is picked from a species and genus both named, an organism or gene family a person names is not recognised, or the count under the answer is wrong or missing |
| `src/system_03_search_agent/core/clarify.py` | The words of a question asked back (fix-plan item 12.3; the decision split out on 2026-09-25, build phase 8.2): `build_clarify_messages` wraps a one-to-three-word opening question in an unguessable delimiter tag behind `CLARIFY_SYSTEM_INSTRUCTION`, and `parse_clarify_reply` strictly validates the guard tier's reply into `ClarifyChoices` (a `question` and 2 to 4 subject-tailored `options`, each bounded and each ending in "?"), raising `ClarifyUnavailableError` on anything else. `recent_window_choices` writes the three fixed windows (the last 12 months, 5 years, 10 years) in the person's own words. Whether to ask is NOT decided here: `core/graph.py`'s `think_node` asks `decide(point="think.ask_back")` and `decide(point="think.recent_years")` and shows these choices only on those picks; the 1-to-3-word trigger and the fail-open rule live there too. | A short question is asked back when it should search or answered when it should ask (read the `think.ask_back` or `think.recent_years` record on the `done` event first), the offered options do not fit the subject or are not real questions, or a broken or slow writer reply blocks a question instead of falling through to search |
...
| `src/system_03_search_agent/core/next_step.py` | The go-deeper follow-up: the query an accepted next-step offer sends (`build_next_step_query`, built in code from the omitted findings' record type and the turn's entity) and how the next turn recognises it (`is_go_deeper_query`), one template and one pattern side by side. | "Yes, go deeper" refuses, re-shows the same records, or sends the offer text as the question |
...
| `src/system_03_search_agent/guardrail/prefilter.py` | Section 10.2: the cheap non-LLM pre-filter. `screen` refuses on injection markers, then medical-advice requests, no model call. `clears_biomedical_allowlist` only ADMITS since 2026-09-25 (build phase 8.2): a hit skips the relevancy classifier, a miss sends the question to `decide(point="guardrail.relevancy")` in `core/graph.py`'s guardrail node, whose `off_topic` refuses with `OFF_TOPIC_REASON`. | A query is refused as injection or advice before the classifier ever runs, or a plainly biomedical question pays for a relevancy call the allowlist should have spared it |
...
| `src/system_03_search_agent/harness/call_budget.py` | Section 21.3's at-most-20 Layer 2/3 calls per query and Section 21.4's query-class wait ceiling: `charge_one_call`, `calls_made`, `wait_ceiling_s`, `query_budget_scope`. Counted at the two transport chokepoints (`ncbi_transport.execute_get`, `pathogen_ftp_transport`), never at `act_node`. | A query was refused with "reached its Layer 2/3 API call ceiling", a call fanned out further than 20, or a queued call failed fast sooner or later than its query class should allow |
| `src/system_03_search_agent/harness/coordinator_worker.py` | The coordinator-worker split (Section 3.4): an isolated guard-tier reader pass over tool output, bounded and capped before it reaches synthesis. Holds the single 2000-character string cap every structured tool result passes through, cut on a word boundary. | A tool result looks corrupted, oversized, or wrongly capped before Write |
| `src/system_03_search_agent/harness/cost_control.py` | The four Section 19.1 cost caps (per-query, per-user daily, system-wide daily, anon daily) plus the `cost` event: `check_per_query_cap`, `check_user_daily_query_cap`, `check_system_daily_cost_cap`. The operator-only `cost` event also carries `call_elapsed_s`, the time of the call it follows (build phase 8.6). | A cost cap did not fire, fired wrongly, or a `cost` event looks wrong |
...
| `src/system_03_search_agent/harness/harness.py` | `Harness.call_tier`: the LiteLLM/OpenRouter call wrapper with cost accounting. Since build phase 8.6 it prices the model before sending anything, retries once without the `reasoning` block only when the provider's reply carries its own refusal phrase, "Reasoning is mandatory for this endpoint and cannot be disabled" (logged as "refused the reasoning block"; a 400 that merely mentions reasoning is not retried), and records each call's `elapsed_s` beside its cost. Per-step timeouts live here: `budget_for_step`, `budget_for_query_class`. | A model call times out, mis-prices, a model with no known price or one that cannot turn reasoning off is deployed, or `call_tier` raises unexpectedly |
| `src/system_03_search_agent/harness/jev_client.py` | The HTTP calls to OpenRouter's alpha decisions endpoint (Jev): `call_jev` asks one question, `call_jev_batch` asks up to 30 over one shared state in one call (build phase 8.6). Request and response shapes pinned live on 2026-09-25 and 2026-09-26, a 3-second total timeout a caller may shorten but never lengthen, no retries, strict validation of every answer. The caller's description of a decision rides in the endpoint's own `instructions` and `criteria` fields, never in `state`. A reply that came back but cannot be used (malformed, an option outside the set, a cost above `MAX_JEV_COST_USD`) still carries its reported cost on `JevCallError.billed_cost_usd`, which the caller charges to the question; only a figure that is not an amount of money (infinite, not a number, negative) charges nothing, with a warning (fix round, F-8.6-J10). | A Jev call raises, times out, or a response fails schema validation, or a question's cost leaves out a Jev call whose reply was unusable |
| `src/system_03_search_agent/harness/task_tiers.py` | The lookup table naming, for every model call site in the loop and every classifier decision (the sentence check and the decision points asked through `decide`), which tier or provider serves it: `task_tier_for`. A "classifier" row means Jev alone with `CLASSIFIER_PROVIDER=jev` and the guard tier otherwise. Pure data; nothing calls it, so no behaviour follows from it, and it can drift from `core/graph.py` unnoticed. | Checking which tier or provider a call site is supposed to run on |
| `src/system_03_search_agent/harness/tiers.py` | Tier resolution for guard, plan, and synth: `resolve_model` and `TierContext` hold a tier's resolved model id, never hardcoded. Also resolves Jev's own model id via `resolve_jev_model`. | The wrong model answered a tier, or a tier switched mid-query |
...
| File | What it does | Open it when |
|---|---|---|
| `src/system_03_search_agent/tools/cypher_query.py` | The only path to Layer 1. Runs the three step pipeline, slice, generate, validate and execute, with exactly one repair retry; for a known question shape the generate step is replaced by a code-chosen template from `cypher_templates.py`. | A graph answer comes back empty, uncited, or with a wrong row count |
| `src/system_03_search_agent/tools/cypher_templates.py` | Deterministic Cypher templates for the question shapes the graph answers every day, chosen in code from the bound CURIEs, their count and a keyword test, each with an ORDER BY on the record id so the same question returns the same sources every run. Falls back to the model path when no shape matches or two match. | The same question returns different sources on different runs, a template is rejected by the validator, or a question that should take a template is going to the model |
...
| `src/system_03_search_agent/tools/catalogue.py` | Build phase 8.2's in-process function catalogue: every existing action of all seven tools (spanning all three layers), each with a name, description, JSON schema generated from that tool's own locked input model, and its timeout and rate-limit pool. Changes no tool. Also owns `resource_options()`, the closed option list for the `"plan.resource"` decision point. | An action is missing from the catalogue, its schema does not match the tool's own input model, or `resource_options()` looks wrong |
...
| `src/system_03_search_agent/tools/pathogen_detection.py` | Bulk isolate, cluster and AMR access over the NCBI Pathogen Detection PDG snapshot tree, three modes: one isolate by BioSample accession, a cluster's SNP neighbours, and since 2026-09-22 `isolate_search`, a full scan of a taxon's Metadata TSV for isolates whose AMR genotype list carries a gene prefix (boundary-aware, so `blaOXA-48` never matches `blaOXA-484`), counting every match and keeping the first `max_isolates`. | An isolate lookup or a cluster's SNP neighbors come back empty or wrong, an isolate search reports a count that is not exact when `scan_complete` is true, or a prefix matches a longer gene name |
...
| `src/system_03_search_agent/adapters/web_sse/proxy_scheme.py` | Pure ASGI middleware, `ProxySchemeMiddleware`, that sets `scope["scheme"]` from `x-forwarded-proto` and touches nothing else, deliberately never `scope["client"]`, so the redirect Starlette's `/mcp` mount builds behind Railway's edge proxy is `https`, not a plaintext downgrade. | A redirect's `Location` header names `http://` for a request that arrived over `https`, or a rate limit keyed on client address seems to have moved |
...
| `src/system_03_search_agent/synthesis/answer_layout.py` | UI fix set 9: the structure of a written answer. Reads the model's reply into paragraphs and `##` headings before grounding, maps each surviving sentence back to its paragraph, decides whether a model heading may be shown (`heading_is_supported`: topic words or words the findings carry), picks the bold terms from the run's own entity and record names, and the one two-column table mapping the graph supports (variant, clinical significance). | A Researcher answer has no headings, shows a heading that asserts something, bolds the wrong words, or lists records where a table was expected |
...
| `src/system_03_search_agent/synthesis/disease_names.py` | Build phase 6.2: resolves a MedGen concept id to a disease name over Layer 2, because the graph's Disease `name` holds the source vocabulary rather than the name. Two E-utilities calls total for any number of ids, ESearch on `[ConceptId]` then one batched ESummary, mapped back by the payload's own `conceptid`. Never invents a name and never raises into the loop. | An answer names a disease the record does not, or falls back to `MedGen:C0346153` where a name should be |
| `src/system_03_search_agent/synthesis/mesh_terms.py` | 2026-09-23: resolves a MeSH descriptor id to its preferred heading over Layer 2, because every one of the graph's 30,790 `OntologyClass` vertices carries the identifier (`[MeSH] D000818`) in its `name`. Two E-utilities calls total for any number of ids, ESearch on `[MHUI]` then one batched ESummary, mapped back by the payload's own `ds_meshui`. Never invents a heading and never raises into the loop. | An answer to "what MeSH terms are assigned to this article" shows `MeSH:D000818` where a term should be, or shows a term that belongs to a different id |
| `src/system_03_search_agent/synthesis/findings.py` | Section 8.1: the code-built findings list Synth is given, compressed from tool results, with no raw record reachable outside it. Also `apply_resolved_disease_names` and the `name_resolved` render branch, which decide whether the model is handed an identifier or a name. | A finding cites the wrong value, Synth seems to reach outside its list, or an identifier appears in prose beside a resolved name |
...
| `src/system_03_search_agent/synthesis/grounding.py` | Section 8.2: deterministic cite-or-refuse. Exact or substring match after normalization, with no similarity score anywhere. Also drops a whole sentence that is a fragment (unbalanced parentheses, or a bare-pronoun opener whose antecedent was dropped, UI fix set 9 item 9.7), and reports which input sentence each survivor came from (`sentence_origins`). | An answer ships uncited, refuses when it should not, or opens on a garbled sentence |
| `src/system_03_search_agent/synthesis/sentence_check.py` | 2026-09-23, items 12.9 and 12.10: the model check on REWORDED answer sentences, the one bounded exception to deterministic acceptance, approved by the product owner. `check_reworded_sentences` is the entry point, and exactly one model judges: the guard tier by default, and with `CLASSIFIER_PROVIDER=jev` Jev alone, one call per answer with a yes-or-no question per sentence (build phase 8.6). A sentence is approved only when Jev picks "no" with a strictly higher probability for "no" than for "yes", so even odds approve nothing, and a failed or late Jev call approves nothing without asking the guard tier (its fix round, F-8.6-A01 and J14). Open it when a reworded sentence ships that says more than its quote (the prompt, or Jev's question text), or when prose that should survive is stripped (look for `sentence check approved nothing` and `Jev made no verdict` in the logs: an unreadable reply, a failed or late call, the cost cap or too little budget all fail closed). The exact checks in front of it are `grounding.exact_synthesis_checks_pass`; the two-pass wiring is `core/graph.py`'s `_ground_with_sentence_check` |
...
| `src/system_03_search_agent/synthesis/trust.py` | Section 8.3: the deterministic trust decision table over risk tier, grounded and triangulated, yielding answer, flag, ask, or refuse. Also `answer_trust_line`, the one plain line on an answer ("Based on 1 source, not yet confirmed"), counting independent origin databases. | A high-risk claim comes back `answer` instead of `ask`, triangulation looks wrong, or the trust line says "confirmed" without concordance |
...
| `frontend/src/components/screens/RunProgress.tsx` | The progress block a running search shows: the elapsed counter and Stop, the five-step stepper with its hidden live region, the Search stopped block, refusal and failure notices, persona caption, tool chips and the reasoning log, with no card of its own. `RunScreen.tsx` wraps it as the full page for a first question; `AnswerScreen.tsx` renders it inline below the thread for a follow-up (UI fix set 7, R22). A follow-up that still swaps the whole screen, or a Stop that behaves differently inline, starts here. |

...
| `src/system_03_search_agent/harness/harness.py` | `Harness.call_tier`: the LiteLLM/OpenRouter call wrapper with cost accounting. Since build phase 8.6 it prices the model before sending anything, retries once without the `reasoning` block when a provider refuses it (logged as "refused the reasoning block"), and records each call's `elapsed_s` beside its cost. Per-step timeouts live here: `budget_for_step`, `budget_for_query_class`. | A model call times out, mis-prices, a model with no known price or one that cannot turn reasoning off is deployed, or `call_tier` raises unexpectedly |

...
| Priority | Action | Why |
|----------|--------|-----|
| 1 | Add a pre-flight network and graph-tunnel health check before dispatching any premise-gate run or review agent, using the exact commands already written in `tracker/phase_2.2.md`'s "Resume here" section | Recovers the single largest measured waste on 2026-08-03 (1 to 1.5 hours), and prevents the same diagnostic confusion (outage read as defect) from costing time a third time |

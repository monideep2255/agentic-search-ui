...
|------|-----------------------------------|------|--------------------------|----------------------------|-----------|
| Guardrail | Whether the question looks like a prompt-injection attempt against the system, as a yes/no verdict with a reason, and whether it is off topic | Guard. With `CLASSIFIER_PROVIDER=jev` the injection half is Jev's `guardrail.injection` decision instead, asked beside this call; this call then judges only whether the question is off topic (build phase 8.6) | A Pydantic schema (`InjectionClassification`, `extra="forbid"`) must parse the reply; an unparseable reply is retried once, then fails the whole question rather than being treated as admitted | The question is not admitted. A parse failure or timeout raises and the run ends with a generic "a step hit a temporary error" message, never a silent pass-through. With Jev deciding, a `guardrail.injection` decision with no usable pick from either model ends the run the same way | `src/system_03_search_agent/core/graph.py`'s `_guardrail_after_prefilter`, classifier and the injection decision's description in `src/system_03_search_agent/guardrail/classifier.py` |
...
| Think, classification | What kind of question this is (a lookup, a comparison, a multi-hop question, and so on) and which entities it names | Plan. Since build phase 8.6 the class the run uses is the `think.query_class` decision's pick, asked beside this call; this call's own class is used only when that decision has no usable pick | A Pydantic schema (`_ThinkClassification`) must parse; one retry on an unusable reply, with a bounded excerpt of the bad reply logged so the cause is visible | Never fabricates or defaults a classification. Two unusable replies in a row fail the step and end the question with a generic error | `src/system_03_search_agent/core/graph.py`'s `_run_think_classification` and `_decided_query_class` |
...
| Write, repair pass | A second attempt at the same answer, only when the first pass left out a fact it was shown and the code-built listing will not show it either. Since build phase 8.6 (T-8.6-07) a paper's abstract or PMID counts as shown once the listing's row for that paper is cited, the same rule the answer's own omission count applies; before, those views kept the repair firing on nearly every question | Synth | The same grounding gate re-runs against the repaired text | Runs only within whatever time remains of the Write step's one shared budget; if that budget or the cost cap is spent, the first pass's already-grounded answer stands | `src/system_03_search_agent/core/graph.py`'s `_code_built_lines_will_cite` decides; the call is in `_write_answer` |
...
- Whether each reworded answer sentence says anything its quoted record words do not (the Write step's sentence check, build phase 8.6). Built in `synthesis/sentence_check.py`'s `check_reworded_sentences`: one call per answer carrying one yes-or-no question per sentence, which the endpoint accepts (thirty questions measured at 343 to 611 ms for $0.00033), with the guard tier only when Jev fails. `core/graph.py`'s `_ground_with_sentence_check` calls it since builder K's patch was applied.
- Whether the question is a prompt-injection attempt (`guardrail.injection`, build phase 8.6), asked beside the guardrail's classifier call and only when `CLASSIFIER_PROVIDER=jev`; with the code default the classifier's own verdict stands, byte for byte as before. This is the one decision that fails closed: no usable pick ends the run in the classifier's step error, never an admission.
- The question's shape (`think.query_class`, build phase 8.6), one of the five classes Think has always used, started with Think's other decisions and read when Think's own call returns, waiting at most one second; with no usable pick the plan tier's class stands. The plan tier still extracts the entities.
- Whether the question asks about a condition's features, signs, symptoms or phenotype (`think.asks_features`, build phase 8.6), read by Write. Only on "asks_features" does an answer say "MedGen lists no clinical features for ..." or keep ten of the model's prompt slots for the disease's features; any other outcome speaks of neither.

...
Current state, 2026-08-01: build phase 2.1 is closed and merged as PR #15, 31 commits, branch deleted. It was merged first as PR #13, before the rework had any independent review, then superseded by PR #15 once the fifth judge and fifth adversary passes actually reviewed it. The fifth judge pass returned PREMISE: PASS, the first of five review rounds to return one, and did not accept the round-four root-cause claim on trust. It tested the claim with a controlled A/B: one constant changed, same model, same question. At the old value the schema slice carried no Disease label and the model returned 25 non-human orthologs, round four's worst case exactly. At the corrected value it returned the correct 4 diseases. The premise gate, `tests/system_03_search_agent/tools/test_cypher_query_premise.py`, went 9 of 9 on three consecutive runs. The fifth adversary pass followed the same day and filed 8 findings, all fixed or explicitly deferred with a reason. Full detail: the "Fifth judge pass" and "Fifth adversary pass" sections below.

Total LLM spend across the build phase 2.1 session, six probe calls plus three real tier calls, was roughly 0.003 US dollars. This covers that one session, not the phase total, which ran across four calendar days and five review rounds.

Cost control note, recorded during this phase: the per-query cost cap holds end to end, and both daily caps read zero since nothing writes `interactions` rows until build phase 4.6 (F-2.0-04). The OpenRouter account ceiling, set to 20 US dollars and verified live during this phase, is what actually bounded spend during build phase 2.1, not either daily cap. A reader of F-2.0-04 alone would conclude nothing bounds cost today; this is the fact that corrects that.

A ruff detail from a checkpoint during this phase, dated 2026-08-01 and superseded by the final gates recorded above: at that checkpoint, `ruff check` on `src/` was clean and `ruff check` on `tests/` reported 2 pre-existing errors, both predating this phase's work and verified against HEAD. Recorded here since a future reader hitting the same two errors on `tests/` would otherwise re-investigate whether build phase 2.1 introduced them; it did not.

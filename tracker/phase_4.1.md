...
Amended at judge round 3, 2026-08-11, after the adversary round and two fix rounds. The eight clauses above are unchanged and all still hold; what follows is added, not substituted. The premise as first written graded what this surface returns when the run underneath it succeeds, and said nothing about what it returns when that run goes wrong, which is where every critical and major adversary finding landed. A guarantee a premise does not state cannot be graded against it, so the three the phase actually built are now stated:

- A run that dies on a fatal error, or is cancelled, never presents its partial text as a complete answer. Whenever a fatal `ErrorPayload` reaches the fold loop, and regardless of whether any answer text was already accumulated, the top-level `trust_signal` is floored (`outcome` to no better than `flag`, `grounded` to `False`, `risk_tier` to `high`) and a sanitized disclosure naming the failure class is appended to `trust_signal.message`. Floored, never raised: a run that already reported `refuse` or `ask` keeps that outcome. A cancellation reaches this path as the same shape (`error_class="cancelled"`), so `POST /v1/query/{run_id}/stop` and the abandonment timer are both covered.
- The top-level `trust_signal` never asserts a verdict the run's own events contradict, and never asserts one they never supported. When only claim-scoped signals arrived, the answer-scope signal is aggregated from them, most-restrictive-wins per Section 8.3.4, on every one of `outcome`, `risk_tier`, `grounded` and `triangulated`, rather than synthesized from a fixed cheerful default. When no `trust_signal` arrived at all, `grounded` reflects whether a citation actually exists, never the terminal `trust_outcome` alone, so a zero-citation answer can never report itself grounded.
- The tool call is bounded in wall-clock time by this surface itself, not only by the core it waits on. The fold loop carries its own 240-second budget; on expiry it raises a protocol-level timeout carrying an actionable next step, and releases its registry subscription so the entry can be reclaimed.
- Both defensive caps disclose rather than truncate silently: an over-length answer is cut on a word boundary and says so, and an over-cap citation list names how many were omitted.
...
## Learnings-coverage check

`python3 tracker/check_learnings_coverage.py 4.1` returns "nothing to cover," the same false negative already flagged on `tracker/BOARD.md` for build phase 3.3 (`check_learnings_coverage.py format blind spot`): the script's `_STATUS_LINE_RE` only recognizes a narrative `### F-x:` block, and this phase's findings live in table rows (`| F-4.1-A-01 | CRITICAL | closed (judge round 3) | ... |`), which it cannot see. Not a false pass in outcome: verified by hand that `LEARNINGS.md` carries four real, substantive 2026-08-11 entries for this phase (the three-part mount failure, the premise-gate singleton/exception-group fixes, the production-mount singleton-safety proof, and the recursive exception-unwrap fix), each with a "Full account below" detail section. The existing BOARD.md flag already covers this generically ("whenever the script is next touched, widen the regex"); no new row needed for this phase specifically.

## eval-harness gate disposition

Not run in full, determined at phase-end gates. `core/graph.py`'s answer-generation and cite-or-refuse logic is untouched by this phase (confirmed via `git diff develop..HEAD -- src/system_03_search_agent/core/`, empty), so the 8-point rubric and its hard-fail conditions, which grade whether the agent generates a correct grounded answer, have nothing new to grade here. This phase's actual risk was relay fidelity: whether an already-generated, already-graded answer reaches this one new adapter's caller without the fold loop itself corrupting the verdict, which is exactly what the two critical adversary findings (F-4.1-A-01, F-4.1-A-02) were. That risk was tested more rigorously than a golden-dataset run would: the premise gate adversarially constructs the exact worst-case event shapes (claim-scoped-only trust signals, zero-citation answers, a fatal error mid-stream) a live must-pass-question run might never happen to produce, verified across three independent judge rounds against direct code reads, not inferred pass rates. Running the v1 must-pass set now would also cross Step 6.2's own logged 2026-08-10 decision to keep the golden-dataset gate deferred to build phase 5.1 rather than run a smaller version early. Disposition: not applicable in the full sense, satisfied in the relay-fidelity sense by the premise gate and three judge rounds already run this phase.
...
F-4.1-J-04 (minor): added `mcp-types>=2.0` and `httpx2>=2.0` to both `requirements.txt` and `pyproject.toml`, matching the repo's existing floor-pin convention. Verified both PyPI distribution names against the installed wheel before adding, confirming the import name and the distribution name are identical for both packages so no name-mapping error was introduced. Ran `pip-audit -r requirements.txt` afterward: no known vulnerabilities.

Correction (F-4.1-J2-01, applied directly by the lead rather than a fourth judge round, since the underlying dependency declaration was never wrong, only this sentence): the paragraph above originally reported `pip show mcp-types` and `pip show httpx2` as both `2.0.0`, MIT. `mcp-types` is correct at `2.0.0`, MIT. `httpx2` is actually `2.10.0`, `BSD-3-Clause` (Pydantic Services Inc., `github.com/pydantic/httpx2`), already recorded accurately in `DECISIONS.md` by commit `311bace`. The declared floor `httpx2>=2.0` is satisfied either way, so nothing about the shipped dependency declaration changes; only this narrative sentence was wrong.
...
Round 3, 2026-08-11, against `6134af6` (the full branch, `develop..HEAD`, which is fix round 2's commit on top of the two passed judge rounds and the adversary round): PASS.

Fresh context. Did not run rounds 1 or 2, did not write either fix round, did not run the adversary round.

A note on how this round was produced, recorded because it affects how much weight the record below carries. This round ran in two passes. The first pass performed the finding-by-finding re-verification and wrote the disposition text now standing in every row of the Adversary findings table above, then its process died mid-task (the machine slept, an infrastructure failure, not a logic failure) before it could write this section. Rather than discard that work or rubber-stamp it, this second pass re-ran the highest-stakes half independently and states its own evidence below. What was NOT re-derived from scratch: the per-finding repro detail and code citations in the rows above, which are the first pass's and read as sound. What WAS re-derived, with fresh commands in this session: both criticals, the cancellation variant, the sanitizer claim behind the one new security-shaped finding, the full suite, ruff, and the two structural surface guarantees. Where a first-pass disposition label did not survive that check, it was corrected rather than left standing, and two are corrected below.

### The two criticals, re-derived independently

Not read off the fix round's self-report and not taken from the first pass. Probed directly against the current code.

F-4.1-A-01 and F-4.1-A-05, the fatal-error and cancellation shape. The mechanism is really in the code as written, confirmed by reading it rather than by trusting the prose: `_fold_run_to_response` (`server.py:629`) is a bare `if fatal_error_payload is not None:` sitting at the top level of the post-loop block, NOT nested inside the `if not answer_text:` branch at line 607 that the adversary's repro exploited. The two are now independent: line 607 still supplies fallback text for an empty answer, and line 629 fires regardless of whether any text exists. It calls `_floor_trust_signal_for_fatal_error` and appends a disclosure, in that order, and the disclosure is merged into `trust_signal.message` at line 633.

`_floor_trust_signal_for_fatal_error` (`server.py:439`) is a genuine floor, not a relabel. Driven directly this session:

```text
before floor: outcome='answer' risk_tier='low'  grounded=True  triangulated=True
after  floor: outcome='flag'   risk_tier='high' grounded=False triangulated=True
refuse input: outcome='refuse' risk_tier='high' grounded=False   (stays refuse, never raised to flag)
```

The second line is the one that matters and is easy to get wrong: the floor uses `aggregate([outcome, "flag"])`, so a run that already reported `refuse` keeps `refuse` rather than being "floored" upward into the milder `flag`. A naive `outcome = "flag"` assignment would have passed every test in the file and silently improved a refusal. It does not do that.

F-4.1-A-02, the claim-signal aggregation. `_aggregate_claim_trust_signals` (`server.py:391`) was read line by line and then driven against the adversary's own exact repro input, two claim-scoped signals both `flag`/`high`/`grounded=False`/`triangulated=False`, with `done.trust_outcome="answer"`:

```text
ADVERSARY OBSERVED (pre-fix): outcome='answer' risk_tier='low'  grounded=True  triangulated=None
CURRENT CODE                : outcome='flag'   risk_tier='high' grounded=False triangulated=False
```

All four inverted fields are corrected, and the correction is computed rather than defaulted: `outcome` via `synthesis.trust.aggregate` over the claim outcomes, `risk_tier` via `any(... == "high")`, `grounded` via `all(...)`, `triangulated` via a `min` over an explicit tri-state severity map. Two counterfactuals were run to prove it floors rather than clamps, which a fixed pessimistic default would also have passed:

```text
mixed answer+refuse claims: outcome='refuse' risk_tier='high' grounded=False triangulated=None
all-clean claims          : outcome='answer' risk_tier='low'  grounded=True  triangulated=True
```

The second line is the one that distinguishes a real aggregation from a cheerful-default-swapped-for-a-gloomy-one: a genuinely clean claim set still folds to a clean answer signal, so this fix did not buy its correctness by making the surface uniformly pessimistic.

### The premise gate, run in this session
...
Green on the actual current code, 48 of 48, both MCP test files in one session (no one-shot session-manager collision). Every test named in fix round 2's own list is present in the collected output and passing, including `TestFatalErrorDisclosure` (2), `TestClaimScopedTrustAggregation` (1), `TestUncitedAnswerGrounding` (1), `TestAnswerAndCitationTruncationDisclosure` (2) and `TestFoldLoopWallClockBound` (2).

### The two structural surface guarantees, re-dumped

The module roughly doubled in size during fix round 2, so round 2's verification of these two does not carry forward on its own. Re-derived this session:

```text
TOOL COUNT: 1  NAMES: ['ask_biomedical_question']
ALL RESOLVED KEYS (23): answer, assertion_confidence, citation_id, citations, claim_text,
  display_index, evidence_kind, fallback_link, field, grounded, layer, license, message,
  outcome, population_ancestry_context, risk_tier, run_id, scope, source, source_id,
  source_url, triangulated, trust_signal
COST/INTERNAL-SHAPED KEYS (cost|cap|usd|think|plan|token|tool|elapsed|spend|billed): NONE
required = ['answer', 'citations', 'trust_signal', 'run_id']
additionalProperties=False on AskBiomedicalQuestionOutput, CitationPayload, TrustSignalPayload
```

`additionalProperties=False` on all three models is the structural half of F-4.1-A-07's disposition, confirmed here directly rather than accepted from the fix round's claim: an undeclared field cannot be constructed, so the gate's new allowlist assertion is a second line of defense rather than the only one. `message` is the one key added to the tree since round 2's dump; it is `TrustSignalPayload`'s own pre-existing Section 8.4 field, now also the channel every new disclosure rides, and it carries no cost or internal-event content.

### No regression

| Check | Command | Result |
| --- | --- | --- |
| Full repo suite | `pytest -q` | `6 failed, 2445 passed, 113 skipped, 1 xfailed in 75.63s` |
| Regression identity | inspected all 6 failures | All 6 in `tests/system_03_search_agent/synthesis/test_citation_trust_full_premise.py`, every one a `LiveHttpCallInUnitSuiteError` against a live host (`clinicaltrials.gov`, and the graph-gated siblings). Develop's known live-network-opt-in baseline, unchanged. `2445 = 2397 (develop's passing baseline) + 48 (this phase's MCP tests)`; skip, xfail and fail counts identical to develop. Zero new failures |
| MCP suite | `pytest tests/system_03_search_agent/adapters/mcp/ -v` | `48 passed in 11.05s` |
| Ruff, files touched this phase | `ruff check` on `adapters/mcp/server.py`, `adapters/mcp/__init__.py`, `auth/dependencies.py`, `adapters/web_sse/app.py`, `tests/.../adapters/mcp/` | `All checks passed!`, exit 0 |
| Ruff, whole repo | `ruff check . --output-format=concise` | 17 errors, the identical count both prior judge rounds recorded, none in any file this phase touched. Zero introduced |

### New findings, round 3

The first pass surfaced three items and named them inline inside existing rows without defining them. Each is given a definition, its own evidence, and a disposition here, so none is left half-labeled.

| Finding | Severity | Status | Description |
| --- | --- | --- | --- |
| F-4.1-J3-01 | minor | carried open, owner named | Raw exception stringification into an externally-visible `ErrorPayload.message` is an established pattern at two `core/graph.py` call sites, and F-4.1-A-09's fix does not reach them. Both are real, confirmed by reading the file: `core/graph.py:734` and `:737` each do `return _decline_for_daily_cap(state, sink, source, str(exc))`, and `_decline_for_daily_cap` (`graph.py:874-881`) puts that string into `ErrorPayload(message=message[:256])`, which bounds length but not content. Two independent reasons this is NOT live-exploitable, both verified rather than assumed. First, it is structurally unreachable from the MCP surface: `_fatal_error_disclosure` (`server.py:173`) keys purely off the four-value `error_class` literal and never reads `.message` at all, and `grep` over `server.py` returns zero reads of `error_payload.message` anywhere in the module (the only `.message` reads are `trust_signal.message`, a different field). Driven directly: an `ErrorPayload` carrying `"psycopg2.OperationalError: could not connect to server at <server-ip>:5432, user=age_reader, db=knowledge_graph"` renders as `"This query could not complete as requested."`, with no host, user, database name or exception class surviving. Second, it is not exploitable on the REST/SSE surface either, though for a weaker reason: `harness/cost_control.py`'s `sanitize_event_for_end_user` drops `cost` events and redacts `total_cost_usd` from `done` and touches `ErrorPayload.message` not at all, so an `error` event does reach a REST client verbatim, but both raise sites feed it a curated purpose-built decline string (`user_daily_cap_decline_message(...)` at `cost_control.py:360` and the `SYSTEM_DAILY_CAP_DECLINE_MESSAGE` constant at `:425`), never arbitrary exception text. So the guardrail on the REST surface is source discipline, not a sanitizer, exactly the "coincidence rather than design" F-4.1-A-09 named. Corrects the first pass's label, which read as though F-4.1-A-09's fix introduced this: it did not, the pattern pre-dates this phase entirely, and the fix in fact removed the one surface that was consuming it. Not fixed here for two reasons: `core/graph.py` and `harness/cost_control.py` are outside this phase's file scope, and adding an `ErrorPayload.message` sanitizer to the REST surface is a behavior change to build phase 4.0's shipped adapter that belongs in a phase that owns it |
| F-4.1-J3-02 | minor | carried open, product decision | The residual of F-4.1-A-03, which fix round 2 closed by half. A-03 named TWO manufactured assertions in the fully-silent fallback branch, `grounded` and `risk_tier`, quoting both by line. The fix changed `grounded` to `bool(citations) and resolved_outcome == "answer"` and left `risk_tier="low"` hardcoded, still asserted from nothing: `server.py:594` is a literal `risk_tier="low"` inside the branch that fires when a run emitted no `trust_signal` event of any scope. Read from the live function source this session to confirm the line is really still there and really still unconditional. Severity is materially lower than A-03's was, for a reason worth stating so a future reader does not re-escalate it: `grounded=False` now carries the load-bearing signal for a zero-citation run, so the caller is no longer told the answer was verified, and `RiskTier` is a strict two-value `Literal` (`synthesis.trust.RiskTier`) with no "unknown" member, so the only alternative to "low" is asserting "high" for every silent run, which would be its own unearned assertion in the opposite direction and would fire on the ordinary guardrail-refusal shape. Closing this properly means widening the `RiskTier` domain or giving the adapter a real basis for the tier, both product decisions that reach past this adapter. Carried rather than fixed, and named rather than left inside A-03's row where it would be invisible to a grep |
| F-4.1-J3-03 | minor | closed (this round) | The two carried-open adversary findings, F-4.1-A-10 and F-4.1-A-15, were both described in fix round 2 as "tracked in `tracker/BOARD.md`'s Open flags table" and neither had a row there. Verified absent, then closed by adding them, per the same condition build phase 4.0's own judge round 3 imposed on itself: every carried finding gets a row with a named owner or trigger, so it cannot fall through the way F-2.0-15 fell through twelve phases with nothing naming it. Four rows added in total, since F-4.1-J3-01 and F-4.1-J3-02 above are carried on the same terms, and phase 4.1's Flags cell was updated in the same edit to keep `render_board.py`'s flag-count reconciliation (line 653) honest. Bookkeeping only, no code touched |

### Round 2's two minor findings are still open

Recorded rather than quietly dropped: fix round 2 addressed the adversary's findings and did not touch either of judge round 2's own. Both re-checked this session and both still stand.

- F-4.1-J2-01: the Fix round section's F-4.1-J-04 paragraph (line 184 of this file) still reads "`pip show mcp-types` and `pip show httpx2` both report version `2.0.0`, MIT license". Re-checked directly: `httpx2` is `2.10.0`, home page `github.com/pydantic/httpx2`, author-email `tom@tomchristie.com`; `mcp-types` is `2.0.0` from LF Projects and is the only one of the two the sentence describes correctly.
- F-4.1-J2-02: `.coverage` is still absent from `.gitignore` (and is untracked in the working tree right now, exactly as predicted), and `test_phase_4_1_production_mount.py`'s module docstring still omits `tests/system_03_search_agent/auth/test_router.py` from its list of files sharing the `app` singleton.

Neither is a behavior defect, neither blocks the PASS, and both are one-line edits any subsequent commit can carry.

### Every adversary finding, dispositioned

The detailed per-finding evidence is in the Adversary findings table above and is not duplicated here. This is the roll-up.

| Finding | Severity | Disposition |
| --- | --- | --- |
| F-4.1-A-01 | CRITICAL | closed. Unconditional fatal check at `server.py:629`, floor verified by direct drive, disclosure merged into `trust_signal.message` |
| F-4.1-A-02 | CRITICAL | closed. Aggregation verified against the adversary's exact repro plus two counterfactuals proving it floors rather than clamps |
| F-4.1-A-03 | major | closed on the `grounded` half; `risk_tier` residual carried as F-4.1-J3-02 |
| F-4.1-A-04 | major | closed. Word-boundary truncation plus a disclosure naming the character limit |
| F-4.1-A-05 | major | closed. Same mechanism as A-01; a cancellation reaches the fold loop as `error_class="cancelled"`, which the floor and `_MCP_FATAL_ERROR_DISCLOSURE` both cover |
| F-4.1-A-06 | major | closed. `asyncio.timeout(240.0)` wrapping the event loop, with an actionable `REQUEST_TIMEOUT` message and a test proving the subscriber is released so the registry can reclaim the entry |
| F-4.1-A-07 | moderate | closed. `additionalProperties=False` on all three models re-confirmed this session; the gate gained an allowlist assertion alongside, never replacing, its denylist |
| F-4.1-A-08 | moderate | closed. Independent `citation_events_seen` counter and an omitted-count disclosure |
| F-4.1-A-09 | moderate | closed. `_fatal_error_disclosure` keys on `error_class` only; zero reads of `error_payload.message` remain in the module. Underlying `core/graph.py` pattern carried as F-4.1-J3-01 |
| F-4.1-A-10 | moderate | carried open. Product decision (does this repo's own untrusted-content obligation run outward when this system becomes somebody else's tool). Board row added |
| F-4.1-A-11 | minor | closed. `headers.getlist` rejects duplicate `Authorization` headers outright, both orderings tested |
| F-4.1-A-12 | minor | closed, deliberately narrow. Case-insensitive scheme only; the two RFC 7230 OWS gaps are stated in a code comment and logged in DECISIONS.md |
| F-4.1-A-13 | minor | closed. `_reject_unknown_arguments` raises `INVALID_PARAMS` before `create_run` |
| F-4.1-A-14 | minor | closed. The no-database scheme check runs before `session_scope()` opens |
| F-4.1-A-15 | minor | carried open. Latent until build phase 4.5 or 4.6 wires a `session_id` consumer. Board row added |
| F-4.1-A-16 | minor | closed. `AfterValidator` rejects C0 and DEL, excluding tab, newline and carriage return |

Sixteen filed, twelve closed, two carried open with named owners, one closed with a carried residual, one closed with a carried underlying pattern. Zero adversary findings rejected across the whole round: every one was a real defect or a real, honestly-flagged uncertainty.

### Premise re-read

The Phase premise paragraph was amended this round rather than only re-checked. Round 2 verified eight clauses and all eight still hold, unchanged in substance. What changed is that the premise as originally written was silent on what this surface says when the run underneath it goes wrong, which is precisely where every critical and major adversary finding landed. A premise that does not state a guarantee cannot have that guarantee graded against it, so the three guarantees fix round 2 actually built (fatal and cancelled-run disclosure with a trust floor, claim-signal flooring, and a wall-clock bound) are now stated in the premise explicitly. This is an append to a living section, not an edit to a log: the original eight clauses are untouched, and the Findings, Decisions and per-round records above are append-only and were not rewritten.

| Clause | Round 3 |
| --- | --- |
| The original eight clauses | All hold, unchanged. Re-confirmed this round: `list_tools()` returns exactly one tool, the resolved output schema tree carries zero cost-shaped or internal-event-shaped keys with `required` exactly the four fields, and the premise gate's auth arms are green |
| Fatal or cancelled run discloses (new) | Holds. Verified by direct drive of `_floor_trust_signal_for_fatal_error` plus reading the unconditional call site at `server.py:629` |
| Claim-scoped signals floor rather than default (new) | Holds. Verified against the adversary's exact repro and two counterfactuals |
| Wall-clock bound (new) | Holds. `_FOLD_LOOP_TIMEOUT_S = 240.0` wraps the event loop; `TestFoldLoopWallClockBound`'s two arms are green, one of them asserting the subscriber is released on timeout |

### Ticket status

| Ticket | Status | Reason |
| --- | --- | --- |
| T-4.1-01 | done | The gate grew from 20 to 48 tests across fix round 2 and is green in this session. Its two known integrity defects are both closed and both were closed by demonstration rather than inspection: judge round 2 mutation-proved the vacuous-assertion repair, and fix round 2 proved the denylist-to-allowlist addition against the adversary's own renamed-key leak. The coverage-exclusion statement remains present and honest |
| T-4.1-02 | done | The fold loop roughly doubled in size this phase and every added branch is exercised. The two guarantees that carry the product claim, never-cost and single-tool, were re-derived from the live registry and the live schema tree in this session rather than carried forward from round 2's dump |
| T-4.1-03 | done | Auth gained three hardening fixes (duplicate-header rejection, case-insensitive scheme, no-database pre-check) and the property T-4.1-03 actually pins, no run and no budget before authentication, is unchanged and green |
| T-4.1-04 | done | Dependencies declared and verified by live import. F-4.1-J2-01's correction to the narrative sentence is still outstanding and still does not affect the declaration |

### Verdict

PASS. Both criticals are closed by mechanisms this round read in the code and drove directly, not by reports it accepted. The full suite matches develop's baseline exactly with zero new failures, ruff is clean on every file this phase touched, and the two structural surface guarantees were re-derived rather than carried forward. Four findings are carried open, each with a named owner and a board row. Two round-2 minor findings remain open and are one-line documentation edits.

The adversary's own ship judgment was that "the answer path is not trustworthy enough to ship as it stands", and it named four repairs: honor `fatal_error_payload` when partial text exists, stop synthesizing `risk_tier="low"` and `grounded=True` out of nothing, read the cancellation, and disclose both truncations. Three of the four are fully done and verified here. The fourth is done on the half that mattered (`grounded` no longer asserted from nothing) and carried on the half that needs a product call (`risk_tier`'s two-value domain), which is F-4.1-J3-02. That is a smaller and better-named gap than the one the adversary objected to, and it no longer produces the failure it objected to, since a zero-citation answer can no longer report itself grounded.

...
| 3.1 | `phase/3.1-ncbi-efetch` | ncbi_efetch over E-utilities and Datasets API v2 | 2.0, 3.0 | v1 | done | refined | | | 3.1 re-review outstanding |
...
|------|--------|----------------|
| 3.1 re-review outstanding | Build phase 3.1 merged as PR #22 on 2026-08-05 without an adversarial pass over its own fix round, which was the stated pre-merge condition. Twenty-six findings sit at `fix-landed` in `tracker/phase_3.1.md`: a commit claiming the fix has merged and no independent role has confirmed any of them. Six of those were filed critical, including F-3.1-13, where `summary` returned a schema-valid citation for a record that does not exist, which is a direct cite-or-refuse breach. The suite is green at 1571 passed and doc drift is clean, but this repo's own history is that a green suite is exactly the state build phase 2.1 was in through four consecutive failing reviews | The next primary-provider session, ahead of the F-2.1-C15 branch and ahead of opening 3.2 |
...
| T-3.0-08 needs its own slot | F-2.1-C15's generation half and F-2.2-01 are untouched. The builder dispatched to analyse it inverted its contract, implemented a validator rule without the required analysis, and left a rule that rejects `[:orthologous_to {weight: 2*3}]` as an unbounded traversal. Reverted; the attempt is preserved as a diff. RE-HOMED 2026-08-04: this previously read "build phase 3.1, whichever phase next touches generation", which would never have fired. 3.1 is `ncbi_efetch`, a Layer 2 tool, and neither it nor 3.2, 3.3 or 3.5 opens `tools/cypher_generation.py`. F-2.1-C15 is the finding where a generated query took the graph server down for every user, so an owner that never arrives is the wrong disposition for it | IMMEDIATELY AFTER BUILD PHASE 3.1 MERGES, on a dedicated `fix/c15-generation-bound` branch. Dated 2026-08-04 by the product owner rather than left as an open slot, because "a dedicated branch" names an owner and not a schedule, and this is the finding where a generated query took the graph server down for every user |

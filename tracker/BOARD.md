...
| Done | 12 | Judge only, never the builder |
...
| 3.0 | `phase/3.0-guardrail-node` | Full guardrail replacing the stub: validation, prompt-injection rejection, forbidden types, rate and cost pre-checks | 2.0 | v1 | done | refined | | | F-3.0-01 write category gap, T-3.0-07 blocked on the graph tunnel, T-3.0-08 fix in review on fix/c15-generation-bound |
...
|------|--------|----------------|
| F-3.1-41 stopword/gene ambiguity, F-3.1-42 lowercase gene mentions | Build phase 3.1's re-review debt (PR #22 merged 2026-08-05 without the adversarial pass over its own fix round) is fully closed as of PR #23, 2026-08-07, after three independent re-review rounds: 40 of 42 numbered findings closed, F-3.1-04 correctly carried. Two remain open, both genuine product decisions rather than bugs: F-3.1-41 (should acronym-shaped gene symbols like `ADHD`, `COPD` stay stopword-blocked) and F-3.1-42 (should a lowercase gene mention get a fallback, which reopens a live-lookup-budget problem if built wrong). Full detail: `tracker/phase_3.1.md` | Whenever the product owner decides |
...
| T-3.0-08 fix in review | F-2.1-C15's generation half is closed on `fix/c15-generation-bound`, opened 2026-08-07: a generated variable-length relationship pattern (`[:orthologous_to*]` or similar, the mechanism behind the original OOM incident) is now rejected by `validate_cypher` before execution. The `_MEMORY_GUARD_SQL` session-level mitigation is unchanged. The first version of the fix was itself found bypassable by a fresh-context review (a nested bracket alongside a `*range` spec defeated the hop-scoped check), fixed with a standalone, nesting-immune pattern, and confirmed by a second independent review, APPROVE WITH NOTES. Full account: `tracker/fix_c15_generation_bound.md`. F-2.2-01 (the missing-parentheses generation flake) is deliberately NOT touched in this branch, per the ticket's own allowed alternative: recorded as still open at its last-measured rate of roughly 1 run in 10, since re-measuring needs the same live graph tunnel T-3.0-07 cannot reach from this environment | User review and merge of the `fix/c15-generation-bound` pull request |

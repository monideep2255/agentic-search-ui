...
This table is parsed by `tracker/render_board.py`. Keep the nine columns and their order. Owner is `product owner` or empty. Gates and Flags are comma separated, or empty.

| Phase | Branch | Delivers | Depends on | Group | Status | Owner | Gates | Flags |
|-------|--------|----------|------------|-------|--------|-------|-------|-------|
| 1.0 | `phase/1.0-fastapi-skeleton` | FastAPI skeleton, health endpoint, the run() contract stub with the v1 event taxonomy typed, Pydantic boundary validation | | prototype | todo | | | |
| 1.1 | `phase/1.1-auth-service` | Minimal v1 auth, the PostgreSQL user-data schema | 1.0 | prototype | todo | | | |
| 1.2 | `phase/1.2-react-shell-sse` | React shell, SSE consumption of the event stream, empty chat endpoint wired end to end, the stop button | 1.0 | prototype | todo | product owner | playwright | playwright not installed |
...
| 2.2 | `phase/2.2-write-step-grounding` | Deterministic cite-or-refuse, provenance for Layer 1 citations, the first trust signal, the two required tests | 2.1 | prototype | todo | | eval-harness | |
...
| 3.3 | `phase/3.3-enrichment-tools` | pubtator_annotate and litvar2_lookup, each with untrusted-source-reader separation | 3.1 | v1 | todo | | | relations endpoint unverified |
...
| 3.4 | `phase/3.4-citation-trust-full` | Provenance extended to Layers 2 and 3, the two-tier risk gate, freshness and conflict resolution | 2.2, 3.1, 3.2, 3.3, 3.5 | v1 | todo | | eval-harness | |
...
| 4.4 | `phase/4.4-kgx-export` | KGX export utility, a batch job over the existing graph. No in-repo dependency, so it can land any time | | v1 | todo | | | |
| 4.5 | `phase/4.5-personalization-memory` | Bounded session memory, audience-level depth control, the named scientist persona. Never touches grounding | 1.2, 2.2 | v1 | todo | product owner | playwright | |
...
| 5.0 | `phase/5.0-observability` | LangSmith per-run tracing on trace_id, PostHog analytics, the append-only tool-call audit log | 2.0 | v1 | todo | | | |
| 5.1 | `phase/5.1-golden-dataset-eval` | The 50-query golden dataset, eval-harness grading, the cost dashboard | 3.4, 5.0 | v1 | todo | | eval-harness | needs domain sign-off owner |
| 6.0 | `phase/6.0-rate-limit-concurrency` | Per-layer throttling, the bounded queue per API family, the 20-calls-per-query budget | 3.1, 3.2, 3.3, 3.5 | v1 | todo | | | |
| 6.1 | `phase/6.1-hardening-release` | The full dev-standards six-lens pass, CI and CD gates, the security scan, accessibility | everything | v1 | todo | | dev-standards | |
| 7.0 | `phase/7.0-model-bench` | Benchmark candidate models per tier against the golden dataset, pick the winners | 5.1 | v1 | todo | | | |
| 7.1 | `phase/7.1-ab-mechanism` | The online A and B randomized-routing mechanism, human-gated | 7.0, 5.0 | v1 | todo | | | |

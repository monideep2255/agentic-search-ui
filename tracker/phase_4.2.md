...
| F-4.2-RR-01 | CRITICAL | independent verifier | closed round 3, lead-probed | The round-2 sanitizer escaped C0 and C1 and implicitly trusted everything else, so every bidirectional format character passed through and a cited clinical claim could display the opposite of what it encoded, inside the answer body. Closed by escaping on Unicode general category (`Cc`, `Cf`, `Cs`, `Co`) rather than by adding the named characters, since adding the named ones is how the next gap gets made. Lead-probed with the verifier's own payload plus U+2066, a zero-width space and a byte-order mark: all escaped, and legitimate biomedical text still passes byte-for-byte unchanged |
| F-4.2-RR-03 | major | independent verifier | closed round 3 | ROUND 2'S OWN REGRESSION, introduced while closing F-4.2-A-10. A would-be-fatal frame that failed validation was silently skipped and a later well-formed non-fatal `done` reported exit 0 with a clean trust-tagged answer. Closed by separating three cases on what the frame CLAIMS to be: an unrecognised type stays skippable per rule 10, a known type that fails validation synthesises a fatal error through the same path a real server error takes, and a truncated trailing frame is unchanged. This is the third time in this repository's history that a round's worst defect was inside the previous round's fix |
| F-4.2-RR-02 | major | independent verifier | closed round 3, with a stated residual | The forgery guard was case-sensitive and literal-only. Now matched against a same-length NFKC-folded view, built per character so a length-changing fold cannot drift the escape span off the original text. The cross-script homoglyph residual is NOT closed, needs Unicode's UTS 39 confusables data, and is pinned by its own named test so the boundary cannot be moved silently later |
| F-4.2-RR-04 | minor | independent verifier | closed round 3 | An invalid UTF-8 byte mid-stream raised an uncaught `UnicodeDecodeError` out of the splitter. Handled locally now, replacing rather than truncating, since a bad byte mid-stream does not mean the connection ended |
| F-4.2-RR-05 | informational | independent verifier | open, not reproduced | A theoretical signal-handler race at the scope's entry and exit boundaries. The pre-entry case is provably harmless. The post-exit case could in principle swallow one interrupt as a run is already finishing. NOT reproduced across dozens of real signal deliveries in six scenarios. Recorded as an open question rather than a demonstrated defect, and deliberately not fixed on a theory |
| F-4.2-RR-06 | minor | ruff, at the round-3 gate | closed round 3 | The fix for bidirectional injection embedded LITERAL bidirectional characters into this repository's own source as test payloads, which `ruff` flagged as PLE2502. That is the Trojan Source shape turned on us: a file that renders one way to a reviewer and parses another way, and a test asserting a literal RLO is absent is a test whose own source misleads its auditor. Replaced with explicit escapes; runtime values identical |
| F-4.2-10 | MAJOR | lead, at fix-round merge | closed, verified by the lead | THE PREMISE CONTRADICTS ITSELF, and it is the lead's error rather than any builder's. Two clauses cannot both hold on one path: "the rotated refresh token is written back BEFORE the retried request is issued" and "the retry policy NEVER covers `POST /v1/query`". `TestRefreshRotation` drove its 401 through the create call and asserted a second create arrived carrying the rotated token, so it was pinning clause 1 by REQUIRING the very behaviour clause 2 forbids. Nothing noticed while the code violated clause 2, because the arm and the code agreed with each other and disagreed with the premise. Fixing F-4.2-A-08 made create unretriable and the contradiction surfaced as a failing arm. Resolved: clause 2 wins for create, since that call spends a user's allowance and the server offers no idempotency key, and clause 1 governs the idempotent calls. The arm is being rewritten to drive rotation through `s3 stop` AND to additionally pin that a 401 on create yields exactly one create, which is new coverage: the gate previously observed two creates on that path and treated it as correct |
| F-4.2-09 | major | lead, full-suite run | closed by fix round 2, verified by the lead | `tests/system_03_search_agent/harness/test_tiers.py::test_no_model_id_shaped_string_outside_the_default_table` fails on `adapters/cli/client.py:358: 'text/event-stream'`. The repo-wide guard forbids a hardcoded model id anywhere outside `harness/tiers.py`'s `_DEFAULT_MODELS`, and enforces it with a `vendor/model` shape regex. An SSE `Accept` header has that exact shape, so this is a false positive of the MECHANISM rather than a violation of the PURPOSE: `text/event-stream` is a registered media type and no model id is that string. This repository has tripped this same guard once before, in build phase 2.2, when parallel workers each fixed their own file correctly and one added a sentinel that matched it (LEARNINGS.md, 2026-08-03). THE FIX IS HELD ON PURPOSE: the judge and adversary rounds are reading this exact diff, and editing files underneath a running review invalidates the evidence it is gathering. It is fixed after both report. The intended fix is a narrow, documented media-type exemption in the guard carrying its own control test proving no `_DEFAULT_MODELS` value can ever be in the exempt set, so the exemption is auditable and structurally incapable of hiding a real model id. Whether that counts as a precision improvement or as narrowing a verify surface is a question for the judge, and it is put to it deliberately rather than decided unilaterally |

...
Environment verified at open, so no builder rediscovers it: PostgreSQL 15.17 (Homebrew) is running on `localhost:5432` with superuser `<user>`. The `search_agent_users` database does not exist yet and the migration creates it. Extensions `pgcrypto` 1.3 and `pg_trgm` 1.6 are both available but not yet installed. A separate local `ncbi_kg` database exists on the same instance and is out of bounds for this phase: nothing here connects to it.
...
Evidence:
- Judge re-ran everything independently; no builder self-report was accepted. Venv confirmed as this repo's own, closing the LEARNINGS row 21 risk: `pip 26.1.2 from <repo-root>/venv/lib/python3.11/site-packages/pip (python 3.11)`, interpreter `<repo-root>/venv/bin/python`, Python 3.11.6.
...
- 2026-07-28 fix-security-findings: fixed the seven judge-confirmed findings assigned to this pass, each with a regression test derived from the finding's own reproduction. F-1.1-07: a replayed refresh token now revokes every live `auth_sessions` row for that user (RFC 6819 Section 5.2.2.3), and a new `absolute_expires_at` column carries a 90-day ceiling forward unchanged across every rotation, so a continuously rotating chain still expires; the caller still sees the same 401 and learns nothing. F-1.1-08 and F-1.1-13: email is NFKC-normalized, stripped, and lowercased at the Pydantic boundary on both signup and login, format-checked there with no new dependency, and enforced independently by a unique index on `lower(email)`. F-1.1-12: a unique index on `auth_sessions.refresh_token_hash`, plus `scalar_one_or_none` on the follow-up lookup. F-1.1-09: `decode_access_token` now requires `exp` and `user_id`, rejects a non-numeric `exp`, and rejects an `exp` beyond the 15-minute policy. F-1.1-14: `Cache-Control: no-store` and `Pragma: no-cache` on every token-bearing response. F-1.1-16: `user_agent` truncated to 512 characters before persistence. Schema changes ship as alembic revision `0002_auth_hardening` with a verified downgrade; pre-existing case-variant emails and duplicate refresh hashes were quarantined, never deleted, with originals preserved in `users.profile` and restored on downgrade. Full suite `314 passed` (284 baseline plus 30 new); one existing test, `test_refresh_rotates_and_old_token_401s_after_use`, had its final assertion reordered rather than weakened, because it asserted a chain surviving a replay, which the F-1.1-07 cascade now correctly ends. Commit `8313d82`. Status untouched: the finder is not the closer, and the judge closes

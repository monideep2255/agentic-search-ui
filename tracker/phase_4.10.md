...
| F-4.10-V-02 residual: a stopped run still costs an answer | Deliberate, not an omission. Every token was already streamed, so refunding a stopped run would let a caller read an answer to its last sentence, stop before `done`, and get the search back, a free-answer path of exactly the class F-4.10-R-01 was filed for. Recorded here because an earlier version of this table claimed every carried finding had an owner while this one and its sibling had neither a fix nor a row, which was a confident statement nobody had checked | Closed by decision, not by code |
| F-4.10-V-05 the refusal category is inferable | `charged` never reaches the wire, but `guard.category` does, and `injection` comes only from the free pre-filter while `off_topic` comes only from the paid classifier. What leaks is screen identity, not a cost figure, and the category is a product requirement | Assessed and accepted. Revisit only if a cost figure itself becomes inferable |
| Many source addresses | A caller with genuinely many source addresses gets one share of the day per address and is bounded only by the day again. This is the residual after four rounds, and it is the point at which the honest answer is that a free anonymous tier can be griefed by someone with real infrastructure. The MONEY stays bounded by `ANON_DAILY_RUN_CAP` throughout, which is the property that actually matters | Build phase 6.0's real rate limiting, and the whole-repository security scan already triggered by exposure |

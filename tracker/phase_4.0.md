...
| F-4.0-J3-01 | moderate | closed (judge round 4) | New, filed by judge round 3 against the adversary fix round. The two disclosure headers that round added, `X-Run-Cancelled` (F-4.0-A-05, closing a major finding) and `X-Citations-Export-Truncated` (F-4.0-A-13), are unreadable by a cross-origin browser client, because the CORS middleware at `adapters/web_sse/app.py:70-76` declares `allow_headers` but no `expose_headers`. Verified live: a `GET /v1/query/{run_id}/citations` carrying `Origin: http://localhost:5173` returns `access-control-allow-origin: http://localhost:5173` and `access-control-expose-headers: None`, so JavaScript on the frontend's own dev origin cannot read either header. Section 13.1 names the web UI as one of the two first-class consumers of this surface, and the CORS middleware exists in this app precisely because the frontend is cross-origin, so this is the one consumer class the disclosure does not reach. The named build-order consumers are unaffected: build phase 4.1's MCP adapter and 4.2's CLI are server-side HTTP callers with no CORS layer, and the header is correct on the wire for both. Not blocking, argued both ways in the Judge review section below. The fix is one line, adding `expose_headers=["X-Run-Cancelled", "X-Citations-Export-Truncated"]` to the existing middleware call, with no design risk. Judge round 4 verified the fix independently, with its own probe rather than the pinning test the fix added: a real `GET /v1/query/{run_id}/citations` carrying `Origin: http://localhost:5173` now returns `access-control-expose-headers: 'X-Run-Cancelled, X-Citations-Export-Truncated'` alongside `access-control-allow-origin: http://localhost:5173`, so a browser client on the frontend's own dev origin can read both. Exactly the two intended headers are exposed, nothing wider. The three round-3 probes for F-4.0-A-02/A-03, A-05, A-06 and A-13 were re-run against the same build and still pass, confirming the middleware change introduced no regression in the disclosure behavior it exposes. The accompanying comment at `adapters/web_sse/app.py` correctly states the load-bearing distinction the original miss turned on: `allow_headers` governs what a browser may SEND, `expose_headers` what it may READ back |
| F-4.0-J4-01 | minor (blocks ship, not code) | fixed, pending re-verification | New, filed by judge round 4. `tracker/BOARD.md`'s phase 4.0 row carries the status `in_progress`, but `tracker/render_board.py`'s `KNOWN_STATUSES` is built from `COLUMNS`, whose keys are hyphenated (`todo`, `in-progress`, `blocked`, `in-review`, `done`, plus `rejected`). The underscore form is not a valid status, so `python3 tracker/render_board.py --check` fails outright: `error: malformed board, refusing to render. unknown status 'in_progress' on phase 4.0`. It is the only offending row (`grep` over the phase table returns 17 `done`, 13 `todo`, 1 `in_progress`), and the value does not appear anywhere in `develop`'s copy of the file, so it was introduced on this branch by `d5b79f6`'s tracker housekeeping. Consequences, both live: `.claude/hooks/sync-board.sh` invokes this renderer (its own `depends_on` header names `tracker/render_board.py`), so the generated page has not been regenerated since the typo landed, and `tracker/board.html` is now stale against `tracker/BOARD.md` (Aug 10 21:41 versus Aug 11 08:54). Zero shipped-code impact: no `src/` file, no test and no runtime behavior is affected. Filed as blocking for the ship step rather than for the phase's code, since `/phase-checkpoint` and the board hook both run these tracker tools. Fix is one character, `in_progress` to `in-progress`. Not applied by the judge: the phase's own row status is the lead's to set, and moving it to `done` is only correct once the phase actually closes. Judge round 4 also owns the miss: rounds 2 and 3 ran `tracker/check_doc_drift.py` but never `tracker/render_board.py`, so a broken board survived two PASS verdicts |
...
### Round 4, confirmation of the CORS fix and the carry-tracking follow-ups

2026-08-11, against `11a6d27`: PASS on all code and all findings. One new tracker-tooling defect filed (F-4.0-J4-01), which does not touch shipped code but does block the ship step until a one-character fix lands.

| Check | Result |
| --- | --- |
| Phase premise gate | `26 passed in 8.02s` |
| Targeted regression | `420 passed, 1 warning in 18.07s` |
| Full repo suite | `6 failed, 2397 passed, 113 skipped, 1 xfailed, 1 warning in 57.65s` |
| Ruff, `app.py` | `All checks passed!` |
| `tracker/render_board.py --check` | `error: malformed board, refusing to render. unknown status 'in_progress' on phase 4.0` (F-4.0-J4-01) |

F-4.0-J3-01 verified closed with the judge's own probe rather than the pinning test the fix added; evidence is on its row in the Findings table above. The fix's own framing is worth keeping: the miss turned on `allow_headers` (what a browser may send) being a different CORS control from `expose_headers` (what it may read back), and the new comment says exactly that. The lead's observation that `Access-Control-Expose-Headers` appears only on the actual response and not on an `OPTIONS` preflight is correct, and matches the probe shape used here in round 3 and again in round 4, a real `GET` carrying an `Origin` header.

Both round-3 conditions on the A-10/A-11 carry were checked and are met:

- The carried-open paragraph no longer rests on `.claude/rules/v1-scope-boundary.md`. It now records the correction explicitly, states that rate limiting is on neither the PRD out-of-scope list nor the fast-follow table, and names the real basis: the pre-registered coverage exclusion, build phase 6.0 owning rate limiting by name, and no deploy existing yet.
- `tracker/BOARD.md`'s Open flags table gained three rows covering F-4.0-A-10/A-11, F-4.0-A-12 and F-4.0-A-14, each carrying its repro summary, the reason it was not fixed here, and an owner. Verified accurate against the adversary's own measurements (9,615 runs accepted, `PER_USER_DAILY_QUERY_CAP` enforced one layer below `create_run`, the sweep's 1.53 ms to 4.18 ms cost curve, the five-silent-socket result at the shipped 30-second default). The A-10/A-11 row also carries forward the judge's qualification that the premise's "does not grow without bound" clause is false as literally written.

One observation on those rows, recorded rather than filed: F-4.0-A-10/A-11 has a hard owner (build phase 6.0), while F-4.0-A-12 and F-4.0-A-14 have conditional ones ("whichever phase next touches...", "whenever a future round revisits..."). Both do name a terminal backstop phase, so neither repeats F-2.0-15's actual failure, which was an owner field reading "unassigned". Worth watching at the next checkpoint rather than blocking on.

...
| F-5.0-09 | major | The tracing redactor's structural default-deny FAILS OPEN. `tracing.py:146` recognizes a Query-shaped dict with `key_set.issubset(_QUERY_FIELDS)`, so a payload carrying ONE key the model does not declare is not recognized, the safe-field allowlist never applies, and the whole structural rule disengages. Measured: with the exact model shape `session_id` is `[redacted]`; add one unrelated key and `session_id` ships in cleartext alongside the unexpected field | lead, verifying the T-5.0-03 builder's own stated property | open | The recognition test keys on a PROXY, does this dict look exactly like a Query, rather than on the PROPERTY, is this field safe to transmit. That is build phase 4.3's check-the-value-not-its-provenance shape, and unexpected content is precisely the condition that should tighten the control rather than switch it off. HALF THE BUILDER'S CLAIM IS TRUE and is recorded so the fix does not discard it: a field ADDED TO THE MODEL is caught by default, because `_QUERY_FIELDS` reads `model_fields` at import and the payload stays a subset. What fails is an extra key that is not on the model. Owned by a fix to T-5.0-03; the category rule still covers today's named fields, which is why this is major rather than critical |

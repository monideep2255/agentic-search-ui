...
| F-5.0-19 | major, BLOCKING, RULE 4 | A non-secret assignment whose value is a URL SWALLOWS the secret assignment inside it, and nothing is redacted at all. Measured, output byte-identical to input: `url=https://h/x?api_key=<secret>`, `endpoint=https://h/x?api_key=<secret>`, and `request url=https://... failed`. The discriminator is a preceding `name=` whose value is a URL, NOT the character class and NOT whether the value is a URL: `db=gene api_key=<secret>` redacts, `api_key=<secret> db=gene` redacts, and a DSN under a leading assignment redacts because the netloc rule catches it independently | lead, adjudicating the disagreement between F-5.0-16 and F-5.0-18 | open, ESCALATED | THIS IS THE SAME CLASS AS F-5.0-14, which this round existed to remove: something upstream of the credential consumes it, so the credential is never scanned. The shape is ordinary rather than exotic, since `url=https://...` is a natural way for an error message to name what it was calling. RULE 4 APPLIES, and this is the THIRD stop in this phase |

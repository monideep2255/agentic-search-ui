...
| F-5.0-08 | critical | The audit log can write the NCBI API key in cleartext into an append-only file. `ncbi_transport._append_api_key` (line 1105) appends the credential to the QUERY STRING, so the `url` at the transport chokepoint carries the secret. Section 20.3 requires the audit line to record "endpoint or database called", and `redact_params` matches on KEY NAME, so a field named `endpoint` or `url` holding that URL is not redacted by anything | lead, reviewing the T-5.0-02 module against the transport it will hook | open | Not yet reachable: nothing calls `record_tool_call` until T-5.0-05 wires it. It becomes reachable in the same edit that makes this phase work, which is why it is filed as a blocker on T-5.0-05 rather than as a note. THE EXISTING MODULE ALREADY KNOWS: `_host_of` (line 1123) exists for exactly this reason and its docstring says so, and every diagnostic in that module uses it instead of the raw url. The audit hook must do the same, and the redactor must additionally scan VALUES for a secret pattern rather than key names alone, since a key-name rule cannot see a credential embedded in a URL under an innocuous key. Worse than an ordinary leak because the sink is append-only and never mutated after write, so it cannot be scrubbed afterwards without breaking the guarantee the log exists to provide |

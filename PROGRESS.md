...
1. Teach the system to understand a stretch of chromosome coordinates, so "what is in this region" gets an answer rather than a request to name a gene.
...
3. The rest of the screen work: a bigger, clearer disclaimer notice, and the smaller items on the tester's list that nobody has objected to yet.

Done since this list was last written, on the evening of 22 September: the two questions whose own database search used to fail before it ran, comparing two genes and asking for expression datasets, now get that search, and the datasets question now searches the government's dataset catalogue as well. It is on the practice site and waits for the product owner to try it.

...
1. Fix the two questions whose own database search still fails before it runs, comparing MLH1 and MSH2, and GEO datasets on TP53. The reader is now told; the search is not fixed.
2. Teach the system to understand a stretch of chromosome coordinates, so "what is in this region" gets an answer rather than a request to name a gene.

...
The web page now looks like a real product, which it did not two days ago. It is not finished. Comparing the built page against the approved design side by side found five places where it falls short: every source is shown fully expanded instead of folding away until you click it, so an answer with six sources is a wall of text; a source does not say in words which of the three data layers it came from, so you have to already know what "L1" means; the date our own database snapshot was taken is missing from a source, which is the one field that lets a reader tell a stored value from a freshly fetched one; and once an answer has landed there is no way to reopen the record of how it was found. None of these is a wrong answer, and all five are written down and owned.
...
1. Other ways to reach the system, none of which depend on the web page. A command-line client, a second style of programming interface, and a bulk export of our own database in a standard format. These are three separate sprints and any of them can go first.

   The web page redesign that used to sit at the top of this list is done. It looks like a real product now, and it was worth doing carefully: three independent reviews all failed it, and a fourth check, simply opening the page and looking at it, found two more problems on top.
...
|-------------------------|--------------------|
| Nothing in this project looks at the finished web page. Every check asks what is on the screen, never where it is, which is how the whole application ended up drawing itself into a narrow strip down the middle with the product's own name wrapped onto three lines, while every check stayed green | No sprint owns this yet. Until one does, opening the page and looking at it is a required step, not a nicety |
| The built web page falls short of the approved design in five places: sources are always fully expanded instead of folding away, a source does not say in words which data layer it came from, the date our database snapshot was taken is missing, the citation markers use the plainer of two approved styles, and there is no way to reopen the record of how an answer was found once it has landed | A follow-up design pass, or whichever sprint next touches the answer screen |

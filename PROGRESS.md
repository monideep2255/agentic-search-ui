...
1. Teach the system the one remaining kind of question it cannot start on: a description of a bacterial sample. Unlike the chromosome stretch and the project accession, there is no existing search to build on, so the first step is agreeing what such a question should return, then adding that search to the pathogen catalogue tool.
...
3. Decide three test questions where the gatekeeper and the expected answer disagree.

...
1. Measure how close a search comes to its limit of twenty outside lookups, now that a question can add lookups for datasets or for a stretch of chromosome, before deciding whether the limit moves.
2. Teach the system the two remaining kinds of question it cannot start on: a description of a bacterial sample and a project accession number.
...
Done since this list was last written, on the evening of 22 September: the two questions whose own database search used to fail before it ran, comparing two genes and asking for expression datasets, now get that search, and the datasets question now searches the government's dataset catalogue as well. Later the same night, a stretch of chromosome coordinates gets an answer: the genes under it, named without being told, and the structural and clinical variants recorded for that exact stretch, and a question that forgets to say which genome build is asked which. All of it is on the practice site and waits for the product owner to try it.

...
- ~~Two questions that the product is not supposed to handle at all, one asking it to run a sequence search and one handing it a file of genetic variants, were answered rather than turned away.~~ DECIDED and FIXED on 22 September: the product owner chose to turn them away, and the gatekeeper now does, with a message saying the capability does not exist and where to run a sequence search instead. On the practice site, waiting to be confirmed.
...
| ~~Two requests the product is not built to handle, a sequence search and a file of genetic variants, were answered about the disease terms found inside them rather than turned away~~ | DECIDED and FIXED 22 September: turned away with a message saying the capability does not exist. On the practice site, waiting to be confirmed |

...
- One search in ten quietly loses an entire source of information. First seen over twenty live runs on 21 September, and now measured over the full test on 22 September: of 119 database lookups where the same question had found records on another try, 12 came back with nothing. We can now see where it happens. The lookup reports that it failed, with no reason attached, and the answer carries on as if nothing happened. Nothing on the screen tells you, nothing in the server's log says why, and the reason exists only in a tracing record nobody has read yet.
- Nine of the fifty test questions fail that database lookup every single time, not just sometimes. Six of them end as "I could not find grounded evidence" for questions we expect to be answerable. This is the biggest single reason a test question does not answer, and it is a defect with a fixed list of examples rather than bad luck.
- Two questions that the product is not supposed to handle at all, one asking it to run a sequence search and one handing it a file of genetic variants, were answered rather than turned away. The gatekeeper let them through, the system found real disease terms inside the wording, and it answered about those. Whether to refuse outright or answer honestly for the terms named is a product decision, not yet made.
...
- One question shape is slow every time. "I am a student, explain in plain terms what the BRCA1 gene does" took 110, 104 and 97 seconds on three tries, while the typical search takes about 12 and nine in ten finish under 40. So the earlier one-off 127-second search now has a name: a plain-language explanation over a gene with a great deal of data behind it. Nobody has found out why it costs that much.
...
2. Read one tracing record for a search that lost its database results, and find out why the lookup failed. Nine questions fail this way every time and one in ten fails at random; one reason may explain both. This is an hour's work and it decides everything after it.
3. Decide what to tell the reader when a lookup fails. We now know exactly where that note would attach. The cheapest honest fix is to say it happened, rather than to stop it happening.
4. Decide what the product should do with a request it cannot fulfil, like running a sequence search: turn it away, or answer for the terms it named and say plainly what it cannot do.
...
| Two requests the product is not built to handle, a sequence search and a file of genetic variants, were answered about the disease terms found inside them rather than turned away | A product decision, raised 22 September: refuse outright, or answer for the terms named and say what the product cannot do |
| One question shape takes about 100 seconds every time: a plain-language explanation of a well-studied gene. The typical search takes 12 seconds and nine in ten finish under 40 | Named on 22 September, cause not found. Nobody owns it |

...
1. Decide four test questions where the gatekeeper or the expected answer disagree with what the product does: three where the gatekeeper turns away a question the test expects answered, and one where the test names an older address for an organism's record page than the one the product now links to.
...
3. Three conversations that each come before a piece of building: whether the integrations address should trust the network in front of it, whether the outside lookups should be wrapped as separate services, and which kinds of question need the system to connect facts across several steps.
...
Done since this list was last written, late on the night of 22 September: the last kind of question that never got an answer, a description of bacterial samples carrying a resistance gene, now does, after the product owner agreed what such an answer should contain. Earlier the same evening: the two questions whose own database search used to fail before it ran, comparing two genes and asking for expression datasets, now get that search, and the datasets question now searches the government's dataset catalogue as well. Later the same night, a stretch of chromosome coordinates gets an answer: the genes under it, named without being told, and the structural and clinical variants recorded for that exact stretch, and a question that forgets to say which genome build is asked which. Late the same night, three more: the limit on outside lookups was measured on eight kinds of question and never reached, so it stays; an answer no longer ends by listing conditions the person never mentioned; and a project accession gets its answer. All of it is on the practice site and waits for the product owner to try it.

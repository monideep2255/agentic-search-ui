...
Internal MCP servers for Layer 2 and Layer 3, the product owner's idea of
2026-09-20: wrap each live API behind an internal MCP server rather than
calling it directly from the tool layer. The connection maps and per-database
setup notes are under the `NIH/NCBI Technical-development-workflow/
Architecture-and-databases` tree in the symlinked reference material, which the
product owner named when raising this. <!-- local-refs: allow -->
Recorded as a backlog item and nothing
more: it crosses the tool-integration boundary the locked technical
specification's Section 6 defines, so it needs scoping against
`.claude/rules/v1-scope-boundary.md` before any work starts.

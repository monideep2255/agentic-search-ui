...
Every closed item from the UI fix loop lives here, with its detail and its
history: built and live on develop, whether the product owner has approved it
...
we stopped", and the detail behind every item still on the board except the
architecture cards, whose detail sits under the board's To do.
...
- Your retest: waiting on you: retests. Each of these is built, live on
  ...
  on them
- Parked: parked, and never work the product owner will build. A discussion
  that precedes a build waits on Nobody on it, not Parked. Corrected
  2026-09-24 on the product owner's instruction, after discussions that
  precede a build had been marked Parked
...
- THE ARCHITECTURE WORK IS NOT PARKED. The product owner will build it, so
  ...
  items Parked and moved every word of their detail into this file. That
  detail, with their direction on the model architecture, now sits under
  the board's To do.
...
Moved here from "Where we stopped" in `testing/UI_fix_plan.md` at the

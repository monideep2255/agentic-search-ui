...
| 11.33 | PubMed abstracts reach the answer page cut off MID-WORD, for example "...inherited breast and/or ovarian c [29]" and "...has been uncle [33]" | FIXED, live pending | Found 2026-09-21 on develop at `46fff40`. Every character cap in the codebase was ruled out BY EXECUTION, including `_cap_text`, which appends a literal " [truncated]" that appears nowhere in the capture. The sentence splitter cannot cut mid-word either. The remaining hypothesis, that the model itself emitted the fragment while attempting a long verbatim quote, could NOT be reproduced, because `claim_introduces_no_new_content` rejects the reconstruction. SOLVED LATER THE SAME DAY, and the first investigation's conclusion was WRONG in an instructive way. It ruled out `answer_layout`'s 500-character slice because the visible fragments were 49 to 153 characters, far short of 500. What that missed is that a fragment begins at the last sentence boundary INSIDE the slice, so its length says nothing about where the slice fell. Settled by fetching the real NCBI gene summary and finding the offset of the rendered fragment's end in it: 500 EXACTLY. Fixed with `clip_to_word`, which cuts at the last word boundary and appends an ellipsis, applied to all five slices in `answer_layout.py`. A value with no whitespace, such as an HGVS name, is still cut hard, because there is no word boundary to honour. The earlier diagnosis is kept at `testing/Developer/reports/2026-09-21_11.31_divergence/truncation/findings.md` rather than deleted, since how it went wrong is worth more than the fact that it did |

...
|---|---|---|
| The answer answers the question instead of listing what was found | 12.10 | NOT STARTED, raised 2026-09-23 by the product owner. Judged on their two criteria: information provided in six of seven, question answered in ONE of seven. Same root cause as 12.9, and they should be decided together |
| The sources chip and the trust line agree with each other | 12.11 | NOT STARTED. A regression from 12.8 the same day: the line counts citations and the list counts distinct pages |
...
| Plain language and researcher stop returning the same text | 12.9 | DIAGNOSED 2026-09-23, NOT BUILT. 11.31 is BYPASSED rather than broken: the depth reaches the model, the model writes different prose, and the grounding gate deletes all of it, so the code-built listing ships at both depths. Three options, all the product owner's call. See its row |
...
12.6 was found already built; 12.8 and 12.9 are in progress.

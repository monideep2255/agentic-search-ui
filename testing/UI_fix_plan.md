...
| 10 | A question asking for recent papers asks what recent means | 12.15 | Nobody on it | RAISED 2026-09-23 from the fix-2 screenshots. NOT STARTED, nobody on it |
| 11 | No hardcoded decisions: prompts, routing and the clarify decision stop relying on word lists and test-question examples | 12.16 | Parts 1 and 2 in progress; part 4 your decision | RAISED 2026-09-24 by the product owner. Audit in 12.16's row |
| 12 | Four golden test rows disagree with what the product does, row by row | the golden rows | Your decision | The test set only, nothing a user sees |
| 13 | Is twenty sources the right ceiling? | the ceiling, measured and unchanged | Your decision | Every answer hits it and then tells the reader it was cut short, which is a large part of why a good answer reads as a thin one |
| 14 | Tell the reader when the system wrote its own search rather than using a checked one | "Next, in order" item 5 | Nobody on it | Not started. The data exists: `CypherQueryOutput.template` is None exactly then. THE TRAP: the degradation is `ok` to `ok`, never `empty` |
| 15 | Hard and soft edges over a fuller graph, "connecting the dots" | 11.29 | Parked | A discussion that precedes a build. Its scoping document now exists and is measured: `testing/Developer/reports/2026-09-23_overnight/soft_edges_scoping.md` |
| 16 | A bounded trial of the probability model | 11.38 | Parked | Backlog only, nothing designed and nothing promised |
| 17 | Does the Plain language answer keep its small grey medical-advice line? | 9.11 | Your decision | One line under every plain-language answer |
| 18 | The trust-line wording | 9.9 | Your decision | One line under every answer |
| 19 | Judge answer quality once answering is reliable | 10.4 | Nobody on it | Not built. After the release, once 10.3's consistency run shows reliable answering |
| 20 | The load-dependent frontend tests | D4, under "Developer follow-through" | Nobody on it | Journey 7 FIXED 2026-09-23. The load-dependent suite is diagnosed and was still being worked at the close of that session |
| 21 | Internal MCP servers around the Layer 2 and Layer 3 calls | 11.32 | Parked | A discussion that precedes a build under `/bossman-mode`, reclassified 2026-09-22 |
| 22 | The explanation half of 11.31 | 11.31 | Parked | Parked. You approved the current state as is on 2026-09-21 |
| 23 | The byte ceiling at 50,000 | see its row | Parked | Parked |

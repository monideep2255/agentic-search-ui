...
further down; the detail stays with the item.
...
Order: 12.9 is part of item 1 of "Next, in order" (in "Where we stopped"),
12.3 is its item 2, and 11.11 is not on that list.

| Priority | Feature, in plain words | Item | Where it stands |
|---|---|---|---|
| 1 | Plain language and researcher differ on EVERY question, not just a few | 12.9 | IN PROGRESS 2026-09-23 on the product owner's direction: "The plain vs researcher should vary duh for all questions. Not just a few. Think hard on what rules to set". The six rules are in 12.9's row in Set 12. Today only the model's prose differs by depth; the opening sentence and the record list are identical, so whenever the prose does not survive the two answers are the same text |
| 2 | A one-to-three-word question is asked back, such as `Reflux disease` | 12.3 | APPROVED 2026-09-23 by the product owner: "If clarify needed -> yes approved". IN PROGRESS. Design in 12.3's row in Set 12 |
| 3 | Answers modelled on the reference prototype's depth, formatting and structure | 11.11 | In progress: the detail agent is modelling answers on it. The answer-writing model is unchanged; switching models is a separate decision |
...
| 5 | The answer answers the question instead of listing what was found | 12.10 | Your retest | LIVE on develop at `5d53f78` 2026-09-23, awaiting your retest. A second, cheap model checks each reworded sentence against the exact record words it quotes; code first checks the quote is in the record character for character, the numbers, the negation, and that the sentence does not open on a yes or no verdict. Measured live: every plain-language answer of the seven answered in sentences; two of six guarded reruns fell back to a list, one on a failed model call, which is the check failing closed |
| 6 | The sources chip and the trust line agree with each other | 12.11 | Your retest | LIVE on develop at `6f8902e` 2026-09-23, awaiting your retest. It now counts distinct pages, the same key the source list merges on. A regression from 12.8 the same day: the line counts citations and the list counts distinct pages |
| 7 | Broken sentences and repeated records in the answer | 12.12 | Your retest | LIVE on develop at `6f8902e` 2026-09-23, awaiting your retest. An orphan fragment starting lowercase with a stray quote mark, a paragraph opening "Another is titled" with no first, and the same records shown up to three times |
| 8 | A search clicked in the history rail re-runs instead of showing its saved answer | 12.13 | Your retest | LIVE on develop at `5d53f78` 2026-09-23, awaiting your retest. A search asked in this tab now opens its saved answer |
| 9 | A question about phenotypic features names none | 12.14 | Nobody on it | RAISED 2026-09-23 from the fix-2 screenshots. NOT STARTED, nobody on it |
| 10 | Four golden test rows disagree with what the product does, row by row | the golden rows | Your decision | The test set only, nothing a user sees |
| 11 | Is twenty sources the right ceiling? | the ceiling, measured and unchanged | Your decision | Every answer hits it and then tells the reader it was cut short, which is a large part of why a good answer reads as a thin one |
| 12 | Tell the reader when the system wrote its own search rather than using a checked one | "Next, in order" item 5 | Nobody on it | Not started. The data exists: `CypherQueryOutput.template` is None exactly then. THE TRAP: the degradation is `ok` to `ok`, never `empty` |
| 13 | Hard and soft edges over a fuller graph, "connecting the dots" | 11.29 | Parked | A discussion that precedes a build. Its scoping document now exists and is measured: `testing/Developer/reports/2026-09-23_overnight/soft_edges_scoping.md` |
| 14 | A bounded trial of the probability model | 11.38 | Parked | Backlog only, nothing designed and nothing promised |
| 15 | Does the Plain language answer keep its small grey medical-advice line? | 9.11 | Your decision | One line under every plain-language answer |
| 16 | The trust-line wording | 9.9 | Your decision | One line under every answer |
| 17 | Judge answer quality once answering is reliable | 10.4 | Nobody on it | Not built. After the release, once 10.3's consistency run shows reliable answering |
| 18 | The load-dependent frontend tests | D4, under "Developer follow-through" | Nobody on it | Journey 7 FIXED 2026-09-23. The load-dependent suite is diagnosed and was still being worked at the close of that session |
| 19 | Internal MCP servers around the Layer 2 and Layer 3 calls | 11.32 | Parked | A discussion that precedes a build under `/bossman-mode`, reclassified 2026-09-22 |
| 20 | The explanation half of 11.31 | 11.31 | Parked | Parked. You approved the current state as is on 2026-09-21 |
| 21 | The byte ceiling at 50,000 | see its row | Parked | Parked |
...
- 11.30 fixed 2026-09-23 and live-verified end to end
...
- A BLAST or VCF request is refused at the guardrail as `compute_request`,
  ...
  golden texts refuse with the compute copy and a CFTR variants question
  still answers.
...
- Item 11.33's fix: one string cap in the coordinator worker at the finding's
  ...
  zero ellipses. APPROVED by the product owner's retest the same day.
...
1. SET 12, THE SECOND TESTER'S QUESTIONS. The priority, and the one item a
   ...
   at the head of Set 12. Three pieces of work, independent of each other,
   ...
   - 12.6 turned out to be ALREADY BUILT and is closed with no work. The
     ...
     what the screen shows, and it is corrected in the Set 12 table rather
     than deleted.

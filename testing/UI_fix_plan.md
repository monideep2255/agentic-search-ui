...
|---|---|---|
| May a second, cheap model check each written sentence against the exact record words it quotes? The quote itself stays checked by code, character for character. Yes lets answers actually answer; no keeps every answer a list | Whether any answer can be written in the product's own words. MEASURED 2026-09-23: with code-only checking, 0 of 53 model sentences survived across six live replies, because the model paraphrases ("kidney" for "renal") and no fixed rule can verify a synonym. It changes the written rule that acceptance is never a model's judgement | 12.10, 12.9 |
| Fix the history rail now? A search asked in the same tab re-runs when clicked instead of showing the saved answer. Cause found, one small frontend change | Every signed-in user who clicks back to a search they just asked | 12.13 |
...
|---|---|---|
| The answer answers the question instead of listing what was found | 12.10 | BLOCKED ON YOUR DECISION, 2026-09-23. Option B was built as code-only checking and MEASURED: 0 of 53 model sentences survived across six live replies. The model writes the answer you want and paraphrases it, and code cannot verify a synonym. The next step needs your yes or no, first row of section 2 |
...
| A search clicked in the history rail re-runs instead of showing its saved answer | 12.13 | DIAGNOSED 2026-09-23 from the fix-2 screenshots, NOT BUILT, waiting on your go (section 2) |
...
| Plain language and researcher stop returning the same text | 12.9 | BLOCKED ON THE SAME DECISION AS 12.10, 2026-09-23. The two depths differ once the model's own prose survives the gate, and it does not survive code-only checking. The fix-2 screenshots confirm researcher is SHORTER: one sentence against two, 13 sources against 53, on the Marfan phenotype question |

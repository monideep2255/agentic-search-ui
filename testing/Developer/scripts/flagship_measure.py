...
for q in QUESTIONS:
    ...
    for i in range(N):
        ...
        started = time.time()
        try:
            run = post("/v1/query", {"text": q, "audience_depth": "researcher", "session_id": session}, token)
            events = stream(run["run_id"], token)
        except Exception as exc:  # noqa: BLE001
            results[q].append(("transport_error", 0, frozenset(), str(exc)[:80]))
            print(f"{q[:45]:45} run {i+1}: transport_error")
            continue
        ...
        if outcome in ("answer", "ask", "flag") and not sources:
            outcome = "no_sources"
        results[q].append((outcome, elapsed, sources, error["payload"]["source"] if error else ""))
        print(f"{q[:45]:45} run {i+1}: {outcome:10} {elapsed:5}s sources={len(sources)}")

print("\n=== summary ===")
for q, rows in results.items():
    answered = sum(1 for r in rows if r[0] in ("answer", "ask", "flag"))
    sets = Counter(r[2] for r in rows if r[2])
    print(f"{q[:50]:50} answered {answered} of {len(rows)} | distinct source sets {len(sets)} | sizes {[len(s) for s in sets]}")

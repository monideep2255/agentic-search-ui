...
| 10 | Journey 7 selects nav items by `getByRole("link")` when they are buttons, inside a `.catch()`, so it has probably never navigated anywhere while reporting success | FIXED 2026-09-23: correct role plus the overflow-menu path below 720px, `.catch()` removed, proven red against the old selector and green against the new one on a local stack |
...
| 12 | The frontend suite fails by machine load. Identical code gave 5 failed on a 139-second run and 245 passed on a 50-second run | FIXED 2026-09-23, with a caveat stated rather than hidden. Root mechanism: `useAnswerReveal`/`usePacedEvents` held the landed answer behind real `setTimeout` waits racing a fixed 10000ms ceiling, measured at 61-77% of that ceiling unloaded; a first fix (fake timers via a 400-step `act()` loop) removed the wait but was itself CPU-bound and failed for real in an unloaded 10-run confirmation, so it was cut to a 73-step loop sized to the hooks' own cadence (`perItemMs`). Verified green, at the cut settings: `phase49Premise.test.tsx` 3 unloaded runs plus 1 moderate-load run, worst case 2459ms against 15000ms (16%); the other three affected files individually and combined; one full-suite run, 527/527. NOT verified: a full 10-run confirmation of the fixed version specifically (the 10-run confirmation that exists was correctly stopped after run 1 caught the pre-cut version failing, and was not restarted from zero). Full account: `testing/Developer/reports/2026-09-23_overnight/findings.md`, every entry headed "Worker C". |

...
async def one(question: str, depth: str) -> dict:
    ...
    async for event in run(query, RequestContext(surface="rest_sse")):
        ...
        if event.type == "think":
            think_narrative = payload.get("narrative")
    ...
    return {
        ...
        # Variant-to-disease detail (2026-09-14): the code-built table, list
        # and notes, verbatim, so the proof can count rows and diseases.
        "table_rows": [t.get("cells") or [] for t in tokens if t.get("kind") == "table_row"],
        "table_row_marker_counts": [len(t.get("marker_ids") or []) for t in tokens if t.get("kind") == "table_row"],
        "list_items": [(t.get("cells") or [""])[0] for t in tokens if t.get("kind") == "list_item"],
        "notes": [t["text"] for t in tokens if t.get("kind") == "note"],
        "think_narrative": think_narrative,
    }

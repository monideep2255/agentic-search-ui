...
test.describe("the architecture page", () => {
  for (const { label, width, height } of [
    ...
    test(`renders its three systems at ${label} width with no sideways scroll`, async ({
      page,
    }) => {
      ...
      const systems = page.getByTestId("architecture-systems");
      await expect(systems, "the architecture page did not render").toBeVisible();
      await expect(systems.getByRole("heading", { level: 3 })).toHaveCount(3);

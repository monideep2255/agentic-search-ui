...
Kick-off: 2026-05-06. Last updated: 2026-08-24.
...
| Phase 6: build (bossman execution) | In progress. Step 6.1 (prototype) COMPLETE, all six build phases merged. Step 6.3 (build v1) underway. Build phases 1.0 to 3.0 merged as PRs #5, #6, #9, #12, #15, #18, #19. Build phase 3.1 (`ncbi_efetch`, the first Layer 2 tool) merged as PR #22 (superseded by PR #23), with the premise restated and the answer-path half (Act-step wiring, Layer 2 citation, trust gate) carried to T-3.1-28. It merged without the adversarial pass over its own fix round; that debt was fully closed via three independent re-review rounds and a fix round, merged as PR #23. 40 of 42 numbered findings closed, two left open on genuine product decisions (F-3.1-41, F-3.1-42). F-2.1-C15's generation half (the finding where a generated query took the graph server down for every user) closed on `fix/c15-generation-bound`, merged as PR #24: a variable-length relationship pattern is now rejected before execution, after a fresh-context review found a bypass in the fix's own first version and a second review confirmed the correction. Build phase 3.2 (`ncbi_dbsnp`, the second Layer 2 tool) closed after six review rounds (adversary, judge FAIL, fix round, independent re-review, second fix round, lead re-verification): 16 adversary findings and 6 judge findings, all real, zero rejected. Three spec-versus-reality gaps carried to Step 6.2: the dbVar coordinate-overlap sub-tool Section 25 names for this phase already shipped in 3.1, Section 6.3's named SPDI normalization endpoint is confirmed live-broken and substituted, and the locked `clinical_significance` cap is too tight for real ClinVar vocabulary (now disclosed via a `fields_withheld` field rather than raised unilaterally). Build phase 3.3 (`pubtator_annotate` and `litvar2_lookup`, the two Layer 3 enrichment tools) closed on `phase/3.3-enrichment-tools`, merged as PR #26, after ten review rounds: a blocking premise gate written and watched failing first, a judge round (FAIL, two majors), a fix round, an independent re-review that found a real regression in that fix round, a second fix round, an adversary round against the live APIs (13 findings, 1 critical: both tools silently discarded the upstream API's own match-relevance signal, so a bare number or a common word returned confidently cited but wholly unrelated data), a third fix round, a fourth fix round closing several carried findings that turned out addressable without a product decision after all, an independent re-review of that round that found one more real regression (a multi-match result citing only its first, unrelated match as if it covered the whole answer), and a fifth fix round closing it. Build phase 3.5 (`pathogen_detection` and `clinicaltrials_search`) closed on `phase/3.5-pathogen-clinicaltrials-tools`, completing the seven-tool roster, after a judge round (one critical, three majors, two minors, all closed) and an adversary round that found the judge round's own fix had introduced two NEW critical regressions of the identical discard-real-data shape (`cluster_snp_neighbors` could no longer ever succeed; `clinicaltrials_search` pagination errored past page one), both closed across two more fix rounds and re-verified live against the adversary's own repro case each time. Build phase 3.4 (citation trust extended to Layers 2 and 3, the two-tier risk gate, data freshness and conflict resolution, T-3.1-28 folded in) closed on `phase/3.4-citation-trust-full`, merged as PR #28: the last of the six Step 6.3 tool-and-trust phases, after two judge rounds, a fix round, an adversary round (7 findings, 1 critical: a two-gene query silently dropped the second gene under a confident `answer` outcome), a fix round closing the critical and two majors, and a final judge round that independently live-verified the fix round itself. With 3.0 through 3.5 all merged, Step 6.2 (the document-reconciliation pause) ran and closed the same day, 2026-08-10, across eight PRs (#29 through #36): see Step 6.2's own section below for the full account. Build phase 4.0 (the REST plus SSE adapter finalized as the public API surface) merged as PR #39, 2026-08-11: four judge rounds (round 1 FAILED on two blocking findings, rounds 2 through 4 PASSED after fix rounds) and one adversary round (14 findings, 8 fixed and judge-confirmed, 2 resolved by disclosure, 4 carried open with a named owner each on `tracker/BOARD.md`). Full account: `tracker/phase_4.0.md`. Build phase 4.1 (the outbound-only MCP server wrapping the same tool functions) merged as PR #40, 2026-08-11: three judge rounds (round 1 FAILED on a gate-integrity bug in the premise gate's own leak-detection assertions, round 2 PASSED after a fix round, round 3 PASSED with independent live re-verification including counterfactual mutation testing) and one adversary round (16 findings, 2 critical; 13 closed, 2 carried open with a named owner each on `tracker/BOARD.md`, F-4.1-A-10 and F-4.1-A-15). Full account: `tracker/phase_4.1.md`. The same day, build phase 4.8 (Web UI visual design: MUI adoption, a real theme, restyling the screens built in phase 1.2) was inserted into Section 25's locked build order, a deliberate product-owner-directed exception to the Step 6.2-only edit cadence, since the shipped UI was found too unstyled to demo. Numbered as the next open slot in the delivery-surfaces group, it depends only on 1.2 (already merged) and runs immediately after 4.1, before 4.2 through 4.7, none of which depend on it. Build phase 4.8 (Web UI visual design) is COMPLETE, merged to `develop` as PR #41 on 2026-08-13, then closed out via PR #42 on 2026-08-14, which settled the product owner's three findings and aligned the stored-searches rail to the approved prototype. All 15 tickets done. Gated first by a design review that ran OUTSIDE the build cadence, since a visual deliverable has no natural failing test and the design system serves as the premise gate's fixture: the product owner iterated on a clickable prototype in Claude Design, the lead pulled and validated it, corrections went back, and an explicit approval opened the phase. Three independent review rounds ran and ALL THREE returned FAIL, filing 56 findings between them; 48 are closed and 8 carried with a named owner each. Round 1 (judge) found that an anonymous visitor asking ANY question was shown a fabricated fully-cited answer with a real NCBI source URL and a "Grounded, every claim cited" pill, bypassing the phase 3.0 guardrail on the most-travelled path. Round 2 (adversary) found the root cause of a whole family: `marker_ids`, the wire's exact token-to-citation binding, had no production consumer, and the UI re-derived it with a substring heuristic, so a citation whose claim_text was "cancer" cited every sentence containing the word. Round 3 (re-review, aimed at the fix rounds) found the previous round's own fix skipped the run screen for every question after the first, making Stop unreachable on a cost-capped loop. Two long-standing repository defects were also unmasked and fixed: the Playwright webServer timeout carried as "environmental" since build phase 3.3 was Vite binding IPv6-only against an IPv4 probe, and behind it the e2e mock backend's Guard-tier response had not matched the classifier's schema since build phase 3.0, so NO browser test in this repository had run green for five phases. Gates: premise gate 24, vitest 147, Playwright 19 executed cases, typecheck clean, production build succeeds, doc drift clean. A post-merge visual pass on `develop` then found two more major defects, both position defects no check in this repository can see, since every frontend check asserts what is on screen and never where it is: the whole application rendered in a 720px strip, and the provenance spine's segments drifted out of register with the claims they describe. Both fixed with mutation-tested guards as commit `e08c656`. Full account: `tracker/phase_4.8.md`. Build phase 4.9 (the answer-screen and chrome fidelity gaps against the approved prototype, plus the account menu) is COMPLETE, merged as PR #44 on 2026-08-14. Four of its nine gaps were found by screenshotting the running app beside the prototype in the same states, a comparison never run before in this repository, which also surfaced a live bug in the same pass. Three review rounds ran and ALL THREE returned FAIL, filing 47 findings: the adversary found product-level lies (raw cost figures rendered on screen under a positive trust verdict), the judge found the phase's own GATE could not fail (deleting the sources count badge left it green, because the assertion read the whole disclosure and a source id contained the digit), and the re-review found that three of the four critical fixes were themselves wrong. All four criticals, all five re-review majors and both gate defects are closed; every remaining finding carries a named owner in `tracker/phase_4.9.md`. A design-system contrast and focus-nesting pass followed as PR #45: an axe sweep over all 21 cards found seven violations, four of them one token, fixed at source rather than worked around per call site for a fourth time, and a permanent gate now holds the design system to the same bar as the app. Build phase 4.10 (the anonymous run path and the server-side guest allowance) was then taken up out of order, pulled ahead of 4.2 through 4.7 by product-owner decision, since until it existed nobody could use the product without creating an account first. Build phase 4.10 (the anonymous run path and the server-side guest allowance) merged as PR #46 on 2026-08-15, split out of build phase 6.0 and pulled ahead of 4.2 to 4.7 by product-owner directive: a visitor with no account can now complete five real cited runs, counted server-side and bounded three ways. Seven review rounds, four FAIL, 30 findings and five criticals, every critical a defect in the phase's own design or its premise gate rather than in a builder's code. With it the out-of-order insertion that ran 4.8, 4.9 and 4.10 ahead of the locked order is finished. Build phase 4.2 (the CLI adapter, `system3-cli`, command `s3`) merged as PR #47 on 2026-08-16, resuming Section 25's order: six review rounds, 56 findings, five critical, all closed and independently probed. Its transferable result is a process finding rather than a code one, and it is logged in `DECISIONS.md`: five of the six rounds found their worst defect inside the previous round's fix, caused every time by parallel agents editing one file in one round, and round 5 run as a single serial agent immediately found a site five parallel rounds had missed. Full account: `tracker/phase_4.2.md`. Build phase 4.3, the GraphQL surface via Strawberry, merged as PR #48 on 2026-08-17 after SIX independent review rounds: a typed request/response surface wrapping the one core rather than exposing the seven tools, registered accounts only. Two criticals were found and closed in the same decision (whether an exception is safe to disclose), both caused by inferring that from a proxy rather than checking the message text; fourteen vacuous gate arms were found across the phase. It converged only once given an explicit merge bar. Then, 2026-08-18, the HARNESS changed rather than the product, merged as PR #49 and PR #50, prompted by the product owner's observation that a build day runs long and still does not land well: the review loop is now capped at two rounds with escalation instead of a third, fixes touching one file go to a single serial agent, a finding inside an earlier fix stops the phase mid-round, and `tracker/preflight.py` probes each transport before any expensive dispatch. Plan.md Phase 8 was opened the same day for the NCBI infrastructure migration. Full account: this document's Revision history for 2026-08-18. Build phase 4.4, the KGX export, merged as PR #51 on 2026-08-19. It was the first phase to run under the capped review loop, and it ran THREE rounds: the cap held, the third was authorized by the product owner through the escalation the cap exists to force. Its critical was a well-formed export holding none of the answer, living inside the premise gate's own stated blind spot. Build phase 4.5, personalization and session memory, merged as PR #52 on 2026-08-20, then closed out fully via PR #53 on 2026-08-21 after the judge and adversary rounds it shipped without were run against it, completing the delivery-surface work in Section 25's order apart from 4.6 and 4.7. Of the three questions its handoff said needed the product owner, two were already answered in the locked specification (Sections 14.3 and 14.5) and only the persona list was open. It closed three long-open findings, F-4.1-A-15, F-4.8-A-22 and F-4.2-03, the last open since build phase 4.2. It was also the FIRST phase in this build to merge with no judge round and no adversary round: stages 8 and 9 never ran. That gap was closed as PR #53 on 2026-08-21, and the result is this repository's clearest evidence for why the maker-checker split is not optional. Two rounds run with separate briefs and separate contexts converged INDEPENDENTLY on the same three worst defects, and both criticals were reachable in the shipped product: session memory could defeat the deterministic unresolved-entity refusal, so a question naming a gene that does not resolve was answered about a remembered gene instead, grounded, cited, and shipped as outcome `answer` with no disclosure; and every guest shared one ownership identity, since `user_id` is NULL for a caller with no account, so any anonymous caller could read and overwrite any other guest's session memory. The gate was the third finding: it ran `1 failed, 1 passed, 14 skipped in 0.06s`, so most of it never ran, and four arms were rebuilt rather than repaired. One finding the round filed was WITHDRAWN the following day and is recorded rather than removed: a claimed budget regression turned out to be an artifact of the measuring instrument. Build phase 4.6, feedback capture, MERGED as PR #54 on 2026-08-21: interaction capture into `interactions`, a feedback endpoint and its wired frontend, the weekly review ritual and few-shot promotion, closing F-2.0-04 and F-2.0-10 and taking F-4.10-A-14 and F-4.1-A-15 with it. It took THREE review rounds against a two-round budget, the third authorised by the product owner through the escalation the cap exists to force, and produced thirty findings including two criticals that both defeated the daily caps this phase existed to make work. Build phase 4.11, the read-only HTTPS graph query service, MERGED as PR #55 on 2026-08-22, taken ahead of 4.7 by product-owner decision on 2026-08-21 because 4.6 had measured the hand-opened SSH tunnel failing twice in one week. THE TUNNEL IS GONE: 59 of 59 live premise arms pass against the deployed service with none open. It ran FOUR review rounds against a two-round cap, 26 findings and three Rule 4 stops, each escalated; what justified continuing was that severity fell monotonically each round, from a control that did not bound the unauthenticated path, to a bound whose own bookkeeping was unbounded, to a diagnostic that under-reported. Its most transferable result is that the lead wrote THREE vacuous gate arms, all caught by mutation and none by reading, one of them vacuous twice while quoting the lesson against vacuous arms in its own docstring. Build phase 4.7, competency-question routing, MERGED as PR #56 on 2026-08-23. It is where `think_node` stopped being a build-phase-2.0 stub: real query-shape classification into Section 17's five shapes, real entity resolution by exact-ID-first order followed by live-confirmed model extraction, the seeded few-shot pool loaded once into the stable prefix, and the removal of the capitalized-token gene guess with no fallback. It CLOSES F-2.0-15, the finding that survived twelve phases with an owner field reading "unassigned", verified end to end rather than by inspection: all seven must-pass moat questions route correctly and the four previously refused with "I could not identify that gene" no longer mistake a database name mentioned in passing for a gene symbol. It also closes F-4.6-A-08, F-3.1-41 and F-3.1-42, the last two dissolved rather than decided. WHAT IT COST: four review rounds against a two-round cap, one Rule 4 stop, and three separate product-owner escalations. THE MOST TRANSFERABLE RESULT IS NOT THE FEATURE: the phase produced THREE separate vacuous-gate-arm findings, all written by the lead, in a file whose own docstring quotes build phase 4.11's lesson against writing them, and one of them survived its own repair because the populate-check verified a correlate of the property instead of the property. Reading an assertion has now been demonstrated three times not to be a method for validating it, so the durable output is a permanent offline mutation harness that breaks each control and asserts the arm goes red, making vacuity a build failure rather than a review finding. TWO CRITICALS MERGE OPEN, each on a dedicated branch with a trigger and both hard blockers on build phase 4.12 before any public URL: a crafted question can still steer which gene is looked up, which is Guard-tier classification failing and therefore build phase 3.0's control, and a retired gene name is answered as its replacement undisclosed, which was established as PRE-EXISTING rather than counted against this phase. Both of those criticals CLOSED on 2026-08-24, on their own branches with triggers exactly as filed: PR #59 (`fix/a02-discontinued-gene-record`) and PR #60 (`fix/a01-injection-guardrail`), which also takes F-4.7-A-05's `guardrail/classifier.py` half. The F-4.7-A-01 fix is DETERMINISTIC and sits in `guardrail/prefilter.py` ahead of any model call rather than in the Guard prompt, because that prompt already carried the abstract rule and admitted the payload 6 of 6 anyway; Guard-tier admission fell from the adversary's measured 12/18 to 0/18. Two transferable results, both in `LEARNINGS.md`: a ticket's stated premise is an assembly step and must be verified before the fix is written (the board and the adversary report agreed with each other and were both wrong about where the F-4.7-A-02 fix goes), and a mutation that finds nothing is evidence about the gate rather than proof the control is unnecessary. NEXT: build phase 4.12, the demo deployment, now unblocked apart from the security scan it already owns and the product owner's own Railway provisioning. |
| Phase 7: iteration and new information | Not started |

Decisions logged: 400 (DECISIONS.md). Deliverables produced: the Phase 1 synthesis, the evaluation playbook, the PRD (locked), the verified API capability sheet, the technical specification (locked), and the strategic memo. The dated change log is in Revision history at the end of this document.

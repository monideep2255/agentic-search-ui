...
| Next | What | Gated on |
|---|---|---|
| 1 | The product owner's verdict on `testing/UI_feedback.md`'s workflows W1 to W9 | Nothing. It is their turn |
...
WHAT STAYS AHEAD OF ALL OF THIS: the disease-name defect recorded in `testing/UI_feedback.md`. Disease nodes carry the source vocabulary in `name`, so a readable answer was never expressible from Layer 1, and `ncbi_efetch` already runs in that same query and already reaches MedGen. It is the single highest-value change available and it lives in this repository.

...
| Phase 6: build (bossman execution) | In progress. Step 6.1 (prototype) COMPLETE. Step 6.3 (build v1) has merged build phases 3.0 through 3.5 and 4.0 through 4.12. THE PRODUCT IS DEPLOYED AND ANSWERING as of 2026-08-24 (PR #61), live at https://search-agent-web-production.up.railway.app with CD watching `develop`. BUILD PHASE 4.16, the seven UI defects the live demo surfaced, MERGED as PR #63 on 2026-08-25 and is LIVE on the demo, verified on the deployed product rather than asserted. BUILD PHASE 4.14, CONTINUOUS INTEGRATION, is COMPLETE on its branch and awaiting review as PR #68, with CI FULLY GREEN on run 7. It is the first pull request CI has ever run on here, and it found FOUR pre-existing defects no gate, premise test or review round in this repository could see: `pip install -e .` broken for the life of the project so the declared console scripts were uninstallable; ~30 environment values the suite needs that a clean machine lacks; a build phase 4.7 assertion that called a healthy arm vacuous depending on hardware; and 25 tests that could never run in CI, hidden behind a green `4019 passed`. It also took THREE review rounds and hit the Rule 4 stop, because the premise gate was defeated twice by text that read correctly and executed nothing, the second time by `:;#ruff check`, which neutralised eight of ten gates with all 97 tests green. The product owner's decision was to change the approach rather than patch it a fifth time, so the shell left the workflow entirely: every gate's command is a checked-in script and every gate step's `run:` is exactly one token. ONE FINDING MERGES OPEN and needs a product-owner decision: nothing makes these gates merge-blocking, because branch protection needs GitHub Pro or a public repository. BUILD PHASE 4.13, DURABLE CROSS-RELOAD SEARCH HISTORY, MERGED as PR #69 on 2026-08-27, with all four CI gates green on the pull request, which is CI's first outing on new work since 4.14 built it. A person's past questions now survive closing the browser. It cost THREE review rounds against a two-round budget, the third authorised by the product owner, plus a fourth blocking finding from the final verifier, and the review loop's STOP CONDITION FIRED FOR THE FIRST TIME since it was written. The transferable result is not the feature: TWO defects shipped because a change made an UNSTATED INVARIANT false, and both survived review because a comment asserted the invariant that SURVIVED, which reads as diligence and functions as camouflage. Twice in one phase a reviewer's severity was trustworthy while its SCOPE was not: one finding filed against one endpoint was six routes wide, which turned a patch into a category fix closing five gaps that predated the phase. THREE THINGS MERGE OPEN, all product-owner decisions: a reload still signs an account out, a shared browser can carry one person's guest searches to whoever signs up next, and three minor latent findings carry owners. BUILD PHASE 4.15, THE TWO-APP RELEASE FLOW, MERGED as PR #71 at the phase merge on 2026-08-28, all four CI gates green, then closed by the release-permission follow-up PR #74. There are now two independent apps, a develop app on the `develop` branch and a production app on the new `production` branch, in two SEPARATE Railway projects, each with its own Postgres, Redis and signing key; `develop` remains the default branch. Cutting `release/<version>` and merging it into `production` deploys the production app and cuts a release: version from the Conventional Commit subjects, a CHANGELOG.md section, an annotated tag, a GitHub Release, and a back-merge pull request. TWO PROJECTS RATHER THAN TWO ENVIRONMENTS because a Railway service's git source is SERVICE-level, measured after pointing one service at a branch for one environment moved BOTH (F-4.15-03); the phase was designed the other way and reversed the same day. It cost FOUR review rounds against a two-round budget, 44 findings, and the Rule 4 stop fired TWICE. THE TRANSFERABLE RESULT IS NOT THE FEATURE AND NOT A BUG: four defects were a confident sentence describing a check that was not there, including a mutation harness that claimed full coverage twice, the second time inside the fix for the first, and a board cell that made the same claim a third time while the second was being fixed. The fix was DELETING the completeness claims rather than correcting them again. An adversary round found the critical nothing else could: the develop web app was live, answered 200, and could reach no API, because `VITE_API_BASE_URL` is compiled in at build time and no arm had ever fetched a web URL. THREE THINGS MERGE OPEN: CI stays advisory, the back-merge pull request runs no CI because the built-in token cannot raise workflow events, and P5 closes at the first release rather than at this merge. Next: the FIRST RELEASE, which is the only end-to-end proof the promotion path works, then 5.0 and 5.1. BUILD PHASE 5.0, OBSERVABILITY, IS COMPLETE ON `phase/5.0-observability` AND NOT YET MERGED, with the pull request the next step. It delivers tech spec Section 20 in full as THREE SEPARATE RECORDS so no single outage blinds the picture: LangSmith per-run tracing joined on `trace_id`, live and with account PII proven absent by reading traces back OUT of the service rather than by asserting it locally; PostHog behavioural analytics, aggregates only; and the append-only JSONL tool-call audit log, one line per Layer 1, 2 and 3 access with its authorization. Verified on the branch: the Python suite `4371 passed, 171 skipped, 1 xfailed, 0 failed`, the phase premise gate 8 passed 1 skipped, the observability suite 213 passed 1 skipped, `ruff check` clean over the whole repository, and doc drift 0 stale 0 structural. WHAT IT COST: one judge round, one adversary round and FIVE fix-and-verify rounds on ONE control, three Rule 4 stops, and a design reversal authorized by the product owner. Four consecutive rounds each shipped a fix defeated by an input of the SAME family, something upstream consuming the credential before the check saw it; the reversal that worked was to stop scanning text and BOUND what may enter the sink, which is `attack-the-constraint` applied to a check. THE MOST TRANSFERABLE RESULT IS A CRITIQUE OF THE LEAD'S OWN DECOMPOSITION rather than of any line of code: four rounds hardened the LOCAL audit sink's error field while the identical string went OFF-BOX to LangSmith with no control at all, found independently by both the judge and the adversary and by neither the lead nor any fix agent, because every round was scoped from the file the previous round had been editing rather than from where credentials actually flow. FOUR THINGS ARE CARRIED FORWARD WITH NAMED OWNERS rather than dropped: the analytics event is awaited inline on the query path with the patch written and not applied, an ordinary string under an ordinary key still reaches LangSmith unredacted, the old PostHog personal key still needs revoking by hand, and roughly twenty minor adversary findings are recorded. Per-phase status: `tracker/BOARD.md` |
| Phase 7: iteration and new information | Not started |

Decisions logged: 444 (DECISIONS.md).

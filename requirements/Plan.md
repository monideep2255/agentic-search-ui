...
Decisions logged: 605 (DECISIONS.md).
...
2026-09-22 NIGHT, LATEST, THE COORDINATE RANGE SHIPPED: A CHROMOSOME WINDOW IS ANSWERED WITH THE GENES UNDER IT AND THE RECORDS THAT OVERLAP IT. UI fix loop, so no build phase and no pull request. Commit `66b3811` on develop, verified live, awaiting the product owner's retest; production unchanged on `v0.2.0`.

...
> BUILD PHASE 4.11, THE READ-ONLY HTTPS GRAPH QUERY SERVICE, IS MERGED as PR #55 on 2026-08-22. `develop` is clean and pushed at 13f6e81, the phase branch is deleted, both builder worktrees are removed, and nothing is in flight. THE NEXT ACTION IS TO OPEN BUILD PHASE 4.7, competency-question routing. THE SSH TUNNEL IS GONE: 59 of 59 live premise arms pass against the deployed service with no tunnel open anywhere, and `tracker/preflight.py` now reports `graph ok, HTTPS query service` instead of a refused socket. That is the whole point of the phase, and it is measured rather than asserted. WHAT IT COST, and the reason to read `tracker/phase_4.11.md` before opening any phase with a premise gate: FOUR review rounds against a two-round cap, 26 findings, and THREE separate Rule 4 stops, each escalated to the product owner rather than decided by the lead. The findings changed in kind each round, which is what justified continuing: round 1 found two reachable majors in the original code, round 2's review found an unbounded resource inside round 1's fix, round 3's found a diagnostic regression and a CPU cost inside round 2's fix. Severity fell monotonically, and THAT is the stopping rule that actually applied, not the round number: when the newest round's worst finding degrades a diagnostic rather than breaching a control, the marginal round has stopped buying safety. THE SINGLE MOST TRANSFERABLE RESULT: the lead wrote THREE VACUOUS GATE ARMS in this phase, and all three were caught by a mutation run and NONE by reading. One of them was vacuous twice, in two different ways, while quoting the lesson against vacuous arms in its own docstring. Knowing a failure mode by name does not prevent committing it; running the mutation does. The durable fix that came out of it is the populate-check: an arm that cannot distinguish "the bound held" from "nothing happened" is not a bound arm, and every bound arm now asserts it produced the state it is about to measure.
...
BUILD PHASE 4.11, THE READ-ONLY HTTPS GRAPH QUERY SERVICE, IS MERGED as PR #55 on 2026-08-22. `develop` is clean and pushed at 13f6e81, the phase branch is deleted, both builder worktrees are removed, and nothing is in flight. The service is LIVE and stays running between phases, deliberately, because build phase 4.7's own premise gate depends on it and stopping it would re-create the on-demand fragility 4.11 existed to delete.

WHAT SHIPPED, in one line each:

- A single authenticated HTTPS endpoint on the Hetzner box in front of the AGE graph, behind Caddy with a Let's Encrypt certificate, running as a systemd unit under its own non-login user.
- The client-side transport swap inside `graph_connection.execute_cypher`, dispatching on `GRAPH_QUERY_URL`, with `cypher_query`'s schema and all three of its call sites unchanged. That is Decision D's two-way door, demonstrated rather than claimed.
- Server-side re-validation that REUSES this repository's own `validate_cypher` and `execute_cypher`, copied to the box by the deploy script and asserted byte-identical by `check_drift.sh`, so the two validators cannot drift.
- Three budget controls per `.claude/rules/tool-call-budgets.md`: a hard row limit, a per-call timeout matching Section 6.1's 30 seconds, and rate limits per caller AND per source.
- The deletion of the hand-opened SSH tunnel that had gated every live test since build phase 2.1.

THE DANGLING REFERENCE IS CLOSED. Section 24 said `GRAPH_QUERY_TOKEN` is populated "when the service is built (Section 25 build order)" and Section 25 never assigned it to a phase. It has one now.

WHAT THE REVIEW COST, and it is the most transferable thing this phase produced. FOUR rounds against a two-round cap, 26 findings, three Rule 4 stops, each escalated.

- THREE VACUOUS ARMS, all written by the lead, all caught by mutation and NONE by reading. The worst was vacuous TWICE in two different ways: first it asserted a ceiling of 10000 while producing 256 keys, then, rewritten, it collapsed 250 requests into one source key because `TestClient`'s peer address is the literal string `testclient` and failed the trusted-proxy check. It quoted the lesson against vacuous arms in its own docstring while doing it.
- THE DURABLE FIX IS THE POPULATE-CHECK, and it generalises past this phase: an arm must assert it produced the state it is about to measure, because otherwise every way the setup can silently fail reads as the control working perfectly. The one arm that carried that self-check from the start was never vacuous.
- ROUND 4 WAS RUN GATE FIRST by product-owner decision, arms written and watched failing before the fixes. In one round that ordering caught the real defect, caught TWO WRONG PREMISES IN THE LEAD'S OWN ARMS, and caught a tension introduced minutes earlier. Two of the three findings it set out to fix turned out to be GATE GAPS OVER ALREADY-CORRECT CODE, so acting on the report alone would have meant changing working code.
- A WRONG BLOCKER WAS REPORTED TO THE PRODUCT OWNER and an infrastructure credential requested for a firewall that does not exist. This harness renders "nothing listening" and "filtered" identically, as an 8 second timeout, and the control probe that made it feel safe only tested destinations that HAD listeners. A control that can only come back green is not a control. The credential was never used and was revoked.

Full record: `tracker/phase_4.11.md` (26 findings), `tracker/phase_4.11_judge_report.md`, `_adversary_report.md`, `_reverify_report.md`, `_reverify_r3_report.md`.

CARRIED FORWARD, and it is small: the deployed Caddyfile does not overwrite `X-Forwarded-For`, so the rightmost-element rule in `client_source` is the single safeguard rather than one of two. The code is correct and arm P20 pins it; the docstring that claimed two layers has been corrected. Build phase 4.12 owns adding the directive.

NEXT: OPEN BUILD PHASE 4.7, COMPETENCY-QUESTION ROUTING. Settled 2026-08-22 and logged in `DECISIONS.md`. The lead's first instinct was 4.12 and it was reversed on a concrete argument: build phase 4.6 ships interaction capture, so every question now writes a row recording what was asked and how well it was answered, and `query_class` is hardcoded to `"lookup"` on every query because `think_node` is a stub. Deploying first means collecting real questions into rows that are known-wrong in exactly the dimension anyone would want to analyse, and then discarding them. 4.12 loses little by waiting; its dependency is met and stays met.

THEN build phase 4.12, the demo deployment on Railway. Its dependency is now met, since it was waiting on exactly this phase: it cuts Layer 1 over from `GRAPH_PG_*` to `GRAPH_QUERY_URL` plus `GRAPH_QUERY_TOKEN`, which now exist and are live. THE SECURITY SCAN IS A HARD GATE INSIDE 4.12 and runs BEFORE any public URL exists, because its trigger is exposure. Build phase 4.11 deliberately did NOT run it, and that judgment is logged: what 4.11 exposes is one credential-gated endpoint serving a read-only role over already-public NCBI content, with no user data reachable from it, and the phase's adversary was briefed to attack that judgment specifically and found no path.
WHAT 4.7 IS: F-2.0-15's formal home, and where `think_node` stops being a stub. `query_class` is still hardcoded to `"lookup"` on every query, so every `interactions` row build phase 4.6 writes carries that known-wrong value, and `plan_node` compensates by scanning for a capitalised token and guessing it is a gene symbol, which breaks four of the seven must-pass moat questions. That is the product's worst known quality gap. Its verification is now cheaper than it has ever been, because the graph is reachable without anyone opening an SSH session.

WHAT 4.7 MUST READ BEFORE IT WRITES ANYTHING:

- Two open findings are deliberately routed to it and neither is a bug: the stopword versus gene ambiguity (should acronym-shaped symbols like `ADHD` stay stopword-blocked) and lowercase gene mentions. Both were left open on the reasoning that the stopword list is the very heuristic 4.7 replaces, so deciding them earlier meant designing twice. Both are product decisions, not implementation choices.
- Build phase 4.6 left it a versioned few-shot pool file that nothing reads yet. That is the correct seam rather than an oversight, and 4.7 is the phase that first reads it at process start, which is also where the concurrent-promotion corruption finding is owned.
- EVERY GATE ARM IT WRITES GETS A POPULATE-CHECK FROM THE FIRST LINE. Build phase 4.11 wrote three vacuous arms and all three were caught by mutation rather than by reading. An arm that cannot distinguish the control holding from nothing having happened is not an arm.

WHAT TO READ FIRST: `LEARNINGS.md`'s three newest entries, all from this phase. One is about port probes and why any assertion that a port is closed, written from inside this harness, has a defect by default. One is about agent mortality, four deaths in one phase, and why durability of partial output is a property of the BRIEF rather than of the agent. One is about `2>/dev/null` inside an ssh-quoted string tripping the deletion hook.
...
Final gates: 3966 Python tests (3188 passing, the same six live-network-gated cases carried since build phase 4.0), the GraphQL package alone 206 to 300 tests, 181 frontend, ruff clean, doc drift 0 stale 0 structural.

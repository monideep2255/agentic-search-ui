...
> Build phase 4.5, personalization and session memory, is MERGED as PR #52 (superseded by PR #53, its independent review). All twelve tickets: the `SessionMemorySummary` contract, a token cap counted with the receiving tier's real tokenizer, compaction in Section 14.3's order, injection into Think and Plan only, per-session persistence (alembic 0007, verified up and down against the live database), `audience_depth` finally reaching synthesis after riding the contract unused since build phase 1.0, and 32 curated deceased scientists wired to all four surfaces from one source. Closes F-4.1-A-15, F-4.8-A-22 and F-4.2-03, the last open since build phase 4.2. Of the three questions the handoff said needed the product owner, TWO WERE ALREADY ANSWERED in Sections 14.3 and 14.5. WHAT IT COST, and the reason to read `tracker/phase_4.5.md` before building anything with a premise gate: TWO CRITICALS, both in the lead's own code, both found after the work looked finished. F-4.5-06, caught by the gate: a `clinical_brief` directive told the model not to print identifiers, the grounding pass substring-matches claims against finding values, and a Layer 1 finding's value IS the identifier, so a PRESENTATION instruction made that depth refuse outright. F-4.5-09, caught by nothing: the read side was complete and correct and NOTHING EVER WROTE A SUMMARY, so the feature was inert while eight gate arms passed, because every one of them hands the memory in. That is build phase 4.4's lesson in a new costume, committed while quoting it. WHEN A TEST SUPPLIES THE THING UNDER TEST, IT CANNOT TELL YOU THE THING EXISTS. NEXT: it merged with NO judge round and NO adversary round, the first phase in this build to do so, and closing that is the next action before build phase 4.6.
>
> Previously, and still worth reading once: build phase 4.3, the GraphQL surface via Strawberry, is DONE, merged as PR #48 on 2026-08-17. A typed request/response surface at `/graphql` that WRAPS the one agent core rather than exposing the seven tools, registered accounts only, no live stream. It took SIX independent review rounds, every one of which found its worst defect inside the previous round's fix, and two of those were criticals in the same decision: whether an exception is safe to show a caller. Both came from using a PROXY for that (the package a class was declared in, then its class family, then the phase the error came from) instead of checking the text. The fourth attempt checks the text itself, against shapes captured from the installed library. Fourteen vacuous gate arms were found across the phase, six written by the lead. The phase only converged once it was given a MERGE BAR (a critical or a reachable major blocks; minors and latent findings are tracked), which it had run five rounds without. It pointed at build phase 4.4, the KGX export, which has since shipped.

...
> BUILD PHASE 4.12, THE DEMO DEPLOYMENT, IS OPEN on `phase/4.12-demo-deploy` and its CODE-SIDE WORK IS DONE AND DEPLOYED. The two criticals build phase 4.7 had left open were closed first, on 2026-08-24: F-4.7-A-02 by PR #59, and F-4.7-A-01 by PR #60, which also took F-4.7-A-05's guardrail half. Build phase 4.7 itself merged earlier, as PR #56. THE NEXT ACTION IS RAILWAY PROVISIONING, and it needs the product owner's hands before anything else can move: `railway login` opens a browser, so it cannot run from this harness. WHAT SHIPPED IN 4.12 SO FAR: the Caddyfile now REPLACES `X-Forwarded-For` instead of appending, closing F-4.11-RV-02 on the exact condition build phase 4.11 set for it; `check_drift.sh` now covers the deployed Caddyfile, which it did not, while that file's own header warned a hand-edit would be invisible; and the eight premise gates that decided liveness by socket-probing the SSH tunnel build phase 4.11 DELETED now dispatch on `GRAPH_QUERY_URL` through one shared implementation. MEASURED, NOT ASSERTED: the suite is `3929 passed, 158 skipped, 1 xfailed, ZERO FAILED`, and THE STANDING SIX-FAILURE BASELINE IS GONE. It was never six broken tests: all six were in `test_citation_trust_full_premise.py`, all six pass under `RUN_PREMISE_GATE=1`, and the file FAILED where it should have SKIPPED because its `live_only` mark gated on a model key EXISTING rather than on outbound HTTP being PERMITTED. Deployed and verified rather than reported: `check_drift.sh` 6 of 6 ok, `caddy validate` Valid configuration, both units active, and the live service gate 59 of 59. THE SECURITY SCAN IS DROPPED, not deferred: product-owner decision 2026-08-24, logged in `DECISIONS.md`, on the grounds that this is a throwaway prototype and the same call would be the opposite in production. Exposure is NO LONGER a gate on 4.12. THE MOST TRANSFERABLE RESULT: this phase CAUSED a regression while fixing the thing it set out to fix, and the fix is the category rather than the instance. Correcting the tunnel probe made it answer truthfully, so arms that had been silently skipping began to RUN offline and FAIL against conftest's block, one file going from a clean skip to `6 failed in 169.73s`. "The dependency answers" and "this process may talk to it" are TWO facts, a gate needs both, and neither is a proxy for the other.
...
> Build phase 4.12 is open and its code-side work is done, deployed and verified. Railway provisioning is next and is the product owner's: the CLI is installed (4.30.5) and UNAUTHENTICATED, and `railway login` needs a browser. The security scan is DROPPED for the prototype by product-owner decision, so nothing else gates a public URL. What remains after provisioning is the variable mapping in Section 24's table, the GitHub integration watching `develop`, and setting `GRAPH_QUERY_URL` plus `GRAPH_QUERY_TOKEN` while leaving `GRAPH_PG_*` empty, which needs no code because `execute_cypher` has dispatched on that variable since build phase 4.11.
>
> WHAT TO READ BEFORE OPENING ANY PHASE WITH A PREMISE GATE, because this is the phase's real output. THREE separate vacuous-gate-arm findings, all written by the lead. First, three of the flagship arm's four cases asserted a token absent from the question, so they could not fail; found by a BUILDER reading the gate it was handed, not by the lead and not by a review round. Second, the REPAIR for that survived its own fix: the populate-check proved the token was in the QUESTION and said nothing about the CODOMAIN of the value asserted on, which is the same safety-by-proxy shape build phase 4.3 shipped twice three rounds apart, and it triggered a RULE 4 STOP. Third, a budget arm that computed `max(b, C)` and then asserted `max(b, C) >= C`, a mathematical identity, inert on every line. THE GENERAL FORM: a negative assertion must prove the thing it denies was available to be found, and an arm must read a value capable of taking the failing value. Neither is checkable by reading. The durable fix is the mutation harness now in the tree.
>
> ALSO FOUND AND FIXED HERE, worth knowing because it is the failure class this whole product exists to avoid: a question naming a non-human organism ("the mouse gene Tp53") was answered about the HUMAN gene with 13 real citations. The taxon was hardcoded, the resolver had accepted a taxon since build phase 3.1, and the extraction prompt explicitly told the model NOT to extract organism names, so the one field that could carry the fix was designed out. This phase CAUSED it, verified rather than assumed: the retired regex could not match a mixed-case symbol, so the old code refused honestly, and the new extractor turned that refusal into a confident wrong answer.
...
Twelve build phases are done, all twelve merged into `develop` (renamed from `main` at Step 6.2). The first six complete the Step 6.1 prototype group; 3.0 through 3.5 are all six of the Step 6.3 tool-and-trust v1 phases, and with 3.4's merge every one of them is now closed:
...
Step 6.2 moved on 2026-08-03 to run AFTER the 3.x tool phases rather than between 2.2 and 3.0, because its own written reasoning names 3.x as the code its security scan most exists for, and because reconciling the frozen documents after the tool phases is better input than reconciling before them. With build phase 3.4's merge, that condition was met, and Step 6.2 ran and closed the same day, 2026-08-10 (see "Step 6.2, done" below). Its security scan stays separately PAUSED INDEFINITELY on cost, with one condition that turns it back on: exposure. First contact with a real user, a deploy, or a public URL triggers it, whichever comes first. Step 6.3 continues at build phase 4.0.
...
READ THE BOARD'S FLAG COUNT AS TWO NUMBERS, not one. Its flags table is a LEDGER, not a queue: a closed finding keeps its row so the trail survives, so the total only ever grows and is not a backlog. `render_board.py` reports the split, currently `flags: 57 open, 26 closed`, after the single number was read as 83 outstanding problems on 2026-08-23 when 26 of them were already closed. Of the open ones, most are CONDITIONAL, worded "whenever X is next touched": those are notes attached to code, not scheduled work, and they become work only if someone touches that code. The rows that are genuinely queued name a phase or a branch.
...
BUILD PHASE 4.12, THE DEMO DEPLOYMENT, IS OPEN on `phase/4.12-demo-deploy`, three commits in, pushed. Its code-side work is complete, deployed to the Hetzner box and verified there. THE NEXT ACTION IS RAILWAY PROVISIONING, which the product owner must start because `railway login` opens a browser.

WHAT SHIPPED, in one line each:

- Real query-shape classification in Think: one of Section 17's five shapes per query, from a Plan-tier call whose response is READ rather than discarded, schema-validated so a value outside the five is rejected and never coerced to a default.
- Real entity resolution in Section 17's exact-ID-first order: a deterministic, local, non-async pre-pass for verbatim CURIEs, rsIDs, PMIDs and accessions, then typed model extraction for whatever text remains, every gene span LIVE-CONFIRMED before it contributes a CURIE.
- The retirement of `plan_node`'s capitalized-token gene guess and its hand-maintained stopword list, with NO fallback, by product-owner decision.
- The seven must-pass moat questions seeded into a versioned few-shot pool, loaded once at process start into the stable, cache-hot prefix.
- An atomic, lock-serialized pool write, closing F-4.6-A-08 before the phase that first reads that file at startup could trip over it.
- A real `query_class` reaching `budget_for_step` and `interactions.query_class`, retiring the known-wrong value build phase 4.6 knowingly wrote on every row.

WHAT IT CLOSES: F-2.0-15, F-4.6-A-08, and F-3.1-41 and F-3.1-42, the last two DISSOLVED rather than decided, since both were questions about how to tune a list that no longer exists.

WHAT THE REVIEW COST: four rounds against a two-round cap, one Rule 4 stop, three product-owner escalations, 16 findings.

- THREE VACUOUS GATE ARMS, all written by the lead, in the file whose own docstring quotes build phase 4.11's lesson against them. One was found by a BUILDER reading the gate it was handed, before any review round ran. One survived its own repair and caused the Rule 4 stop.
- THE REPAIR THAT SURVIVED ITSELF is the most useful thing here. A populate-check was added to make vacuity mechanical; it proved the token appeared in the QUESTION and said nothing about the CODOMAIN of the value being asserted on, so the arm still could not fail. Verifying a correlate of the property instead of the property is the same shape build phase 4.3 shipped as a critical twice, three rounds apart.
- THE DURABLE FIX IS IN THE TREE: `tests/system_03_search_agent/core/test_cq_routing_mutation.py`, 43 offline cases over 11 of 13 arms, breaks each control and asserts the arm goes red. Vacuity is a BUILD FAILURE now. It states its own coverage, including the two arms it cannot mutate and why, and it recorded a false GREEN it produced in its own first draft rather than quietly fixing it.
- A CONFIDENT WRONG ANSWER WAS CAUSED AND FIXED HERE: "the mouse gene Tp53" resolved to human TP53 and answered with 13 real citations. The taxon was hardcoded, the resolver had accepted a taxon since build phase 3.1, and the extraction prompt told the model NOT to extract organism names, designing out the one field that could fix it.

BOTH OF THOSE CRITICALS ARE NOW CLOSED, 2026-08-24, each on the dedicated branch this phase filed it to. They were hard blockers on build phase 4.12 before any public URL, and they are not any more. Kept here rather than deleted, because the residual each one carried is what 4.12 actually needs to know:

- `fix/a01-injection-guardrail`. CLOSED by PR #60. The fix is DETERMINISTIC, in `guardrail/prefilter.py` ahead of any model call, not in the Guard prompt: that prompt already carried the abstract rule in almost the finding's own words and admitted the payload 6 of 6 anyway. Guard-tier admission for the three payloads fell from 12/18 to 0/18, and the payload is now refused at $0.00 with no model call reached. It also closed F-4.7-A-05's `guardrail/classifier.py` half with a per-request unforgeable tag. RESIDUAL, and 4.12 should not read this as the class being closed: it raises the floor, not the ceiling. A payload steering entity selection in pure ordinary English, carrying no underscore identifier and no forged processing header, still falls through to the Guard tier. The coordinate-versus-gene consistency check the adversary named as the other missing control is FILED, not built.
- `fix/a02-discontinued-gene-record`. CLOSED by PR #59. A withdrawn record contributes no CURIE and the refusal names the successor. THE TICKET'S OWN PREMISE WAS WRONG and 4.12 should take the lesson rather than the fix: the board and the adversary report agreed the fix was single-site because `status` and `currentid` were already in the response the resolver retrieves, and measured live, the `summary` action's allowlist stripped both, so a fix written on that premise would have refused nothing while looking correct. RESIDUAL: the Datasets leg is not covered because it cannot reach the defect, and two withdrawn symbols are pinned rather than enumerated.

WHAT 4.12 MUST KNOW BEFORE IT WRITES ANYTHING:

- Both branches above are MERGED, so neither gates this phase any more. The security scan still does: its trigger is exposure, and 4.12 is the first exposure.
- The deployed Caddyfile does not overwrite `X-Forwarded-For`, carried from build phase 4.11. 4.12 owns adding the directive.
- Every pre-4.11 premise gate still probes the deleted SSH tunnel's port to decide whether to run its live arms, so those arms skip while printing a skip reason that is FALSE. Seven files. Owned by 4.12.
- EVERY GATE ARM IT WRITES GETS A POPULATE-CHECK AND A MUTATION FROM THE FIRST LINE. Three phases running have now produced vacuous arms, and in this one the repair for a vacuous arm was itself vacuous. Copy the mutation harness pattern rather than trusting a read.

WHAT TO READ FIRST: `tracker/phase_4.7.md`'s Findings table and `LEARNINGS.md`'s newest entries. The transferable lessons are that a negative assertion must prove the thing it denies was available to be found, that an arm must read a value capable of taking the failing value, and that a claim about a control needs N runs and a reported rate because guardrail verdicts here vary run to run.
...
Build phase 4.12, the demo deployment, is OPEN and part-done, on `phase/4.12-demo-deploy`. The two fix branches below merged on 2026-08-24, PR #59 and PR #60, and their rows are kept struck through rather than deleted so the trail from build phase 4.7's adversary round to their closure stays readable, and so the residuals each one carried stay visible.

### What is left in build phase 4.12, in order

The first item is the product owner's and blocks the rest.

1. RAILWAY PROVISIONING, product owner. The CLI is installed (`railway 4.30.5`) and UNAUTHENTICATED (`railway whoami` returns "Unauthorized"). It cannot be done from this harness: `railway login` opens a browser. Product-owner direction 2026-08-24 is to drive Railway through its MCP server rather than the console.
2. WIRE THE RAILWAY MCP, after step 1. Railway's own documentation (https://docs.railway.com/ai/mcp-server) offers three shapes and they are NOT equivalent for this purpose. `railway setup agent` (local) exposes 50-plus tools on the full CLI session, which is full account access. `railway setup agent --remote --oauth` is the one to use: Railway's docs state the operator chooses which workspaces and projects the client may reach, and that tokens are short-lived and revocable, which is the credential scoping `.claude/rules/supply-chain-security.md` requires rather than an optional extra. Either shape exposes DESTRUCTIVE tools (`remove_service`, `delete_domain`, `remove_volume`, `remove_bucket`, `redeploy`), and Railway's own documentation says to review actions before approving them, so the enable-versus-trust gate applies: enabling is not trusting. There is no npm package to pin here, which removes the usual supply-chain question and replaces it with a scoping one.
3. MAP THE VARIABLE SETS to Section 24's table, one row per group, on `search-agent-api`.
4. CUT LAYER 1 OVER: set `GRAPH_QUERY_URL` and `GRAPH_QUERY_TOKEN`, leave `GRAPH_PG_*` empty. NO CODE CHANGE, because `execute_cypher` has dispatched on that variable since build phase 4.11 and `env.example` already documents both transports.
5. CONNECT THE GITHUB INTEGRATION watching `develop` only, per Section 24. Phase branches must never auto-deploy.
6. NOT A GATE ANY MORE: the security scan. Dropped 2026-08-24 by product-owner decision for the prototype, logged in `DECISIONS.md` with its condition attached, since the same call was stated as the opposite for production. The one thing that does not become less true because the code is throwaway: a public URL puts the live OpenRouter key, the NCBI key and the graph bearer token behind a surface strangers can drive, and the guest allowance is a real spend path. That is a COST exposure, and it is worth a per-user cap review before publishing rather than a scan.

The board still shows 4.12 as `todo` and that is CORRECT rather than stale: `render_board.py` refuses to move a phase out of `todo` while its refinement is `product_refine`, on the rule that work does not start until refinement is complete. Marking it `refined` is one word and it belongs to the product owner. Build phase 4.7 merged as PR #56 on 2026-08-23, and the section above carries its full account. With that merge, ALL SIX of Step 6.3's delivery surfaces are done, and so are personalization, feedback capture and competency-question routing. Section 25's 4.x group is finished apart from build phase 4.12, the board-level insertion that owns the demo deployment. This paragraph carried two contradictory counts and a stale "4.4 is next" table until 2026-08-23, from being appended to across four merges instead of rewritten, which is the same failure this file's own Step 4 rule was written to stop.

| Next up | Branch | What it delivers | Depends on |
|---------|--------|------------------|------------|
| 4.12 | `phase/4.12-demo-deployment` | The demo deployment on Railway, cutting Layer 1 over from `GRAPH_PG_*` to `GRAPH_QUERY_URL` plus `GRAPH_QUERY_TOKEN`. THIS IS NEXT | 4.11, merged 2026-08-22 |
| ~~`fix/a01-injection-guardrail`~~ | CLOSED, PR #60, 2026-08-24 | Guard-tier injection classification, Section 10.4. No longer a blocker. The fix is deterministic and sits in `guardrail/prefilter.py` ahead of the model, NOT in the Guard prompt. RESIDUAL, carried rather than dropped: it raises the floor, not the ceiling. A payload steering entity selection in pure ordinary English, carrying no underscore identifier and no forged header, still falls through to the Guard tier, and the coordinate-versus-gene consistency check the adversary named as the other missing control is FILED, not built | closed |
| ~~`fix/a02-discontinued-gene-record`~~ | CLOSED, PR #59, 2026-08-24 | A retired gene name answered as its replacement, undisclosed. No longer a blocker. RESIDUAL: the Datasets leg is not covered because it cannot reach the defect (it returns zero reports for a withdrawn symbol, probed live), and two withdrawn symbols are pinned rather than enumerated | closed |
...
- Every item explicitly tagged "Step 6.2" as owner in the Open items table below closed (PR #34, 14 items): four pure spec corrections (10 vs 11 concept labels, the broken `%s` parameter-mechanism claim, the two-shape per-step timeout budget, the `PER_USER_DAILY_QUERY_CAP` env var name) plus real fixes spanning a live safety gap (F-3.1-50's unbounded list truncation, live-reachable since build phase 3.4 wired `ncbi_efetch` into `act_node`), a new `write_seeking` guardrail category (F-3.0-01, also catching a second hand-maintained copy of the category set in the frontend that would have silently rejected the event), two Layer 3 citation schema widenings (F-3.3-J-06, F-3.3-A-05), a `pathogen_detection` status split (F-3.5-A-09), and a live-probed 14-value `overall_status` enum widen (F-3.5-A-12, live-confirmed via `GET /api/v2/stats/field/values`, 14 real values against the 12 previously estimated). F-3.4-A-06 scheduled as its own dedicated task rather than rushed: the naive one-line fix would have broken every real citation URL.
...
Final gates: 4088 Python tests (3188 passing, the same six live-network-gated cases carried since build phase 4.0), the GraphQL package alone 206 to 300 tests, 181 frontend, ruff clean, doc drift 0 stale 0 structural.
...
What shipped: `adapters/mcp/server.py`, an outbound-only MCP server exposing a single advertised tool, `ask_biomedical_question`. The tool folds the same core `run()` loop build phase 4.0 finalized (iterating `RunRegistry.subscribe` to the terminal event) into one JSON result, never a stream: no `think`, `plan`, or `tool_start` event ever reaches the caller, and the internal `token`/`tool_result`/`citation`/`trust_signal` events fold into a final `answer`, `citations`, `trust_signal`, `run_id` shape matching Section 13.2's locked schema exactly. Auth reuses the same bearer-JWT decode-then-lookup logic every other surface already uses, extracted into a small shared callable rather than duplicated; a missing, malformed, or invalid token fails before `create_run` is ever called, so no run and no budget is spent. `operator_mode` is hard-pinned false for this surface in code, so no MCP response can ever carry a cost field regardless of the authenticated account's allowlist status. `list_tools()` advertises exactly one tool; none of the seven internal tools (`cypher_query`, `ncbi_efetch`, `ncbi_dbsnp`, `pubtator_annotate`, `litvar2_lookup`, `pathogen_detection`, `clinicaltrials_search`) is ever separately reachable. New dependency: the official `mcp` Python SDK (`mcp>=2.0`), supply-chain checked and product-owner approved before any code (see `tracker/phase_4.1.md`'s pre-build source read).
...
Test counts at close: Python suite 2565 collected (2445 passing, 113 skipped, 1 xfailed, 6 failed on the same pre-existing live-network-opt-in-gated set every prior phase has carried, confirmed unrelated), this phase's own premise gate file fully green at 48 tests. `/verify` READY, `dev-standards` READY with 0 blocking issues, `eval-harness` determined not applicable in full (an outbound protocol adapter over the same already-graded core, reasoning documented in `tracker/phase_4.1.md`). Learnings-coverage verified by hand: `check_learnings_coverage.py`'s table-row parsing gap (the same one already flagged for build phase 3.3) means the script cannot see this phase's table-format findings, so its "nothing to cover" result was not trusted.

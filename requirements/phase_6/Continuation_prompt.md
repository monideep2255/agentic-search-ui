...
- BUILD PHASE 5.0, OBSERVABILITY, IS OPEN AND PAUSED mid-phase on `phase/5.0-observability`, 11 commits, tree clean, pushed. PAUSED 2026-08-29 by the product owner ahead of a session limit, NOT blocked on work: six of seven tickets are built, verified and committed, and the phase is blocked on ONE DECISION.
- THE NEXT ACTION IS TO ANSWER F-5.0-13 AND THEN RESUME. Read `tracker/phase_5.0.md`'s "Where this stops, and what the next session does first" section, which carries the three options and the lead's recommendation. Do NOT re-plan the phase and do NOT re-run the research; everything is on disk.
- WHY IT STOPPED RATHER THAN SHIPPED: Rule 4 fired. Half of F-5.0-13 sits inside this phase's own F-5.0-08 fix, and a finding located inside an earlier fix is the stop condition, so the lead escalated instead of patching. The finding itself: `record_tool_call` writes the `error` field RAW while `params` go through redaction, and the value scanner redacts a credential that IS a URL but not one EMBEDDED in a longer message, which is exactly the shape an exception string takes.
- WHAT LANDED, all verified against the LIVE services rather than a stub after the product owner provisioned credentials mid-phase: LangSmith tracing on both `run.py` sites, the append-only JSONL audit log at three TRANSPORT chokepoints, and PostHog analytics wired but deliberately sending nothing.
- THE DESIGN DECISION MOST LIKELY TO BE WRONGLY SIMPLIFIED BACK: the audit hook is at the TRANSPORT, never at `act_node`. Five production call sites reach a data layer without passing through `act_node`, and a live query proved it rather than a test: the first audit line written was `think_node`'s symbol resolution. The transport is also the only place Section 20.3's required HTTP status and latency exist, since both are discarded before a tool returns. Two premise arms now drive the real bypass callers by name.
- F-5.0-12 NEEDS THE PRODUCT OWNER TOO, and analytics send nothing until it is answered: the provisioned `POSTHOG_API_KEY` is a `phx_` PERSONAL API key, which is the wrong credential for the capture endpoint and an account-wide read-write key where an append-only one is needed. The correct `phc_` project token existed earlier in the shell environment, so this looks like the wrong one copied across.

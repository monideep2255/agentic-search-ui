...
BUILD PHASE 4.6, FEEDBACK CAPTURE, IS BUILT AND REVIEWED AND NOT YET MERGED. The branch `phase/4.6-feedback-capture` carries every ticket, three review rounds and their fixes. It has NOT been committed, pushed, or opened as a pull request at the time this line was written, so the entire phase lives in the working tree. Treat that as the first fact, not a footnote: nothing here is safe until `/ship` runs.
...
TWO DECISIONS ARE OPEN AND BELONG TO THE PRODUCT OWNER. Neither blocks the code, and both should be settled at merge rather than inherited silently by build phase 4.7:

- Durable cross-reload history. Section 25's row for 4.6 names three deliverables and history is not among them, but `frontend/src/stubs/registry.ts` names 4.6 as history's owner twice, in notes left by build phases 4.8 and 4.10. The lead followed the locked spec and did NOT build it. Either 4.6 widens, or those two registry notes are corrected to name the phase that really owns it. Filed as F-4.6-03.
- The locked specification contradicts itself about where `trace_id` is minted. Section 20.1 says the Guardrail step; Section 13.1 says the adapter generates it and it doubles as `run_id`, "the same identifier under two names". Both cannot hold. The shipped code follows 13.1 and has since build phase 1.2. Filed as F-4.6-07 for the Step 6.2-style reconciliation. A ticket written from 20.1 alone looks entirely reasonable and cost this phase a reverted ticket, so whoever reads 20.1 next needs this warning.

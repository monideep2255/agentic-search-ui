...
> BUILD PHASE 4.7, COMPETENCY-QUESTION ROUTING, IS MERGED as PR #56 on 2026-08-23. It is where `think_node` stopped being a build-phase-2.0 stub, and it CLOSES F-2.0-15, the finding that survived TWELVE phases and two weeks of green premise gates with an owner field reading "unassigned". All seven must-pass moat questions now route correctly against the real loop, and the four previously refused outright with "I could not identify that gene" no longer mistake a database name mentioned in passing for a gene symbol. THE NEXT ACTION IS TO OPEN BUILD PHASE 4.12, the demo deployment. MEASURED, NOT ASSERTED: live gate `16 passed in 370.86s` none skipped none open; suite `6 failed, 3813 passed, 146 skipped, 1 xfailed` against a baseline RE-MEASURED at this branch point; `ruff` clean; drift 0 stale 0 structural. WHAT IT COST, and the reason to read `tracker/phase_4.7.md` before opening any phase with a premise gate: FOUR review rounds against a two-round cap, ONE RULE 4 STOP, and THREE separate product-owner escalations. THE SINGLE MOST TRANSFERABLE RESULT IS NOT THE FEATURE: this phase produced THREE separate vacuous-gate-arm findings, every one written by the lead, in a file whose own docstring quotes build phase 4.11's lesson against writing them. Three of the flagship arm's four cases asserted a token that did not appear in the question at all. The REPAIR for that then SURVIVED ITS OWN FIX, because the populate-check added to make the defect mechanical proved the token was in the QUESTION and said nothing about the CODOMAIN of the value asserted on. Reading an assertion has now been shown three times in one phase not to be a method for validating it. THE DURABLE FIX IS A PERMANENT OFFLINE MUTATION HARNESS, `tests/system_03_search_agent/core/test_cq_routing_mutation.py`, 43 cases over 11 of 13 arms, which breaks each control and asserts the arm goes red: vacuity is now a BUILD FAILURE rather than a review finding. TWO CRITICALS MERGE OPEN, each on a dedicated branch with a trigger and BOTH HARD BLOCKERS ON BUILD PHASE 4.12 BEFORE ANY PUBLIC URL: `fix/a01-injection-guardrail` (a crafted plain-English parenthetical still steers which gene is looked up; the guardrail admitted it 6 of 6, so it is Guard-tier classification failing and therefore build phase 3.0's control) and `fix/a02-discontinued-gene-record` (a retired gene name answered as its replacement undisclosed, established PRE-EXISTING because the retired regex matched it too).
...
> Build phase 4.7, competency-question routing, is MERGED as PR #56 on 2026-08-23. It is F-2.0-15's real home: `think_node` now makes a real Plan-tier call whose response is READ, classifying every query into one of Section 17's five shapes, and resolves entities in Section 17's exact-ID-first order, a deterministic local pre-pass followed by typed model extraction that must be LIVE-CONFIRMED before it contributes a CURIE. `plan_node`'s capitalized-token gene guess and its hand-maintained stopword list are GONE with no fallback, by product-owner decision, which DISSOLVES F-3.1-41 and F-3.1-42 rather than deciding either. The seven must-pass moat questions seed a few-shot pool loaded once at process start into the stable, cache-hot prefix, and F-4.6-A-08 is closed with concurrency proven by execution: 8 OS processes by 6 appends, twice, 0 lost of 96 across 239,899 concurrent reads.
>
...
BUILD PHASE 4.7, COMPETENCY-QUESTION ROUTING, IS MERGED as PR #56 on 2026-08-23 on `phase/4.7-cq-routing`. Both builder worktrees are merged. THE NEXT ACTION IS TO OPEN BUILD PHASE 4.12, the demo deployment on Railway, whose dependency was met by build phase 4.11 and stays met.
...
| `fix/a01-injection-guardrail` | a branch, not a phase | Guard-tier injection classification, Section 10.4. A HARD BLOCKER on 4.12 before any public URL exists | build phase 3.0's control |
| `fix/a02-discontinued-gene-record` | a branch, not a phase | A retired gene name answered as its replacement, undisclosed. A HARD BLOCKER on 4.12 before any public URL exists | pre-existing, resolver-level |
...
Final gates: 4006 Python tests (3188 passing, the same six live-network-gated cases carried since build phase 4.0), the GraphQL package alone 206 to 300 tests, 181 frontend, ruff clean, doc drift 0 stale 0 structural.

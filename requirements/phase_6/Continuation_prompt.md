...
> Build phase 4.5, personalization and session memory, was, at the time this line was written, BUILT and OPEN AS PR #52, NOT MERGED. All twelve tickets are done: the `SessionMemorySummary` contract, a token cap counted with the receiving tier's real tokenizer, compaction in Section 14.3's order, injection into Think and Plan only, per-session persistence (alembic 0007, verified up and down against the live database), `audience_depth` finally reaching synthesis after riding the contract unused since build phase 1.0, and 32 curated deceased scientists wired to all four surfaces from one source. Closes F-4.1-A-15, F-4.8-A-22 and F-4.2-03, the last open since build phase 4.2. Of the three questions the handoff said needed the product owner, TWO WERE ALREADY ANSWERED in Sections 14.3 and 14.5; only the persona list was open, and both halves of it are logged in DECISIONS.md. WHAT THIS PHASE COST, and the reason to read `tracker/phase_4.5.md` before building anything with a premise gate: TWO CRITICALS, both in the lead's own code, both found after the work looked finished. F-4.5-06, caught by the gate on the first real implementation: a `clinical_brief` directive told the model not to print identifiers, the grounding pass substring-matches claims against finding values, and a Layer 1 finding's value IS the identifier, so a PRESENTATION instruction made that depth refuse outright. F-4.5-09, caught by nothing, found by asking what populates memory: the read side was complete and correct and NOTHING EVER WROTE A SUMMARY, so the feature was inert while eight gate arms passed, because every one of them hands the memory in on `RequestContext`. That is build phase 4.4's lesson in a new costume, committed while quoting it: there five of six cases passed an explicit edge-label list, here the injected fixture was the explicit list. WHEN A TEST SUPPLIES THE THING UNDER TEST, IT CANNOT TELL YOU THE THING EXISTS. NEXT: STAGES 8 AND 9 NEVER RAN. There was no judge round and no adversary round, and every fix and every test on this branch was written by the same agent that wrote the code. Given that two criticals surfaced after the phase looked complete, that gap is the first thing to close, and the change most needing it is the completeness repair in `write_node`, which sits on the most safety-critical path in the product and adds a second Synth call on an incomplete answer.

> Previously, and still worth reading once: build phase 4.4, the KGX export, is DONE. A query-scoped subgraph export: seed CURIEs, bounded hops, `nodes.tsv` plus `edges.tsv` plus a manifest, read out through the existing read-only Layer 1 path. Its scope was UNDECIDED at open, since Section 25 and the PRD scope it only negatively, and the product owner settled it. It also required amending the `file-protection` rule, which banned KGX exporters outright: the amendment splits by direction of data flow, so writing KGX into the graph is still Systems 1 and 2 and still banned here, while reading a scoped subgraph out is a delivery surface. THE PHASE'S CENTRAL RESULT, and the reason to read `tracker/phase_4.4.md` before building anything with a premise gate: the adversary found a CRITICAL that the gate could not see. The default invocation, with no edge-label list, spent its whole node budget on the highest-cardinality label and returned 500 Articles and ZERO of the twelve disease edges the gate itself pinned as ground truth, with a manifest certifying it had traversed all fourteen labels. Five of the gate's six cases passed an explicit single-label list, so none of them touched the default path. The gate's own coverage statement NAMED that omission from the day it was written. Writing a blind spot down makes it arguable; it does not make it safe, and a stated omission covering the DEFAULT path is not an omission, it is a hole. Fixed, and the gate now has two cases pinning it, both seen failing first. The phase ran THREE review rounds against a two-round budget; the third was authorized by the product owner through the escalation the cap exists to force.
>
> Previously, and still worth reading once: build phase 4.3, the GraphQL surface via Strawberry, is DONE, merged as PR #48 on 2026-08-17. A typed request/response surface at `/graphql` that WRAPS the one agent core rather than exposing the seven tools, registered accounts only, no live stream. It took SIX independent review rounds, every one of which found its worst defect inside the previous round's fix, and two of those were criticals in the same decision: whether an exception is safe to show a caller. Both came from using a PROXY for that (the package a class was declared in, then its class family, then the phase the error came from) instead of checking the text. The fourth attempt checks the text itself, against shapes captured from the installed library. Fourteen vacuous gate arms were found across the phase, six written by the lead. The phase only converged once it was given a MERGE BAR (a critical or a reachable major blocks; minors and latent findings are tracked), which it had run five rounds without. It pointed at build phase 4.4, the KGX export, which has since shipped.
...
> Build phase 4.6, feedback capture, is MERGED as PR #54 on 2026-08-21, `develop` at 108e19d, phase branch deleted. All twelve tickets: interaction capture into `interactions`, the deterministic rubric outcome, coverage tags (the `concept:` half only, see below), the owner-scoped writer, the run-epilogue dispatch, the feedback endpoint and its wired frontend, the weekly review ritual, few-shot promotion, and the runbook. Closes F-2.0-04 and F-2.0-10 and takes F-4.10-A-14 and F-4.1-A-15 with it. Suite: 6 failed, 3626 passed, 130 skipped, 1 xfailed, the 6 being pre-existing live-network arms proven at the branch point by a full-suite baseline run. WHAT THE REVIEW COST, and why this phase is worth reading before opening any other: THIRTY findings, TWO CRITICALS both reachable in the shipped product, and one Rule 4 stop when a finding landed inside the phase's own fix. Both criticals defeated the daily caps, which is the exact thing this phase existed to make work: one NUL byte in a question, or simply pressing Stop, made a query free and uncounted. Neither control was wrong alone, and that is the lesson: Section 16 requires capture to be best-effort and never block an answer, the caps count captured rows, and composed, anything that prevents a row raises that caller's cap to infinity. FOUR defects were found in the lead's own premise gate, three of them vacuous arms in three distinct shapes and one arm that works and measures the wrong thing. One ticket was REJECTED outright because its premise was false: every surface already minted a server-side `trace_id`, and building it broke a locked spec section. One fix was REVERTED by product-owner decision because it had zero verification and bypassed the MCP anti-leak allowlist. TWO PRODUCT-OWNER DECISIONS ARE STILL OPEN, both named in "The next session starts here" above: durable cross-reload history, and the locked spec's self-contradiction on where `trace_id` is minted. THE LIVE PREMISE ARMS DID NOT RUN, the graph tunnel being down; they are unverified rather than passed.
>
> Previously, and still worth reading once: build phase 4.3, the GraphQL surface via Strawberry, is DONE, merged as PR #48 on 2026-08-17. A typed request/response surface at `/graphql` that WRAPS the one agent core rather than exposing the seven tools, registered accounts only, no live stream. It took SIX independent review rounds, every one of which found its worst defect inside the previous round's fix, and two of those were criticals in the same decision: whether an exception is safe to show a caller. Both came from using a PROXY for that (the package a class was declared in, then its class family, then the phase the error came from) instead of checking the text. The fourth attempt checks the text itself, against shapes captured from the installed library. Fourteen vacuous gate arms were found across the phase, six written by the lead. The phase only converged once it was given a MERGE BAR (a critical or a reachable major blocks; minors and latent findings are tracked), which it had run five rounds without. It pointed at build phase 4.4, the KGX export, which has since shipped.
...
BUILD PHASE 4.6, FEEDBACK CAPTURE, IS MERGED as PR #54 on 2026-08-21. `develop` is clean and pushed, the phase branch is deleted, and nothing is in flight.

WHAT SHIPPED, in one line each. Every completed query now writes exactly one `interactions` row attributed to the principal that asked, on every surface and at all four trust outcomes. A caller can rate their own answer and nobody else's, through a REST endpoint and a wired frontend control. A weekly review script surfaces flagged rows and writes `cq_candidates`; a promotion script turns approved ones into few-shot examples and eval cases. The two daily caps that had read an always-empty table since build phase 2.0 can finally fire.

FOUR DECISIONS WERE SETTLED ON 2026-08-21, after the merge, and all four are logged in `DECISIONS.md`. They are recorded here because each one changes what a later phase does, and because two of them are traps that will be re-encountered rather than tidy closures:

- Durable cross-reload history gets its OWN board phase, 4.13, rather than being folded into 4.7 or retrofitted into the merged 4.6. `frontend/src/stubs/registry.ts` named 4.6 as history's owner in two notes, written by build phases 4.8 and 4.10, while Section 25 never named it as a 4.6 deliverable. Those notes were a guess about ownership, not a decision, and both are now corrected to point at 4.13.
- WHERE `trace_id` IS MINTED IS SETTLED IN FAVOUR OF SECTION 13.1: the adapter generates it and it doubles as `run_id`. Section 20.1's "minted at the Guardrail step" is the losing half and gets edited at the reconciliation pause, since both sections are locked. READ THIS BEFORE WRITING ANY TICKET FROM SECTION 20.1: build phase 4.6 wrote one, built it, and it split `run_id` from `trace_id`, broke the run registry's stated identity, and forced a builder to rewrite a test that was correctly encoding 13.1. That ticket was rejected and the code reverted. The next reader of 20.1 alone will repeat it.
- A caller-pasted credential in `query_text` is handled by NARROWING the promise rather than by building a redactor. The phase constraint now reads "no secret THIS SYSTEM holds". A redactor was rejected on its failure modes: it is a detection heuristic, a false negative gives false comfort about a key still stored, and a false positive mangles the exact question the reviewer needs to read. Redaction of caller-pasted secrets moves to the production track with the enterprise-security work.
- Two carried findings got owners. Concurrent promotion corrupting `orchestrator/few_shot_examples.json` goes to build phase 4.7, the phase that first READS that file at process start, rather than to 6.1 where the first symptom would be a startup failure in unrelated work. And `tracker/check_learnings_coverage.py` was FIXED on the spot rather than scheduled, because its failure mode was a FALSE PASS: it reported "no confirmed or closed findings, nothing to cover" against a ledger of thirty findings including two criticals. It recognised only the narrative `### F-x:` form and could not see a table-row ledger at all, which is exactly what build phase 3.3 flagged and which then stayed open. It now finds 19 verified findings where it found 0, and both directions are mutation-proven.

THE LIVE PREMISE ARMS DID NOT RUN. `tracker/preflight.py` reports `graph down TCP 15432 connection refused`, so P1b, P9 and P12 skip. They are UNVERIFIED, not passed, and the judge scored them that way. Closing this phase on evidence needs either that tunnel reopened or an explicit product-owner acceptance that three arms ship unrun. The hand-opened SSH tunnel is exactly the fragility build phase 4.11 removes, and 4.11's dependency is already met.

WHAT THE REVIEW COST, and it is the most transferable thing this phase produced. THREE ROUNDS, thirty findings, two criticals both reachable in the shipped product, and one Rule 4 stop when a finding landed inside the phase's own fix.

- Both criticals defeated the daily caps, which is the exact thing this phase existed to make work. One NUL byte in a question, or simply pressing Stop, made a query free and uncounted. Neither control was wrong on its own: Section 16 requires capture to be best-effort and never block an answer, and the caps count captured rows. Composed, anything that prevents a row raises that caller's cap to infinity.
- The blocker at round 2 was a finding the round-1 adversary had already filed. It became a blocker only because the lead acted on the adversary's findings through agent briefs and never entered ANY of the fifteen in the ledger, so nine had no owner. Filed as R-01 and now transcribed.
- FOUR defects were found in the lead's own premise gate: three vacuous arms in three distinct shapes (an arm that grepped its own assertions, an arm naming a control that three layers masked, and an arm that declared a control and never installed it), plus one arm that is not vacuous at all and measures the wrong thing.

Full record: `tracker/phase_4.6.md` (30 findings), `tracker/phase_4.6_judge_report.md`, `tracker/phase_4.6_adversary_report.md`, `tracker/phase_4.6_reverify_report.md`.

NEXT: OPEN BUILD PHASE 4.11, THE READ-ONLY HTTPS GRAPH QUERY SERVICE. Taken ahead of build phase 4.7 by product-owner decision on 2026-08-21, logged in `DECISIONS.md`. Build phase 4.6 merged as PR #54 and `develop` is clean and pushed.

WHY IT JUMPED THE QUEUE, and it is measurement rather than preference. The hand-opened SSH tunnel is a single point of failure for this project's entire live verification surface, and 4.6 paid for it twice in one week: the tunnel was down for two days earlier in the week, and down again through all three of 4.6's review rounds, so that phase merged with three live premise-gate arms unrun and accepted as such. Every tool phase and every premise gate from here inherits that fragility and pays again. The argument for taking 4.11 sooner was made and REJECTED once, on 2026-08-21, when the order was settled in favour of 4.6; what changed is that 4.6 then demonstrated the cost instead of predicting it.

WHAT IT DELIVERS, per Section 24, which specifies it in full: a single endpoint fronting Layer 1 over HTTPS, co-located on the Hetzner box, taking an already-validated parameterized Cypher payload, re-running the forbidden-keyword and edge-label checks SERVER-SIDE rather than trusting the caller's validator, on the existing read-only `kg_reader` role, behind a bearer credential (`GRAPH_QUERY_TOKEN`) because Railway does not guarantee a static egress IP. Decision D already names it the v1 Layer 1 transport. It carries its own three-part budget per `.claude/rules/tool-call-budgets.md`: a hard row limit, a per-call timeout matching `cypher_query`'s own 30 seconds, and a rate limit per caller, all three bounding a runaway query on both sides of the hop.

WHY THIS PHASE EXISTS AT ALL, which is worth knowing before opening it: Section 24 says the token is populated "when the service is built (Section 25 build order)", and Section 25 never assigns it to any phase. This phase closes a dangling reference between two locked sections. It is a board-level insertion and is NOT in Section 25, the same deliberate exception build phases 4.8, 4.10 and 4.12 already set.

THEN build phase 4.7, competency-question routing, which is F-2.0-15's formal home by the product-owner decision of 2026-08-10 and where `think_node` stops being a stub. Read that finding before opening it: `query_class` is hardcoded to `"lookup"` on every query, so every `interactions` row build phase 4.6 writes carries that known-wrong value, and `plan_node` compensates by scanning the question for a capitalised token and guessing it is a gene symbol, which breaks four of the seven must-pass moat questions. That is the product's worst known quality gap and it now waits one phase longer, accepted deliberately on the reasoning that 4.7's own verification is worth more once the graph is reachable without a person remembering to open an SSH session. Build phase 4.6 also leaves 4.7 a versioned few-shot pool file that nothing reads yet, which is the correct seam and not an oversight.
...
Start there, not with build phase 4.6. Dispatch a judge and an adversary against the branch, briefed that the newest code is the most dangerous code. Point them first at:

- The completeness repair in `core/graph.py`'s `write_node`. It sits on the most safety-critical path in the product, it fires a SECOND Synth call when an answer omits a finding, and it decides whether to keep the regeneration by comparing omission counts.
- `core/session_memory.py`'s ownership check, which closes F-4.1-A-15. Read and write deliberately answer a non-owner differently, and that asymmetry deserves an adversary.
- The uuid5 session-row mapping. Build phase 4.6 must reuse it for `interactions.session_id` or the two will disagree about which row a conversation is.

Ten findings are recorded in `tracker/phase_4.5.md`, three of them defects in the gate itself and one in the harness (F-4.5-01: `tracker/preflight.py` never loads `.env`, so it reports the graph unverifiable in the normal case and still exits READY).

Superseded, kept because it is what this section said before the phase ran: "OPEN BUILD PHASE 4.5, personalization and session memory. Nothing is in flight; `develop` is clean at `c27fefa`, build phase 4.4 merged as PR #51, and every gate is green." 
...
Final gates: 3763 Python tests (3188 passing, the same six live-network-gated cases carried since build phase 4.0), the GraphQL package alone 206 to 300 tests, 181 frontend, ruff clean, doc drift 0 stale 0 structural.

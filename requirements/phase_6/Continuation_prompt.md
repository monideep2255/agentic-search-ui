...
> BOTH HARD BLOCKERS ON BUILD PHASE 4.12 ARE CLOSED, 2026-08-24, each on its own branch with a trigger exactly as build phase 4.7 filed them. F-4.7-A-02 closed by PR #59 (`fix/a02-discontinued-gene-record`): a discontinued NCBI gene record no longer answers as its successor. F-4.7-A-01 closed by PR #60 (`fix/a01-injection-guardrail`): a plain-English parenthetical no longer steers which gene is looked up, and F-4.7-A-05's `guardrail/classifier.py` half goes with it. THE NEXT ACTION IS TO OPEN BUILD PHASE 4.12, the demo deployment on Railway. Nothing gates it now except the security scan it already owns, whose trigger is exposure, and the product owner's own Railway provisioning. MEASURED, NOT ASSERTED, on `develop` with both merged: premise gates 12 of 12 and 24 of 24 live with none skipped; mutation harnesses 15 of 15 and 16 of 16, every arm broken in process and proven to go red; Guard-tier admission for the three injection payloads fell from the adversary's measured 12/18 to 0/18; suite `6 failed, 3887 passed, 152 skipped, 1 xfailed` against a baseline RE-MEASURED in a throwaway worktree at `4d759da` (`6 failed, 3826 passed`), the same six pre-existing failures, so the two fixes do not interact; `ruff` clean; drift 0 stale 0 structural. END TO END, LIVE: the injection payload is refused as `injection` at $0.00 with NO model call reached, `BRCA3` refuses naming `NCBIGene:675`, and the two controls (`BRCA1`, and Q1 which names a coordinate range in the same surface shape as the attack) both still answer normally, 3 of 3 and 1 of 1. THE TWO MOST TRANSFERABLE RESULTS, both in `LEARNINGS.md`'s 2026-08-24 entries: a TICKET'S STATED PREMISE IS AN ASSEMBLY STEP and must be verified before the fix is written, because the board and the adversary report agreed with each other and were both wrong about where the F-4.7-A-02 fix goes, in a way that would have shipped an inert fix that looked correct; and A MUTATION THAT FINDS NOTHING IS EVIDENCE ABOUT THE GATE, because the one mutation that produced no red arm had found a hole in the gate's own control set rather than proving the control unnecessary. FOUR mutations across the two branches did not actually mutate, each reported by pytest as "DID NOT RAISE", which is indistinguishable from a control that failed to break; every mutation now carries its own populate-check.
...
> Both hard blockers on build phase 4.12 are closed, 2026-08-24. F-4.7-A-02 (PR #59): a gene symbol resolving to a discontinued NCBI record now contributes NO CURIE, so no answer about a substituted gene is reachable, and the refusal names the successor. Product-owner decision over refusing silently or answering with a disclosure. F-4.7-A-01 (PR #60): the primary control is DETERMINISTIC, in `guardrail/prefilter.py` ahead of any model call, because the Guard prompt already carried the abstract rule in almost the finding's own words and admitted the payload 6 of 6 anyway. Two patterns, each with a discriminator chosen for precision rather than coverage: the colon on a forged processing header, so `processing note:` matches and `RNA processing:` does not, and the underscore on this system's own control vocabulary, so `clinicaltrials_search` matches and "ClinicalTrials search" stays admitted. Prompt hardening and a per-request unforgeable query tag are defence in depth, not the fix.
>
...
BOTH HARD BLOCKERS ON BUILD PHASE 4.12 ARE CLOSED, 2026-08-24: PR #59 on `fix/a02-discontinued-gene-record` and PR #60 on `fix/a01-injection-guardrail`, merged in that order, with #60 merging `develop` in first to resolve a doc-count conflict (no source or test file conflicted; the two fixes touch disjoint code). THE NEXT ACTION IS TO OPEN BUILD PHASE 4.12, the demo deployment on Railway, whose dependency was met by build phase 4.11 and stays met, and which now has nothing else in front of it.
...
Build phase 4.12, the demo deployment, is what opens now, and NOTHING gates it any more: both fix branches below merged on 2026-08-24, PR #59 and PR #60, and their rows are kept struck through rather than deleted so the trail from build phase 4.7's adversary round to their closure stays readable, and so the residuals each one carried stay visible. Build phase 4.7 merged as PR #56 on 2026-08-23, and the section above carries its full account. With that merge, ALL SIX of Step 6.3's delivery surfaces are done, and so are personalization, feedback capture and competency-question routing. Section 25's 4.x group is finished apart from build phase 4.12, the board-level insertion that owns the demo deployment. This paragraph carried two contradictory counts and a stale "4.4 is next" table until 2026-08-23, from being appended to across four merges instead of rewritten, which is the same failure this file's own Step 4 rule was written to stop.

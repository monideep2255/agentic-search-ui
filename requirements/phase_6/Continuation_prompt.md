...
- BUILD PHASE 4.16, THE UI DEFECTS FROM THE LIVE DEMO, IS MERGED as PR #63 on 2026-08-25 and IS LIVE ON THE DEMO.
- All SEVEN reported defects are closed and VERIFIED ON THE DEPLOYED PRODUCT, not asserted: the answer screen now reads `2 tools` rather than `0 tools`, `Single source, not independently confirmed` rather than blaming the reader's question, `MedGen C0346153` rather than repeating its own prefix, and the thumbs-down sits inside its button.
- THE NEXT ACTION IS BUILD PHASE 4.14, CONTINUOUS INTEGRATION, which is now the largest unguarded risk and got sharper with this merge: it auto-deployed to a public URL and nothing ran the 3947-test suite on the way.
- THE TOP DEFECT WAS BACKEND, NOT FRONTEND, and it was measured before a ticket was written: timestamping each SSE frame off the deployed API showed guard at 0.15s, think at 0.15s, plan at 1.41s, then 10.9 SECONDS OF SILENCE, then the whole answer inside 10ms.
- `sink.emit` was never called with `tool_start` or `tool_result`; both are in the contract and the CLI, MCP and GraphQL adapters all handle them, so only the producer was missing and the Act step was invisible on every surface.
- `useRunView`'s own docstring had PREDICTED the consequence in 2026-08-12 and it was read as a design principle rather than a live symptom.
- THE CONSTRAINT WAS THE NODE BOUNDARY, NOT THE MISSING EMIT: events flush at node return, so the obvious fix would have delivered every tool frame in one burst before the answer and left the silence exactly as long.
- AFTER THE MERGE, RE-MEASURE THE DEPLOYED ANSWER SCREEN: four of the six answer-presentation gaps recorded in `tracker/phase_4.16.md` trace to those missing tool events and should close on deploy without further work.
- THEN build phase 4.14, CONTINUOUS INTEGRATION, which is still not started and is now the largest unguarded risk: a merge to `develop` auto-deploys to a public URL and NOTHING runs the 3947-test suite on a pull request.
...
- THE NEXT ACTION IS BUILD PHASE 4.14, CONTINUOUS INTEGRATION (CI), now the largest unguarded risk: a merge to `develop` auto-deploys to a public URL and nothing runs the test suite on a pull request.
...
- Python tests: 4107 (3947 passing, 159 skipped, 1 xfailed, ZERO FAILED) on `phase/4.16-ui-streaming-fidelity`, re-measured 2026-08-25 rather than carried forward.
...
- 4107 Python tests (3188 passing, the same six live-network-gated cases carried since build phase 4.0)

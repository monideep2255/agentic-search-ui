...
> BUILD PHASE 4.12, THE DEMO DEPLOYMENT, IS MERGED and CLOSED at PR #62 on 2026-08-24, its code having merged first as PR #61 and its live-demo defect log following on a `fix/4.12-` branch, the same two-branch shape build phase 3.1 closed with. THE PRODUCT IS LIVE AND ANSWERING at https://search-agent-web-production.up.railway.app. MEASURED on the deployed API rather than asserted: "Which diseases are associated with BRCA1?" returns a grounded answer with FIVE citations in 15.8 seconds, across a Layer 1 graph query and a Layer 2 NCBI confirmation. Railway CD is wired to `develop` for both services, so a merge now auto-deploys. THE NEXT ACTION IS THE UI WORK, which the live demo surfaced as six specific defects recorded in the product owner's own words in `tracker/phase_4.12.md`: streaming does not render incrementally, a second conversational turn is impossible, the search feels very slow, answer presentation is nothing like the approved design, the integrations page does not present the KGX, REST, CLI and MCP surfaces properly, and there is NO client-side routing so every page serves at `/`. START WITH STREAMING: defects 1 and 3 are plausibly the same thing, because the API emits `token` events throughout a 15.8-second answer and a reader who sees none of them incrementally experiences exactly "super slow". Check SSE consumption before anyone optimises latency. WHAT 4.12 COST AND TAUGHT: five defects had to be fixed before the demo answered at all, every one found by RUNNING it rather than reading it, and the phase caused a regression while fixing its own scope, then fixed the CATEGORY rather than the instance. Its most transferable result is that a green status field is not evidence: `Online` was true of a service running a second copy of the wrong application, and three separate "the fix did not work" readings were really "the config was never applied". BUILD PHASE 4.14, CI, IS NEWLY INSERTED and is the other candidate for next: CD now auto-deploys to a public URL and nothing runs the 3930-test suite on a pull request.
...
> Build phase 4.12 is merged and the product is live, with CD watching `develop`. Next is the UI work, six defects recorded verbatim in `tracker/phase_4.12.md`, starting with streaming. Build phase 4.14, continuous integration, was inserted the same day and is the other candidate: Section 24's ten gates already specify it, and nothing currently runs the suite on a pull request. Build phase 4.13, durable history, is still open and unstarted.
>
...
BUILD PHASE 4.12, THE DEMO DEPLOYMENT, IS MERGED as PR #61 on 2026-08-24, and the product is LIVE at https://search-agent-web-production.up.railway.app. THE NEXT ACTION IS THE UI WORK, starting with streaming, per the six defects in `tracker/phase_4.12.md`.
...
Build phase 4.12 is MERGED and the product is live. What follows is a choice between two open phases and one carried defect set, and the product owner has not yet ranked them against each other.

1. THE UI DEFECTS, six of them, recorded in the product owner's own words in `tracker/phase_4.12.md`. Start with streaming: defects 1 and 3 are plausibly one defect, since the API emits `token` events across a 15.8-second answer and a reader seeing none of them incrementally experiences "super slow". Defect 4, answer presentation, has a defined source of truth in `docs/build/design/Design_to_build_workflow.md` and must not be guessed at. Defect 6 needs a router only: the app is served by `serve -s dist`, which is SPA mode, so deep links resolve once routes exist.
2. BUILD PHASE 4.14, CI, newly inserted 2026-08-24. Section 24's ten gates, already specified. Pulled forward because 4.12 wired CD, so a merge to `develop` auto-deploys to a public URL and nothing runs the 3930-test suite on a pull request.
...
Final gates: 4102 Python tests (3188 passing, the same six live-network-gated cases carried since build phase 4.0), the GraphQL package alone 206 to 300 tests, 181 frontend, ruff clean, doc drift 0 stale 0 structural.

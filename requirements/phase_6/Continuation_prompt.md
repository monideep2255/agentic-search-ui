...
TWO CRITICALS MERGE OPEN, and BOTH ARE HARD BLOCKERS ON BUILD PHASE 4.12 BEFORE ANY PUBLIC URL EXISTS, gated by the security scan 4.12 already runs on exposure:

- `fix/a01-injection-guardrail`. A plain-English parenthetical with no tags and no operator framing still steers which gene is looked up. The guardrail admitted it 6 of 6, so this is Guard-tier injection classification failing, Section 10.4, which build phase 3.0 owns rather than 4.7. What 4.7 DID close: the query block now carries a per-request random tag, so a question can no longer forge or close its own delimiter, and the trust asymmetry that let typed CURIE-shaped strings evict live-confirmed entities is fixed. `guardrail/classifier.py:229` still carries the identical forgeable tag.
- `fix/a02-discontinued-gene-record`. A retired gene name is answered as its replacement with no disclosure. Established PRE-EXISTING rather than counted against 4.7: the retired regex matched it too, so the identical wrong answer was reachable before this phase.

WHAT 4.12 MUST KNOW BEFORE IT WRITES ANYTHING:

- Both branches above are prerequisites, not suggestions. The security scan's trigger is exposure, and 4.12 is the first exposure.

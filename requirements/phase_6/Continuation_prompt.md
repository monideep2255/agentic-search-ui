...
NEXT, once 4.6 merges: build phase 4.7, competency-question routing, which is F-2.0-15's formal home by the product-owner decision of 2026-08-10. It is where `think_node` stops being a stub. Read that finding first: `query_class` is hardcoded to `"lookup"` on every query, so every `interactions` row 4.6 writes carries that known-wrong value, and `plan_node` compensates by scanning the question for a capitalised token and guessing it is a gene symbol, which breaks four of the seven must-pass moat questions. 4.6 also leaves 4.7 a versioned few-shot pool file that nothing reads yet, which is the correct seam and not an oversight.

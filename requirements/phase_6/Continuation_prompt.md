...
> Build phase 4.4, the KGX export, is OPEN on `phase/4.4-kgx-export` and BLOCKED on the Layer 1 transport. Its scope was undecided at open, since Section 25 and the PRD scope it only negatively, so the product owner decided it on 2026-08-19: a QUERY-SCOPED SUBGRAPH export, seed CURIEs plus bounded hops writing `nodes.tsv`, `edges.tsv` and a manifest, not a full-graph snapshot. That deliverable was forbidden outright by the `file-protection` rule's ban on KGX exporters, so the rule was amended by direction of data flow: writing KGX into the graph is still Systems 1 and 2 and still banned here, reading a scoped subgraph out is a delivery surface. Six tickets exist in `tracker/phase_4.4.md`. T-4.4-06, the rule amendment, is built and in review at `ade54c9`. The other five all read Layer 1, and preflight reports the graph DOWN: nothing listens on 127.0.0.1:15432 on either loopback family, the host answers on port 22, and its Postgres port is closed by design, so this is an unopened forward rather than an outage. NEXT: open the forward from a terminal (the command is in "The next session starts here"; it cannot be run from inside an agent session), re-probe with `python3 tracker/preflight.py`, then work T-4.4-01, the premise gate, and WATCH IT FAIL before any other ticket opens.
>
> Previously, and still worth reading once: build phase 4.3, the GraphQL surface via Strawberry, is DONE, merged as PR #48 on 2026-08-17. A typed request/response surface at `/graphql` that WRAPS the one agent core rather than exposing the seven tools, registered accounts only, no live stream. It took SIX independent review rounds, every one of which found its worst defect inside the previous round's fix, and two of those were criticals in the same decision: whether an exception is safe to show a caller. Both came from using a PROXY for that (the package a class was declared in, then its class family, then the phase the error came from) instead of checking the text. The fourth attempt checks the text itself, against shapes captured from the installed library. Fourteen vacuous gate arms were found across the phase, six written by the lead. The phase only converged once it was given a MERGE BAR (a critical or a reachable major blocks; minors and latent findings are tracked), which it had run five rounds without. NEXT: build phase 4.4, the KGX export. See "The next session starts here".
...
- Python tests: 3484 (3188 passing, 113 skipped, 1 xfailed, 6 failed; the 6 are `test_citation_trust_full_premise.py`'s live-network-opt-in-gated cases, confirmed not a regression, identical set carried since build phase 4.0's close)
- Frontend tests: 181

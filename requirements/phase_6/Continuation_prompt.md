...
- WORK "NEXT, IN ORDER" FROM ITS CUTOFF. The next action is owned by `testing/UI_fix_plan.md`, section "Where we stopped", REWRITTEN LATE ON 2026-09-22 after the list's first two items closed. Its item 1 is now THE ISOLATE SHAPE (G-035), the one golden shape that still never answers, and it opens with a DISCUSSION of what the question should return rather than with code, because no mode of the pathogen tool searches isolates by an AMR gene, a genotype or a description; a new mode on the existing tool comes before any Think rule. WHAT CLOSED ON 2026-09-22 AFTER THE RETEST, all on develop and all awaiting the product owner's retest (items 7 to 16 in `testing/Shipped_2026-09-22.md`): the two lost searches and the GEO search (`27d68ae`, `b6cd025`); the coordinate range (`66b3811`, then `e477077` and `c72b8a7`); THE CALL CEILING MEASURED (`df657ad` put the query's Layer 2 and 3 count on the done event; 24 runs over eight shapes, every one answered, none refused, the worst pass 17 of 20, so the ceiling stays at twenty, and the variable part is the model's spans the FIRST time a process sees them, since Think caches every confirmed span for the life of the process, evidence `testing/Developer/reports/2026-09-22_call_ceiling/findings.md`); the measurement's own finding (`d8619bc`, a question's own words such as "condition" or "tumour" are never searched as a disease name, after two answers had listed eight MedGen records the person never named as "entities named in the question"; the guard held on four live retests); and THE BIOPROJECT ACCESSION (`a64c44e`: a fixed rule recognises a BioProject, BioSample, SRA or assembly accession, Think resolves it live with one search and one link per linked database, Plan plans the record's summary and the summaries of what it links to with no graph call, and an accession NCBI does not have is answered "was not found in NCBI. Check the accession and ask again"; G-007 3 of 3 live at a fixed eight calls; two follow-up fixes from the live runs, `d882856`, an SRA run shown as its accession rather than NCBI's markup, and `649750c`, a resolved accession counts as the question's subject so "which runs come from it?" no longer asks which gene; re-run after both fixes at `649750c`: the BioSample question 2 of 2 answered with its record and five of its runs as accessions, and the golden question again with the run shown as SRR9496657). The day's summary and the retest list are `testing/Shipped_2026-09-22.md`.

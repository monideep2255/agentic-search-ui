...
- RUN SETS 8 AND 9 IN PARALLEL, THEN RELEASE. As of commit `ac3f468` on develop (2026-09-13), the UI fix loop has closed fix sets 1 to 7, all approved by the product owner, and item 10.1 of set 10 (the same question returns the same sources every time) is built and live. The next action is `requirements/phase_6/Session_prompt_sets_8_and_9.md`: run set 8 (all three layers, the scientists' handoff, plus the GCK extraction fallback) and set 9 (two answer modes, streaming, formatting, one trust line) as two parallel sub-agents with the file ownership that file states, land both, verify live, get the product owner's approval, then release to production so the prototype can be shared, and only then open the rest of set 10 (10.2 to 10.4). `testing/UI_fix_plan.md` is the single owner of per-item status.

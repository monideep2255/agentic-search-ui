...
| 4.9 | `phase/4.9-answer-screen-fidelity` | Answer-screen and chrome fidelity against the design system's prototype: collapsed sources, the provenance spine, the reasoning trace, and the chrome the 4.8 restyle left approximate. Added 2026-08-18 retroactively, product-owner directive, not part of the original locked build order (see the note below the table) | 4.8 | v1 |
| 4.10 | `phase/4.10-guest-allowance` | The anonymous run path and the server-side guest allowance, so the product is usable without creating an account first. Added 2026-08-18 retroactively, product-owner directive, not part of the original locked build order (see the note below the table) | 1.1, 1.2 | v1 |
...
Phases 4.9 and 4.10, added 2026-08-18, retroactively: both were built and merged before they appeared here, 4.9 as PR #44 and 4.10 as PR #46, and both were tracked only in `requirements/Plan.md`, `tracker/`, and `requirements/phase_6/Continuation_prompt.md`. That is the defect this edit fixes rather than the exception it records. This table is what `bossman-mode` Step 1 reads as the single source of truth for the numbered build phases, so a table missing two merged phases meant the opening step could not see work that had already shipped, and 4.9 in particular is a dependency of any later phase touching the answer screen. Recording them late is worse than recording them on time and better than leaving them absent, which is why they are here with the same product-owner exception 4.8 carries. 4.10 also ran ahead of 4.2 through 4.7 by a product-owner decision on 2026-08-14, for the reason its row states: until it existed, nobody could use the product without an account.
